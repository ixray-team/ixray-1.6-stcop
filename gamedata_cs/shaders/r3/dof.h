...
//	half3 	img 	= tex2D		(s_image, center);
	half3 	img 	= s_image.Sample( smp_rtlinear, center);
	return	img;
}
...
//	half3	sum 	= tex2D(s_image,center);
	half3	sum 	= s_image.Sample( smp_nofilter, center);
	half 	contrib	= 1.h;
...
//		half4	tap_color	= s_image.Sample( smp_rtlinear, tap );
		half4	tap_color	= s_image.Sample( smp_nofilter, tap );
		half 	tap_depth 	= s_position.Sample( smp_nofilter, tap).z;
