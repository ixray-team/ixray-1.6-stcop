...
//////////////////////////////////////////////////////////////////////////////////////////
// Lighting formulas			// 
float4 	plight_infinity		(half m, float3 point, float3 normal, float3 light_direction)       				{
...
float4 	plight_infinity2	(half m, float3 point, float3 normal, float3 light_direction)       				{
  	float3 N		= normal;							// normal 
  	float3 V 	= -normalize		(point);		// vector2eye
  	float3 L 	= -light_direction;					// vector2light
 	float3 H		= normalize			(L+V);			// half-angle-vector 
...
float4 	plight_local		(half m, float3 point, float3 normal, float3 light_position, half light_range_rsq, out float rsqr)  {
  float3 N		= normal;							// normal 
