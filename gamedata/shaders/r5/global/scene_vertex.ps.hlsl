...
LegacySceneGBufferOutput Main(in OutputLegacySceneVertex input)
{
    ...
    const float4 BaseColor = Texture.Sample(Sampler, input.UV);

    TiramisuGBufferData Data;
    Data.BaseColor = BaseColor.rgb * input.Color.rgb;
