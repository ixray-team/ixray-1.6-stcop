...
// O.Roughness = 1.0f;
	...
	Jitter.y *= 0.7f; // Bias like screen space stochastic reflections 2015
	
	// O.Roughness = saturate(O.Roughness * 1.1f - 0.1f);
	
	//O.Normal.xyz = normalize(normalize(cross(ddx(O.PointReal.xyz), ddy(O.PointReal.xyz))) + O.Normal);
	...
	if (dot(RefRef, O.Normal) < 0.0f)
    {
        RefRef = normalize(RefRef + O.Normal * 0.5f);
    }
