...
#define VSGI_SAMPLES 20
#define VSGI_DIRS 4

static const float PackedNormalDeltaLUT[VSGI_SAMPLES] = {
	0.077526f, 0.074312f, 0.073479f, 0.072234f,
	0.070586f, 0.068543f, 0.066116f, 0.063320f,
	0.060170f, 0.056683f, 0.052879f, 0.048780f,
	0.044407f, 0.039787f, 0.034943f, 0.029905f,
	0.024699f, 0.019355f, 0.013903f, 0.008373f
};

static const float2 DirectionScale[VSGI_DIRS] = {
	{+1.0f, +1.0f}, {+1.0f, -1.0f}, 
	{-1.0f, +1.0f}, {-1.0f, -1.0f}
};
...
float4 main(PSInput I) : SV_Target
{
	...
	float4 Jitter = 0.5f - s_blue_noise[uint3(uint2(I.hpos.xy) % 128, uint(m_taa_jitter.w) % 32)];
	...
	float3 Sample = O.PointReal;
	
	float4 Global = 0.0f;
	Global.w = 0.00001f;
		
	for(uint d = 0; d < VSGI_DIRS; ++d)
	{
		float MaxDelta = -1.0f;
		float2 SampleDirection = DirectionScale[d] * Direction;
		
		float L = PackedNormalDeltaLUT[0];
		
		for(uint i = 1; i < VSGI_SAMPLES; ++i)
		{
			L += PackedNormalDeltaLUT[i] * lerp(0.5f, 1.5f, Hash(dot(sin(Sample.xyz * timers.x), float3(12.989, 42.364, 78.233))));
			
			Sample = UnPackNormalVector(SampleDirection * L);
			...
			float Delta = dot(LDir * Scale, -ViewD);
			...
			if(Delta >= MaxDelta && rcp(Scale) > 0.01f) {
				...
				float Att = rcp(1.0f + dot(O.Normal, LDir) * rcp(Shadow + 0.01f));
				
				Global.xyz += envColor * Att * saturate(Delta); /// * Fog;
				MaxDelta = Delta;
			}
