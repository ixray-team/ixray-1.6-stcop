...
float4 prev_sbones_array[256-22];
...
v_model skinning_0	(v_model_skinned_0	v)
{
    ...
    o.PrevP     = float4(v.P.xyz*(12.f / 32768.f), 1.f);
...
v_model skinning_1 	(v_model_skinned_1	v)
{
	...
	float4  prev_m0 	= prev_sbones_array[mid+0];
	float4  prev_m1 	= prev_sbones_array[mid+1];
	float4  prev_m2 	= prev_sbones_array[mid+2];

	// skinning
	v_model 	o;
	o.P 		= skinning_pos(v.P, m0,m1,m2 );
	o.PrevP 	= skinning_pos(v.P, prev_m0, prev_m1, prev_m2);
...
v_model skinning_2 	(v_model_skinned_2	v)
{
	...
	float4  prev_m0_0 	= prev_sbones_array[id_0+0];
	float4  prev_m1_0 	= prev_sbones_array[id_0+1];
	float4  prev_m2_0 	= prev_sbones_array[id_0+2];
	float4  prev_m0_1 	= prev_sbones_array[id_1+0];
	float4  prev_m1_1 	= prev_sbones_array[id_1+1];
	float4  prev_m2_1 	= prev_sbones_array[id_1+2];
    ...
    float4  prev_m0  = lerp(prev_m0_0,prev_m0_1,w);
    float4  prev_m1  = lerp(prev_m1_0,prev_m1_1,w);
    float4  prev_m2  = lerp(prev_m2_0,prev_m2_1,w);
...
v_model skinning_3 	(v_model_skinned_3	v)
{
	...
	float4  prev_m1_1 	= prev_sbones_array[id_1+1];
	float4  prev_m2_1 	= prev_sbones_array[id_1+2];
	float4  prev_m0_2 	= prev_sbones_array[id_2+0];
	float4  prev_m1_2 	= prev_sbones_array[id_2+1];
	float4  prev_m2_2 	= prev_sbones_array[id_2+2];
	...
	float4  prev_m0 	= prev_m0_0*w0;
	float4  prev_m1 	= prev_m1_0*w0;
	float4  prev_m2 	= prev_m2_0*w0;

			prev_m0 	+= prev_m0_1*w1;
			prev_m1 	+= prev_m1_1*w1;
			prev_m2 	+= prev_m2_1*w1;

			prev_m0 	+= prev_m0_2*w2;
			prev_m1 	+= prev_m1_2*w2;
			prev_m2 	+= prev_m2_2*w2;
...
v_model skinning_4 	(v_model_skinned_4	v)
{
	...
	float4	prev_m[4][3];	//	[bone index][matrix row or column???]
	[unroll]
	for (int i=0; i<4; ++i) {		
		...
		for (int j=0; j<3; ++j){
			...
			prev_m[i][j] = prev_sbones_array[id[i]+j];
		}
	}
	...
	for (int i=1; i<4; ++i)
	{
		...
		m1 		+= m[i][1]*w[i];
		m2 		+= m[i][2]*w[i];
		prev_m0 += prev_m[i][0]*w[i];
		prev_m1 += prev_m[i][1]*w[i];
		prev_m2 += prev_m[i][2]*w[i];
	}
