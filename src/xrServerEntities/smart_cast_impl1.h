...
namespace SmartDynamicCast {
	...
	template <typename T>
		struct search_base {
			...
			typedef typename 
				selector<std::is_same<Base,typename Head::Head>::value>::result result;
		};
	...
	struct conversion_sequence {
		...
		struct list_iterator {
			...
			struct helper {
				...
				typedef typename
					selector<
					std::is_same<Loki::NullType,search_result>::value>::result result;
			};
		...
		struct selector {
			...
			typedef typename 
				_selector<
					!std::is_same<Loki::NullType,nearest>::value>::result result;
		};
