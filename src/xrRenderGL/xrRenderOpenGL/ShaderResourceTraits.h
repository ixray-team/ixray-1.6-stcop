...
#	include "ResourceManager.h"
	...
	struct ShaderTypeTraits<SHS>
	{
		...
		static IC const char* GetShaderExt() {return ".hs";}
		static IC const char* GetCompilationTarget() {return "hs_5_0";}
		static IC DXIface* CreateHWShader(DWORD const* buffer, size_t size)
		{
			DXIface* hs = 0;
		...
		static IC u32 GetShaderDest() {return RC_dest_hull;}
	};
	...
	struct ShaderTypeTraits<SDS>
	{
		...
		static IC const char* GetShaderExt() {return ".ds";}
		static IC const char* GetCompilationTarget() {return "ds_5_0";}
		static IC DXIface* CreateHWShader(DWORD const* buffer, size_t size)
		{
			DXIface* hs = 0;
		...
		static IC u32 GetShaderDest() {return RC_dest_domain;}
	};
	...
	struct ShaderTypeTraits<SCS>
	{
		...
		static IC const char* GetShaderExt() {return ".cs";}
		static IC const char* GetCompilationTarget() {return "cs_5_0";}
		static IC DXIface* CreateHWShader(DWORD const* buffer, size_t size)
		{
			DXIface* cs = 0;
		...
		static IC u32 GetShaderDest() {return RC_dest_compute;}
	};

	template<>
	IC CResourceManager::map_DS& CResourceManager::GetShaderMap(){return m_ds;}

	template<>
	IC CResourceManager::map_HS& CResourceManager::GetShaderMap(){return m_hs;}

	template<>
	IC CResourceManager::map_CS& CResourceManager::GetShaderMap(){return m_cs;}

    template<typename T>
	IC T* CResourceManager::CreateShader(const char* name)
	{
		...
		else
		{
			...
			// Compile
			HRESULT	const _hr			= ::Render->shader_compile_OpenGL(name,(DWORD const*)file->pointer(),file->length(), c_entry, c_target, D3D10_SHADER_PACK_MATRIX_ROW_MAJOR, (void*&)sh );
	...
	template<typename T>
	IC void CResourceManager::DestroyShader(const T* sh)
	{
		ShaderTypeTraits<T>::MapType& sh_map = GetShaderMap<ShaderTypeTraits<T>::MapType>();
