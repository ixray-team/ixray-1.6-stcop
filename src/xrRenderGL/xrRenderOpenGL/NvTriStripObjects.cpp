		...
		if(!AlreadyExists(faceInfo, faceInfos))
		{
			...
			if (edgeInfo01 == NULL){
				...
				if (edgeInfo01->m_face1 != NULL)	;
					//log_cryray_engine::Msg("! WARNING: BuildStripifyInfo: > 2 triangles on an edge... uncertain consequences");
				else
...
IC int NvStripifier::GetNextIndex(const WordVec &indices, NvFaceInfo *face){
	...
	if (fv0 != v0 && fv0 != v1){
		if ((fv1 != v0 && fv1 != v1) || (fv2 != v0 && fv2 != v1)){
			log_cryray_engine::Msg("! WARNING: GetNextIndex: Triangle doesn't have all of its vertices");
			log_cryray_engine::Msg("! WARNING: GetNextIndex: Duplicate triangle probably got us derailed");
		}
		return fv0;
	}
	if (fv1 != v0 && fv1 != v1){
	...
	// shouldn't get here
	log_cryray_engine::Msg("! WARNING: GetNextIndex: Duplicate triangle sent");
	return -1;
}
