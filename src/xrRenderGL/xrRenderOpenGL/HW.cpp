...
void	CHW::selectResolution	(u32 &dwWidth, u32 &dwHeight, BOOL bWindowed)
{
    fill_vid_mode_list			(this);
    {
        ...
        }else //check
        {
            ...
            if(_ParseItem(buff,xrAPI.vid_mode_token)==u32(-1)) //not found
            { //select safe
                xr_sprintf				(buff,sizeof(buff),"vid_mode %s", xrAPI.vid_mode_token[0].name);
...
//#endif
    ...
    BOOL  bWindowed			= bWindowed = (g_screenmode != 2);
...
u32	CHW::selectPresentInterval	()
{
    ...
    if (!p_engine_flags32.test(R3VSYNC_DEVICE))
    {
