...
SVS*	CResourceManager::_CreateVS		(LPCSTR _name)
{
	...
	else
	{
		...
		string_path					cname;
		strconcat					(sizeof(cname),cname, xrAPI.Render->getShaderPath(),_name,".vs");
...
SPS*	CResourceManager::_CreatePS			(LPCSTR name)
{
	...
	else
	{
		...
		// Open file
		string_path					cname;
        LPCSTR						shader_path = xrAPI.Render->getShaderPath();
...
//		LPCSTR						target		= NULL;
		...
		// vertex
		R_ASSERT2					(fs,cname);
		_hr = ::Render->shader_compile_OpenGL(name,LPCSTR(fs->pointer()),fs->length(), NULL, &Includer, c_entry, c_target, D3DXSHADER_DEBUG | D3DXSHADER_PACKMATRIX_ROWMAJOR /*| D3DXSHADER_PREFER_FLOW_CONTROL*/, &pShaderBuf, &pErrorBuf, NULL);
...
SPS*	CResourceManager::_CreatePS			(LPCSTR name)
{
	...
	else
	{
		...
		LPD3DXSHADER_CONSTANTTABLE	pConstants	= NULL;
		HRESULT						_hr			= S_OK;
		_hr = ::Render->shader_compile_OpenGL	(name,data,size, NULL, &Includer, c_entry, c_target, D3DXSHADER_DEBUG | D3DXSHADER_PACKMATRIX_ROWMAJOR, &pShaderBuf, &pErrorBuf, NULL);
