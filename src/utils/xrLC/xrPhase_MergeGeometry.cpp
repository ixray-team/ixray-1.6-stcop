...
{
	auto it = std::partition(g_XSplit.begin(), g_XSplit.end(), [](auto split)
							 { return !split->empty(); });
	std::for_each(it, g_XSplit.end(), [](auto& split)
				  { xr_delete(split); });
	...
	xr_vector<u32> GetCandidates(const Fvector& center, u32 min) const
	{
		...
		for (int dx = -1; dx <= 1; dx++)
		{
			for (int dz = -1; dz <= 1; dz++)
			{
				...
				if (i != hash.end())
				{
					std::copy_if(i->second.begin(), i->second.end(), std::back_inserter(result), [&](u32 id)
								 { return id > min; });
				}
			}
		}
...
void xrPhase_MergeGeometry_Tbb()
{
	...
	std::transform(mappings.begin(), mappings.end(), std::back_inserter(keys), [](const auto& x)
				   { return x.first; });
