...
void CEditableObject::FillBasicProps(LPCSTR pref, PropItemVec& items)
{
	...
	PHelper().CreateCaption		(items, PrepareKey(pref,"Version\\Creation Time"),AnsiString(Trim(AnsiString(ctime(&m_CreateTime)))).c_str());
	PHelper().CreateCaption		(items, PrepareKey(pref,"Version\\Modified Time"),AnsiString(Trim(AnsiString(ctime(&m_ModifTime)))).c_str());
