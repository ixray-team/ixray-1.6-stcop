...
)
{
	...
	for (unsigned int i = 0; i < ret; ++i)
	{
		...
		SURFACE(contact, i * skip)->mode = Tri.material;
	}
	if (ret && dGeomGetUserData(o1)->callback)
	{
		Fvector verts[3];
		T.GetVerts(verts);
		dGeomGetUserData(o1)->callback(Tri, verts, contact);
	}
	return ret;
}
...
)
{
	...
	// find number of contacts requested
	int maxc = flags & NUMC_MASK;
	maxc = std::clamp(maxc, 1, 3); // no more than 3 contacts per box allowed
...
#undef TEST
	...
	else
	{
		...
		if (gl_state.test(u8(flag & 0xff)))
			RETURN0;
		SideToGlClTriState(Tri.verts[iv0], Tri.verts[iv1]);
		contact->depth = outDepth;
		norm[0] = outAx[0] * signum;
		norm[1] = outAx[1] * signum;
		norm[2] = outAx[2] * signum;
