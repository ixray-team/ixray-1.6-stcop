...
)
{
	...
	if (ContactDepth >= 0)
	{
		...
		Contacts->g2 = Sphere;
		auto& Tri = T.GetTri();
		((dxGeomUserData*)dGeomGetData(Sphere))->tri_material = Tri.material;
		if (dGeomGetUserData(Sphere)->callback)
		{
			Fvector Verts[3];
			T.GetVerts(Verts);
			dGeomGetUserData(Sphere)->callback(Tri, Verts, Contacts);
		}
		SURFACE(Contacts, 0)->mode = Tri.material;
		//////////////////////////////////
		return 1;
	}
