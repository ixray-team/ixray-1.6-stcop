...
{
	for (auto& other_element : MapElements)
	{
...
void UIMinimapEditorForm::ShowElementList()
{
	...
	for (int i = 0; i < MapElements.size(); i++)
	{
		...
		if (MapElements[i].EdHidden)
		{
			status[0] = 'H';
		}
		else if (MapElements[i].EdLocked)
		{
			status[0] = 'L';
		}

		ImGui::PushID(i);
		bool is_selected = MapElements[i].EdSelected;
		if (ImGui::Selectable((status + MapElements[i].name).c_str(), is_selected))
		{
			SelectElement(MapElements[i]);
		}

		xr_string TexturePath = MapElements[i].TexturePath;
		if (TexturePath.empty())
		{
			TexturePath = "<none>";
		}

		if (ImGui::Selectable(TexturePath.c_str(), is_selected))
		{
			if (EFS.GetOpenName("$game_textures$", MapElements[i].TexturePath, false, nullptr, 0, "*.dds; *.tga"))
			{
				LoadTexture(MapElements[i], MapElements[i].TexturePath);
			}
		}
		ImGui::Separator();
...
void UIMinimapEditorForm::RenderCanvas()
{
	...
	ImVec2 bg_display_size(BackgroundRenderSize.x * m_Zoom, BackgroundRenderSize.y * m_Zoom);
	ImGui::GetWindowDrawList()->AddImage(BackgroundTexture->get_SRView()->GetRawSRV(), canvas_p0 + m_BackgroundPosition, canvas_p0 + m_BackgroundPosition + bg_display_size);
	...
	for (int i = 0; i < MapElements.size(); i++)
	{
		...
		if (element.EdSelected || m_AlwaysDrawBorder)
		{
			...
			ImU32 col = (element.EdSelected) ? IM_COL32(255, 130, 130, op_col) : IM_COL32(255, 255, 255, 100);
		...
		{
			return (ImGui::GetIO().MousePos.x >= (x1) && ImGui::GetIO().MousePos.x <= (x2) && ImGui::GetIO().MousePos.y >= (y1) && ImGui::GetIO().MousePos.y <= (y2));
		};
		...
		{
			move_zone = ckCursorZone((image_min.x + resize_off), (image_max.x - resize_off), (image_min.y + resize_off), (image_max.y - resize_off));
		...
		if (m_DebugView)
		{
			...
			sprintf(dbg_info, "= Debug Info =\nname\t = %s\nposition = %.3f : %.3f \n(convas) is_hovered = %d\nCursor inside: %d\nCursor - move zone: %d\nresize_mode = %d\nresize_or = %d\nm_EditMode = %d", element.name.c_str(), element.position.x, element.position.y, is_hovered, inside, move_zone, m_saveResizeMode, m_saveResizeOr, m_EditMode);
...
void UIMinimapEditorForm::RenderBoundCanvas()
{
	...
	if (BoundEditMode && ImGui::IsMouseDragging(ImGuiMouseButton_Left))
	{
		...
		if (m_saveResizeMode == 1)
		{
			if (m_saveResizeOr == 0 && topLeftBoundRect.x + mouse_delta.x < hl3.x && topLeftBoundRect.x + mouse_delta.x > element_screen_pos.x)
			{
				topLeftBoundRect.x += mouse_delta.x;
			}
			else if (m_saveResizeOr == 1 && bottomRightBoundRect.x + mouse_delta.x > hl3.x && bottomRightBoundRect.x + mouse_delta.x < element_screen_pos.x + element_screen_size.x)
			{
		...
		{
			if (m_saveResizeOr == 0 && topLeftBoundRect.y + mouse_delta.y < hl3.y && topLeftBoundRect.y + mouse_delta.y > element_screen_pos.y)
			{
				topLeftBoundRect.y += mouse_delta.y;
			}
			else if (m_saveResizeOr == 1 && bottomRightBoundRect.y + mouse_delta.y > hl3.y && bottomRightBoundRect.y + mouse_delta.y < element_screen_pos.y + element_screen_size.y)
			{
	...
	{
		string_path dbg_info;
		sprintf(dbg_info, "= [Debug Info] Bound Rect EDITOR =\nname\t = %s\nposition = %.3f : %.3f \nsize: %.0fx%.0f\nresize_mode: %d\nresize_or: %d\nBoundEditMode = %d", selectedElement->name.c_str(), m_BoundBackgroundPosition.x, m_BoundBackgroundPosition.y, selectedElement->FileSize.x, selectedElement->FileSize.y, m_saveResizeMode, m_saveResizeOr, BoundEditMode);
...
void UIMinimapEditorForm::ShowPreview()
{
	...
	sprintf(
		...
		sect_name.c_str(),
		BackgroundRenderSize.x,
		BackgroundRenderSize.y,
	...
	for (const auto& element : MapElements)
	{
		sprintf(buff, "[%s]\n        global_rect    = %.2f, %.2f, %.2f, %.2f\n", element.name.c_str(), element.position.x, element.position.y, element.position.x + element.RenderSize.x, element.position.y + element.RenderSize.y);
		data += buff;
	}
...
void UIMinimapEditorForm::CreateElementPopup()
{
	if (ImGui::BeginPopup("Add Location"))
	{
		...
		if (ImGui::Button("Add"))
		{
			...
			MapElements.push_back(el_new);

			CreatingData.name.clear();
			CreatingData.TexturePath.clear();

			isEdited = true;
			SelectElement(MapElements[MapElements.size() - 1]);
		}
...
void UIMinimapEditorForm::ShowMenu()
{
	...
	ImGui::DragFloat2("##bgs", (float*)&BackgroundRenderSize, 1.0f, 128, 1024 * 4);
	ImGui::SameLine();
	if (ImGui::Button("to file size"))
	{
		BackgroundRenderSize = BackgroundSize;
	}
...
void UIMinimapEditorForm::ReloadMapInfo(const xr_string& fn)
{
	...
	if (ltxFile->section_exist("global_map"))
	{
		...
		if (ltxFile->line_exist("global_map", "bound_rect"))
		{
			...
			BackgroundRenderSize.x = tx.z;
			BackgroundRenderSize.y = tx.w;
		}
	}
	...
	for (; it != it_e; ++it)
	{
		...
		if (!LoadTexture(el, texturePath.c_str()))
		{
			el.TexturePath = texturePath.c_str();
		}
...
void UIMinimapEditorForm::SaveFile(bool saveCurrent)
{
...

	{
		...
		ltxFile->w_string("global_map", "texture", BackgroundTexturePath.c_str());
		ltxFile->w_fvector4("global_map", "bound_rect", Fvector4(0.f, 0.f, BackgroundRenderSize.x, BackgroundRenderSize.y));
	}
...
void UIMinimapEditorForm::Draw()
{
	...
	if (BackgroundTexture == nullptr)
	{
		...
		BackgroundTexture->surface_set(Surf);
		Surf->Release();

		BackgroundTexturePath = "ui\\ui_nomap";
		BackgroundSize.x = 512;
		BackgroundSize.y = 512;
		BackgroundRenderSize = BackgroundSize;
	}
...
int UIMinimapEditorForm::LoadTexture(Element& el, const xr_string texture)
{
	...
	if (!LoadRawImage(fn.c_str(), ImageData, W, H, A))
	{
		return 2;
	}
	...
	textureDesc.Format = ERHI_FORMAT::B8G8R8A8_UNORM;
	...
	for (int i = 0; i < H; i++)
	{
		...
		memcpy(dest, ImageData.data() + (W * i), sizeof(unsigned char) * W * 4);
	}
	surf->Unlock();
	surf->Release();
...
void UIMinimapEditorForm::LoadBGClick(const xr_string texture)
{
	...
	u32 ImageW, ImageH, ImageA;

	if (LoadRawImage(fn.c_str(), ImageData, ImageW, ImageH, ImageA))
	{
		isEdited = true;

		xr_string GameTextureDir = FS.get_path("$game_textures$")->m_Path;

		BackgroundSize.x = ImageW;
		BackgroundSize.y = ImageH;
		BackgroundRenderSize = BackgroundSize;

		BackgroundTexturePath = (fn.find(GameTextureDir) == 0) ? fn.substr(GameTextureDir.size()) : fn;

		if (auto p = xr_path(BackgroundTexturePath); p.has_extension())
		{
			p.replace_extension("");
			BackgroundTexturePath = p;
		}

		TextureRemove = BackgroundTexture;

		RHITextureDesc textureDesc = {};
		textureDesc.Width = ImageW;
		textureDesc.Height = ImageH;
		textureDesc.MipLevels = 1;
		textureDesc.ArraySize = 1;
		textureDesc.Format = ERHI_FORMAT::B8G8R8A8_UNORM;
		textureDesc.Usage = ERHI_USAGE::USAGE_DYNAMIC;
		...
		{
			unsigned char* dest = static_cast<unsigned char*>(pBits) + (Pitch * i);
			memcpy(dest, ImageData.data() + (ImageW * i), sizeof(unsigned char) * ImageW * 4);
		}
		Surf->Unlock();
	}
}
