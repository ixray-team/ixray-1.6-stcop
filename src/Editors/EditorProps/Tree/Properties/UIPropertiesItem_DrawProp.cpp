...
inline bool DrawNumeric<float>(PropItem* item, bool& change, bool read_only)
{
	...
	change = ImGui::InputFloat("##value", &temp, 0.01, 0.1, "%.2f", read_only ? ImGuiInputTextFlags_ReadOnly : 0);
	if (change)
	{
...
void UIPropertiesItem::DrawProp()
{
	...
	case PROP_VECTOR:
	{
		...
		if (ImGui::InputFloat3("##value", vector,  "%.3f"))
		{
			for (int i = 0; i < 3; i++)
			{
	...
	break;
		...
		case PROP_CTEXT:
		{
			...
			if (false /*ImGui::OpenPopupOnItemClick2("EditText", 0) */)
			{
		...
		case PROP_GAMETYPE:
		{
			...
			if (false /*ImGui::OpenPopupOnItemClick2("EditGameType", 0) */)
			{
