...
bool IXBeginMainMenuBar()
{
	...
	ImGui::ImageButton("##IXBeginMainMenuBar00", UI->m_HeaderLogo->pSurface->GetRawTexture(), LogoSize);
	ImGui::PopStyleVar();
...
void IXEndMainMenuBar()
{
	...
	if (ImGui::ImageButton("##IXEndMainMenuBar01",UI->m_WinMin->pSurface->GetRawTexture(), ImageSize))
		SendMessageW(EDevice->GetHWND(), WM_SYSCOMMAND, SC_MINIMIZE, 0);

	if (ImGui::ImageButton("##IXEndMainMenuBar02", (EDevice->isZoomed ? UI->m_WinRes->pSurface->GetRawTexture() : UI->m_WinMax->pSurface->GetRawTexture()), ImageSize))
		MaxBut = true;

	if (ImGui::ImageButton("##IXEndMainMenuBar03", UI->m_WinClose->pSurface->GetRawTexture(), ImageSize))
