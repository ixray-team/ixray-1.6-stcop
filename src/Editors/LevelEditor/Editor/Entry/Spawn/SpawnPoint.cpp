...
bool CSpawnPoint::SSpawnData::ExportGame(SExportStreams* F, CSpawnPoint* owner)
{
	...
	if (cform)
	{
		CEditShape* shape = smart_cast<CEditShape*>(owner->m_AttachedObject);
		R_ASSERT(shape);
		shape->ApplyScale();
...
{
	ButtonValue* B = smart_cast<ButtonValue*>(value);
	R_ASSERT(B);
	switch (B->btn_num)
	{
...
CSpawnPoint::CSpawnPoint(LPVOID data, const char* name)
...
void CSpawnPoint::Render(int priority, bool strictB2F)
{
	...
	if (1 == priority)
	{
		if (strictB2F)
		{
			...
			{
				// render icon
				ESceneSpawnTool* st = smart_cast<ESceneSpawnTool*>(FParentTools);
				VERIFY(st);
		...
		else
		{
			...
			if (st->m_Flags.is(ESceneSpawnTool::flShowSpawnType))
			{
				...
				else
				{
					...
					{
						case ptRPoint:
							s_name = "RPoint T:" + xr_string::ToString(m_RP_TeamID);
							break;
						case ptEnvMod:
						{
							string256 Data = {};
