...
void CCustomObject::OnMotionControlPlayClick(ButtonValue* value, bool& bModif, bool& bSafe)
{
...
void CCustomObject::AnimationFillProp(LPCSTR pref, PropItemVec& items)
{
	...
	if (Motionable()){
		...
		B				= PHelper().CreateButton		(items,PrepareKey(pref,"Motion\\State"),	"Play,Pause", 0); 
		B->OnBtnClickEvent.bind(this,&CCustomObject::OnMotionControlPlayClick);
