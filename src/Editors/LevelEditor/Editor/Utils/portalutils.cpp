...
//              if(verts[*it].similar(V) )	
	...
	void export_portals()
	{
		...
		for (sCollector::sPortal& Portal : portals)
		{
			if (Portal.e.size() > 1)
			{
				Msg("portal %d of %d", curr++, PortalsCount);
				// build vert-list
				xr_vector<int>	vlist;
				xr_deque<int>& elist = Portal.e;
				...
				_O->SetSectors(Portal.s[0], Portal.s[1]);
				_O->Update();
				if (_O->Valid()) 
				{
				...
				{
					xr_delete(_O);
					ELog.Msg(mtError, "Can't simplify Portal :(\nPlease check geometry.\n'%s'<->'%s'", Portal.s[0]->GetName(), Portal.s[1]->GetName());
				}
			}
			else if (Portal.e.empty())
			{
				ELog.Msg(mtError, "Can't create Portal from 0 edge :(\nPlease check geometry.\n'%s'<->'%s'\n", Portal.s[0]->GetName(), Portal.s[1]->GetName());
			}
			else
			{
				Fvector& v0 = verts[edges[Portal.e[0]].v[0]];
				Fvector& v1 = verts[edges[Portal.e[0]].v[1]];
