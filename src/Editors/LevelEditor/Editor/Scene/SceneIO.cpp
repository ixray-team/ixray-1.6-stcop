...
bool EScene::LoadLTX(const char* map_name, bool bUndo)
{
	...
	UI->GeneralTabs[0] = {LevelPath, []() -> bool
...
bool EScene::Load(const char* map_name, bool bUndo)
{
	...
	if (FS.TryLoad(full_name.c_str()))
	{
		...
		if (!F)
		{
			ELog.Msg(mtError, "EScene: file was discovered but could not be opened: '%s'", full_name.c_str());
			return false;
		}
		// Version
		if (!F->r_chunk(CHUNK_VERSION, &version))
		{
			ELog.Msg(mtError, "EScene: level has no valid version chunk: '%s'", full_name.c_str());
			FS.r_close(F);
			return false;
		}
		if (version != CURRENT_FILE_VERSION)
