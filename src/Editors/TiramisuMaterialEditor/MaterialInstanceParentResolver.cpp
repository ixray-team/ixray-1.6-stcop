...
void AddError(FMaterialInstanceParentResolution& Result,
...
bool FMaterialInstanceParentResolution::Succeeded() const noexcept
{
    return Master.Id.IsValid() && Parent.MasterHandle.IsValid() &&
        ...
        std::ranges::none_of(Diagnostics,
    ...
    const FMaterialInstanceAsset& CurrentInstance)
{
    ...
    {
        AddError(Result, "editor.instance_parent_missing",
    ...
    {
        AddError(Result, "editor.material_root_missing",
    ...
    std::filesystem::recursive_directory_iterator Iterator(MaterialRoot,
    ...
    if (Error)
    {
        AddError(Result, "editor.material_scan_failed",
            "Failed to enumerate material assets under '" +
                ToXrString(MaterialRoot.string()) + "': " + ToXrString(Error.message()));
        return Result;
    }
    ...
    for (const std::filesystem::path& Path : MasterPaths)
    {
        ...
        if (Json.empty())
        {
            AddError(Result, "editor.material_read_failed",
    ...
    for (const std::filesystem::path& Path : InstancePaths)
    {
        ...
        if (Json.empty())
        {
            AddError(Result, "editor.instance_read_failed",
    ...
    if (!Master)
    {
        AddError(Result, "editor.instance_master_missing",
            "Resolved material instance does not have a live master asset.");
        return Result;
    }
