    ...
    const std::filesystem::path& Path)
{
    ...
    if (!Input)
    {
        AddDiagnostic(Result, "editor.material_open_failed",
            "Cannot open material asset '" + ToXrString(Path.string()) + "'.");
        return Result;
    }
    ...
    const std::filesystem::path& Path)
{
    ...
    {
        AddDiagnostic(Result, "editor.material_save_failed",
            "Cannot save material asset '" + ToXrString(Path.string()) + "': " + WriteResult.Error);
        return Result;
    }
    ...
    const std::filesystem::path& RecoveryPath) const
{
    ...
    if (!WriteResult.Success)
        AddDiagnostic(Result, "editor.material_autosave_failed",
            "Cannot write material recovery file '" + ToXrString(RecoveryPath.string()) +
                "': " + WriteResult.Error);
    return Result;
}
    ...
    const std::filesystem::path& OriginalPath)
{
    ...
    if (!Input)
    {
        AddDiagnostic(Result, "editor.material_recovery_open_failed",
            "Cannot open material recovery file '" + ToXrString(RecoveryPath.string()) + "'.");
        return Result;
    }
    const std::string Text{std::istreambuf_iterator<char>(Input),
        std::istreambuf_iterator<char>()};
    ...
    FMaterialParameterDefinition Definition)
{
    ...
    {
        AddDiagnostic(Result, "editor.duplicate_parameter_id",
            "A material parameter with this GUID already exists.");
        return Result;
    }
...
{
    FMaterialEditorOperationResult Result;
    const auto Existing = std::ranges::find_if(MaterialAsset.Parameters,
        [&ParameterId](const FMaterialParameterDefinition& Parameter)
        { return Parameter.Id == ParameterId; });
    if (Existing == MaterialAsset.Parameters.end())
    {
        AddDiagnostic(Result, "editor.missing_parameter",
            "The material parameter no longer exists.");
        return Result;
    }
    if (Definition.Id != ParameterId)
    {
        AddDiagnostic(Result, "editor.parameter_guid_is_stable",
            "A material parameter GUID cannot be changed after creation.");
        return Result;
    }
    if (!ValidateParameterDefinition(Definition, Result))
        return Result;
    ...
    {
        AddDiagnostic(Result, "editor.parameter_type_in_use",
            "Disconnect parameter nodes before changing the parameter type.");
        return Result;
    }
    ...
    const FMaterialParameterId& ParameterId)
{
    ...
    {
        AddDiagnostic(Result, "editor.parameter_in_use",
            "Remove or reassign graph nodes that reference this parameter first.");
        return Result;
    }
...
FMaterialEditorOperationResult TiramisuMaterialEditorDocument::AddNode(const xr_string_view Type,
    FMaterialNodeId NodeId, const FFloat2 Position, const EMaterialValueType ValueType)
{
    FMaterialEditorOperationResult Result;
    ...
    {
        AddDiagnostic(Result, "editor.duplicate_node_id",
            "A node with this GUID already exists.", NodeId);
        return Result;
    }
    if (Type == "material_output" && std::ranges::any_of(Graph.Nodes,
            [](const FMaterialGraphNode& Node) { return Node.Type == "material_output"; }))
    {
        AddDiagnostic(Result, "editor.duplicate_material_output",
            "A material graph can contain only one material output node.", NodeId);
        return Result;
    }
    ...
    if (!Node)
    {
        AddDiagnostic(Result, "editor.unknown_node_type",
            "Unknown or invalid material node type '" + xr_string(Type) + "'.", NodeId);
        return Result;
    }
    ...
    const FMaterialNodeId& NodeId)
{
    ...
    {
        AddDiagnostic(Result, "editor.cannot_remove_material_output",
            "The material output node cannot be removed.", NodeId);
        return Result;
    }
...
FMaterialEditorOperationResult TiramisuMaterialEditorDocument::Connect(xr_string LinkId,
    const FMaterialPinId& FromPin, const FMaterialPinId& ToPin)
{
    ...
    if (!RequireGraph(Result))
        return Result;
    FMaterialGraph& Graph = MaterialAsset.Implementation.Graph;
    if (LinkId.empty() || std::ranges::any_of(Graph.Links,
            [&LinkId](const FMaterialGraphLink& Link) { return Link.Id == LinkId; }))
    {
        AddDiagnostic(Result, "editor.invalid_link_id",
            "Link GUID is missing or duplicated.", {}, ToPin);
        return Result;
    }
    ...
    const xr_string_view LinkId)
{
    ...
    if (!RequireGraph(Result))
        return Result;
    FMaterialGraph& Graph = MaterialAsset.Implementation.Graph;
    const auto Link = std::ranges::find_if(Graph.Links,
        [LinkId](const FMaterialGraphLink& Candidate)
        {
...
FMaterialEditorOperationResult TiramisuMaterialEditorDocument::SetNodeProperty(
    const FMaterialNodeId& NodeId, const xr_string_view PropertyName,
    FMaterialValue Value)
{
        ...
        PropertyDefinition->Kind == EMaterialNodePropertyKind::ParameterId)
    {
        ...
        if (!ParameterId.empty())
        {
            ...
            if (!Parameter)
            {
                AddDiagnostic(Result, "editor.unknown_node_parameter",
                    "The selected material parameter does not exist.", NodeId);
                return Result;
            }
            ...
            if (!Compatible)
            {
                AddDiagnostic(Result, "editor.node_parameter_type_mismatch",
                    "The selected material parameter is incompatible with this node.", NodeId);
                return Result;
            }
        }
    }
...
FMaterialEditorOperationResult TiramisuMaterialEditorDocument::CopyNodes(
    const xr_span<const FMaterialNodeId> NodeIds, xr_string& ClipboardJson) const
{
    FMaterialEditorOperationResult Result;
    ClipboardJson.clear();
    if (!RequireGraph(Result))
    ...
    {
        AddDiagnostic(Result, "editor.clipboard_empty_selection",
            "Select at least one material expression node to copy.");
        return Result;
    }
...
FMaterialEditorOperationResult TiramisuMaterialEditorDocument::PasteNodes(
    const xr_string_view ClipboardJson, const FFloat2 PositionOffset,
    xr_vector<FMaterialNodeId>& PastedNodeIds)
{
    ...
    {
        AddDiagnostic(Result, "editor.clipboard_graph_version",
            "Clipboard material graph version is not supported.");
        return Result;
    }
    if (Parsed.Graph.Nodes.empty())
    {
        AddDiagnostic(Result, "editor.clipboard_empty_graph",
            "The material clipboard contains no nodes.");
        return Result;
    }
    ...
    for (const FMaterialGraphNode& SourceNode : Parsed.Graph.Nodes)
    {
        ...
        {
            AddDiagnostic(Result, "editor.clipboard_material_output",
                "Material Output cannot be pasted.", SourceNode.Id);
            continue;
        }
        if (SourceNode.Id.Value.empty() ||
            !SourceNodeIds.emplace(SourceNode.Id.Value).second)
        {
            AddDiagnostic(Result, "editor.clipboard_duplicate_node_id",
                "Clipboard nodes must have unique source GUIDs.", SourceNode.Id);
            continue;
        }
        ...
        if (!Definition)
        {
            AddDiagnostic(Result, "editor.clipboard_unknown_node_type",
                "Clipboard contains unknown node type '" + SourceNode.Type + "'.",
                SourceNode.Id);
            continue;
        }
        if (SourceNode.TypeVersion > Definition->TypeVersion)
        {
            AddDiagnostic(Result, "editor.clipboard_newer_node_version",
                "Clipboard node type '" + SourceNode.Type + "' is newer than this editor.",
                SourceNode.Id);
            continue;
        }
        ...
        {
            const auto Output = std::ranges::find_if(SourceNode.Pins,
                [](const FMaterialGraphPin& Pin)
                { return Pin.Direction == EMaterialPinDirection::Output; });
        ...
        xr_optional<FMaterialGraphNode> NewNode = CreateMaterialGraphNode(
            SourceNode.Type, NewNodeId,
            {SourceNode.EditorPosition[0] + PositionOffset[0],
                SourceNode.EditorPosition[1] + PositionOffset[1]},
            ValueType);
        if (!NewNode)
        {
            AddDiagnostic(Result, "editor.clipboard_node_migration_failed",
                "Clipboard node could not be recreated with the current schema.",
                SourceNode.Id);
            continue;
        }
        ...
        for (const FMaterialGraphPin& SourcePin : SourceNode.Pins)
        {
            ...
            {
                AddDiagnostic(Result, "editor.clipboard_duplicate_pin_id",
                    "Clipboard pins must have unique source GUIDs.", SourceNode.Id,
                    SourcePin.Id);
                continue;
            }
            const auto NewPin = std::ranges::find_if(NewNode->Pins,
                [&SourcePin](const FMaterialGraphPin& Candidate)
                {
            ...
            {
                AddDiagnostic(Result, "editor.clipboard_pin_migration_failed",
                    "Clipboard pin is incompatible with the current node schema.",
                    SourceNode.Id, SourcePin.Id);
                continue;
            }
    ...
    for (const FMaterialGraphLink& SourceLink : Parsed.Graph.Links)
    {
        ...
        {
            AddDiagnostic(Result, "editor.clipboard_dangling_link",
                "Clipboard link references a pin outside the copied selection.");
            continue;
        }
        FMaterialGraphLinkValidationResult Validation =
...
bool TiramisuMaterialEditorDocument::SetNodePosition(const FMaterialNodeId& NodeId,
    const FFloat2 Position, const bool RecordUndo)
{
    ...
    const FMaterialGraphCompileOptions& Options) const
{
    if (!IsGraphImplementation())
    {
        ...
        Result.Diagnostics.push_back({EMaterialDiagnosticSeverity::Error,
            ...
            "The graph editor is available only for graph material implementations.",
            {}, {}});
        return Result;
    }
...
bool TiramisuMaterialEditorDocument::RequireGraph(FMaterialEditorOperationResult& Result) const
{
    if (IsGraphImplementation())
        return true;
    AddDiagnostic(Result, "editor.graph_implementation_required",
        "The graph editor is available only for graph material implementations.");
    return false;
}
    ...
    const FMaterialParameterId& ParameterId) const
{
    if (!IsGraphImplementation())
        return false;
    return std::ranges::any_of(MaterialAsset.Implementation.Graph.Nodes,
        [&ParameterId](const FMaterialGraphNode& Node)
        {
    ...
    FMaterialEditorOperationResult& Result) const
{
    ...
    {
        AddDiagnostic(Result, "editor.invalid_parameter_id",
            "Material parameter GUID is missing or invalid.");
    }
    if (Definition.Name.empty())
    {
        AddDiagnostic(Result, "editor.missing_parameter_name",
            "Material parameter name cannot be empty.");
    }
    if (!ValueMatchesParameterType(Definition.DefaultValue, Definition.Type))
    {
        AddDiagnostic(Result, "editor.parameter_default_type_mismatch",
            "Material parameter default value does not match its type.");
    }
    if (Definition.Minimum && Definition.Maximum &&
        *Definition.Minimum > *Definition.Maximum)
    {
        AddDiagnostic(Result, "editor.parameter_range_invalid",
            "Material parameter minimum cannot be greater than maximum.");
    }
    return Result.Succeeded();
}

void TiramisuMaterialEditorDocument::AddDiagnostic(FMaterialEditorOperationResult& Result,
    const xr_string_view Code, xr_string Message, const FMaterialNodeId Node,
    const FMaterialPinId Pin)
{
