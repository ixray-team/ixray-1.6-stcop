...
)
{
	...
	if (!Input)
	{
		AddDiagnostic(Result, "editor.instance_open_failed", "Cannot open material instance '" + ToXrString(Path.string()) + "'.");
		return Result;
	}
...
)
{
	...
	{
		AddDiagnostic(Result, "editor.instance_save_failed", "Cannot save material instance '" + ToXrString(Path.string()) + "': " + WriteResult.Error);
		return Result;
	}
...
) const
{
	...
	if (!WriteResult.Success)
	{
		AddDiagnostic(Result, "editor.instance_autosave_failed", "Cannot write material instance recovery file '" + ToXrString(RecoveryPath.string()) + "': " + WriteResult.Error);
	}
	return Result;
}
...
)
{
	...
	if (!Input)
	{
		AddDiagnostic(Result, "editor.instance_recovery_open_failed", "Cannot open material instance recovery file '" + ToXrString(RecoveryPath.string()) + "'.");
		return Result;
	}
	const std::string Text{std::istreambuf_iterator<char>(Input), std::istreambuf_iterator<char>()};
	const xr_string JsonText = ToXrString(Text);
...
)
{
	...
	{
		AddDiagnostic(Result, "editor.instance_parent_resolution_mismatch", "Resolved parent does not refer to the supplied master material.");
		return Result;
	}
	ParentMaterial = std::move(Material);
	ParentResolution = std::move(Resolution);
...
const FMaterialValue* TiramisuMaterialInstanceEditorDocument::GetInheritedValue(
	const FMaterialParameterId& ParameterId, const bool Static
) const noexcept
{
	if (!ParentResolution)
	{
	...
	const FMaterialParameterMap& Parameters = Static
												  ? ParentResolution->StaticParameters
												  : ParentResolution->Parameters;
...
FMaterialEditorOperationResult TiramisuMaterialInstanceEditorDocument::SetParent(xr_string Parent)
{
	...
	if (Parent.empty())
	{
		AddDiagnostic(Result, "editor.instance_parent_missing", "A material instance parent cannot be empty.");
		return Result;
	}
	if ((!InstanceAsset.Overrides.empty() || !InstanceAsset.StaticOverrides.empty()) &&
		InstanceAsset.Parent != Parent)
	{
		AddDiagnostic(Result, "editor.instance_parent_has_overrides", "Remove overrides before changing the material instance parent.");
		return Result;
	}
...
)
{
	...
	if (!ParentMaterial)
	{
		AddDiagnostic(Result, "editor.instance_parent_schema_missing", "Load the parent master material before editing overrides.");
		return Result;
	}
	...
	if (!Definition)
	{
		AddDiagnostic(Result, "editor.instance_unknown_parameter", "The parent master material does not declare this parameter.");
		return Result;
	}
	if (Definition->IsStatic() != Static)
	{
		AddDiagnostic(Result, "editor.instance_override_storage_mismatch", Static ? "A runtime parameter cannot be stored as a static override." : "A static parameter cannot be stored as a runtime override.");
		return Result;
	}
	if (!ValueMatchesParameterType(Value, Definition->Type))
	{
		AddDiagnostic(Result, "editor.instance_override_type_mismatch", "Override value does not match the parent parameter type.");
		return Result;
	}

	FMaterialParameterMap& Overrides = Static
										   ? InstanceAsset.StaticOverrides
										   : InstanceAsset.Overrides;
...
)
{
	...
	{
		AddDiagnostic(Result, "editor.instance_override_missing", "The material instance has no override for this parameter.");
		return Result;
	}
...
FMaterialEditorOperationResult TiramisuMaterialInstanceEditorDocument::ValidateOverrides() const
{
	...
	{
		AddDiagnostic(Result, "editor.instance_parent_schema_missing", "Load the parent master material to validate overrides before saving.");
		return Result;
	}
	if (!ParentMaterial)
	{
	...
	const auto ValidateMap = [this, &Result](const FMaterialParameterMap& Overrides, const bool Static)
	{
		for (const auto& [Id, Value] : Overrides)
		{
			...
			if (!Definition)
			{
				AddDiagnostic(Result, "editor.instance_unknown_parameter", "Override references a parameter missing from the parent master.");
			}
			else if (Definition->IsStatic() != Static)
			{
				AddDiagnostic(Result, "editor.instance_override_storage_mismatch", "Override is stored in the wrong runtime/static map.");
			}
...
void TiramisuMaterialInstanceEditorDocument::AddDiagnostic(
	FMaterialEditorOperationResult& Result, const xr_string_view Code, xr_string Message
)
{
	Result.Diagnostics.push_back({EMaterialDiagnosticSeverity::Error, xr_string(Code), std::move(Message), {}, {}});
}
} // namespace Tiramisu::Editor
