...
void AddError(xr_vector<FMaterialDiagnostic>& Diagnostics,
    xr_string Code, xr_string Message)
{
    Diagnostics.push_back({EMaterialDiagnosticSeverity::Error,
        std::move(Code), std::move(Message), {}, {}});
}

void Append(xr_vector<FMaterialDiagnostic>& Destination,
    const xr_vector<FMaterialDiagnostic>& Source)
{
...
[[nodiscard]] bool EqualInstance(const FMaterialInstanceAsset& Left,
    ...
    xr_vector<FMaterialDiagnostic>& Diagnostics)
{
    ...
    {
        AddError(Diagnostics, "object_migration.invalid_database",
            "Legacy object material migration database contains invalid JSON.");
        return false;
    }
    ...
    {
        AddError(Diagnostics, "object_migration.unsupported_database",
            "Legacy object material migration database has an unsupported schema.");
        return false;
    }
    ...
    for (const Json& Item : *Items)
    {
        ...
        const auto ReadStringField = [&Item](const char* Name,
            xr_string& Destination)
        {
        ...
        {
            AddError(Diagnostics, "object_migration.invalid_database_entry",
                "Legacy object material migration database contains an invalid or duplicate entry.");
            return false;
        }
...
bool TiramisuLegacyObjectMaterialMigrationService::SaveDatabase(
    xr_vector<FMaterialDiagnostic>& Diagnostics) const
{
    Json RootJson = {
    ...
    std::ranges::sort(Sorted, {},
        [](const FDatabaseEntry* Entry) { return Entry->Key; });
    for (const FDatabaseEntry* Entry : Sorted)
    {
        RootJson["entries"].push_back({
            {"key", Entry->Key},
            {"instance", Entry->Instance},
            {"asset", Entry->AssetPath},
            {"parent", Entry->Parent},
            {"shader", Entry->ShaderName},
            {"compiler_shader", Entry->CompilerShaderName},
    ...
    if (!Write.Success)
    {
        AddError(Diagnostics, "object_migration.database_write_failed",
            Write.Error);
        return false;
    }
    return true;
    ...
    const bool DeferDatabaseSave)
{
    ...
    {
        AddError(Result.Diagnostics, "object_migration.not_initialized",
            "Legacy object material migration service is not initialized.");
        return Result;
    }
    ...
    for (size_t Index = 0; Index < Surfaces.size(); ++Index)
    {
        ...
        auto Existing = std::ranges::find(CandidateEntries, Key,
            &FDatabaseEntry::Key);
        if (Existing != CandidateEntries.end())
        {
            if (!ObjectSource.empty() &&
                std::ranges::find(Existing->Sources,
                    xr_string(ObjectSource)) == Existing->Sources.end())
            {
                Existing->Sources.push_back(xr_string(ObjectSource));
                std::ranges::sort(Existing->Sources);
                DatabaseChanged = true;
            }
            Result.Bindings.push_back({Surface.SurfaceName, Key,
                Existing->Instance, Existing->AssetPath,
        ...
        if (std::filesystem::exists(AssetPath, Error) && !Error)
        {
            const FMaterialInstanceParseResult Parsed =
                ParseMaterialInstanceJson(ReadText(AssetPath),
                    RelativePath);
            if (!Parsed.Succeeded() ||
                !EqualInstance(Parsed.Value, Instance))
            {
                AddError(Result.Diagnostics,
                    "object_migration.instance_conflict",
                    "Generated MaterialInstance path already contains different data: '" +
                        ToXrString(AssetPath.string()) + "'.");
                continue;
            }
            NeedsWrite = false;
        }
        ...
        Result.Bindings.push_back(
            {Surface.SurfaceName, Key, Guid, RelativePath,
    ...
    if (DirectoryError)
    {
        AddError(Result.Diagnostics, "object_migration.directory_failed",
            "Cannot create generated material directory: " +
                ToXrString(DirectoryError.message()));
        return Result;
    }
    for (const FPendingInstance& Pending : PendingInstances)
    {
        const FAtomicTextFileWriteResult Write =
            WriteTextFileAtomically(Pending.Path,
                SerializeMaterialInstanceJson(Pending.Asset));
        if (!Write.Success)
        {
            AddError(Result.Diagnostics,
                "object_migration.instance_write_failed",
                Write.Error);
            return Result;
        }
    }
    ...
    xr_vector<FMaterialDiagnostic>& Diagnostics)
{
    if (!Initialized)
    {
        AddError(Diagnostics, "object_migration.not_initialized",
            "Legacy object material migration service is not initialized.");
        return false;
    }
