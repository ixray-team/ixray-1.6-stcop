...
public:
	...
	[[nodiscard]] FMaterialEditorOperationResult AddNode(xr_string_view Type, FMaterialNodeId NodeId, FFloat2 Position = {0.0f, 0.0f}, EMaterialValueType ValueType = EMaterialValueType::Invalid);
	[[nodiscard]] FMaterialEditorOperationResult RemoveNode(const FMaterialNodeId& NodeId);
	[[nodiscard]] FMaterialEditorOperationResult Connect(xr_string LinkId, const FMaterialPinId& FromPin, const FMaterialPinId& ToPin);
	...
	[[nodiscard]] FMaterialEditorOperationResult PasteNodes(
		xr_string_view ClipboardJson, FFloat2 PositionOffset, xr_vector<FMaterialNodeId>& PastedNodeIds
	);

	// Presentation может обновлять позиции nodes без заполнения undo history.
	[[nodiscard]] bool SetNodePosition(const FMaterialNodeId& NodeId, FFloat2 Position, bool RecordUndo);
...
private:
	...
	static void AddDiagnostic(FMaterialEditorOperationResult& Result, xr_string_view Code, xr_string Message, FMaterialNodeId Node = {}, FMaterialPinId Pin = {});
