...
namespace
{
bool HasDependency(const FEditorViewportMaterialResolution& Resolution,
    const xr_string_view Filename)
{
    return std::ranges::any_of(Resolution.AssetDependencies,
        [Filename](const std::filesystem::path& Path)
        {
...
void TestLegacyViewportResolution(TiramisuMaterialTestRunner& Runner)
{
    ...
    MATERIAL_CHECK(Runner,
        Opaque.Legacy.Resolution == ELegacyMaterialResolution::LegacyMap);
    MATERIAL_CHECK(Runner,
        Opaque.Master.Id.Value == "a0f4d9f3-749c-4bc5-bf64-733927e19b20");
    MATERIAL_CHECK(Runner,
        Opaque.FlattenedInstance.Parent == Opaque.Master.Id.Value);
    MATERIAL_CHECK(Runner,
        std::get<xr_string>(Opaque.FlattenedInstance.Overrides.at(
    ...
    MATERIAL_CHECK(Runner,
        HasDependency(Opaque, "legacy_default.material-instance.json"));
    ...
    MATERIAL_CHECK(Runner,
        std::get<bool>(Lmap.Resolved.StaticParameters.at(
            FMaterialParameterId{"e5d7660f-f7d5-4ccd-9be2-602174fa12aa"})));
    MATERIAL_CHECK(Runner,
        std::get<xr_string>(Lmap.Resolved.Parameters.at(
    ...
    MATERIAL_CHECK(Runner,
        Aref.Master.Id.Value == "5f4b3a8e-85fe-42bc-9080-f71eb497f04b");
    ...
    MATERIAL_CHECK(Runner,
        Sprite.Legacy.Resolution == ELegacyMaterialResolution::LegacyMap);
    MATERIAL_CHECK(Runner,
        Sprite.Master.Id.Value == "d52ad18c-b4ce-4d4d-a5e3-7269071f43e1");
    MATERIAL_CHECK(Runner,
        Sprite.Resolved.BlendMode == EMaterialBlendMode::Translucent);
    MATERIAL_CHECK(Runner,
        Sprite.Resolved.ShadingModel == EMaterialShadingModel::Unlit);
    MATERIAL_CHECK(Runner, Sprite.TwoSided);
    MATERIAL_CHECK(Runner,
        HasDependency(Sprite, "legacy_editor_sprite.material.json"));
    MATERIAL_CHECK(Runner,
        std::get<xr_string>(Sprite.Resolved.Parameters.at(
            FMaterialParameterId{xr_string(LegacyBaseTextureParameterId)})) ==
            "ui\\spawn_test");
    ...
    MATERIAL_CHECK(Runner,
        Additive.Master.Id.Value == "4ea32e17-b945-42ad-9c2d-8cb63b658b9a");
    MATERIAL_CHECK(Runner,
        Additive.Resolved.BlendMode == EMaterialBlendMode::Additive);
    MATERIAL_CHECK(Runner,
        Additive.Resolved.ShadingModel == EMaterialShadingModel::Unlit);
    MATERIAL_CHECK(Runner, Additive.TwoSided);
    MATERIAL_CHECK(Runner, HasDependency(Additive,
        "legacy_editor_particle_additive.material.json"));
    MATERIAL_CHECK(Runner,
        std::get<xr_string>(Additive.Resolved.Parameters.at(
            FMaterialParameterId{xr_string(LegacyBaseTextureParameterId)})) ==
            "particles\\fire");
    ...
    MATERIAL_CHECK(Runner,
        Translucent.Master.Id.Value == "ada19ea7-b527-46c0-a006-2e39f5170b45");
    MATERIAL_CHECK(Runner,
        Translucent.Resolved.BlendMode == EMaterialBlendMode::Translucent);
    MATERIAL_CHECK(Runner, Translucent.TwoSided);
    MATERIAL_CHECK(Runner, HasDependency(Translucent,
        "legacy_editor_particle_translucent.material.json"));
    ...
    MATERIAL_CHECK(Runner,
        GlowSprite.Master.Id.Value == Additive.Master.Id.Value);
    MATERIAL_CHECK(Runner,
        GlowSprite.Resolved.BlendMode == EMaterialBlendMode::Additive);
    ...
    MATERIAL_CHECK(Runner,
        Standard.Legacy.Resolution == ELegacyMaterialResolution::AutomaticStandard);
    MATERIAL_CHECK(Runner,
        HasDiagnostic(Standard.Diagnostics, "legacy.automatic_standard"));
    ...
    MATERIAL_CHECK(Runner,
        Error.Legacy.Resolution == ELegacyMaterialResolution::ErrorMaterial);
    MATERIAL_CHECK(Runner,
        Error.Master.Id.Value == "e67b251d-7905-4583-8450-0903c46ec652");
    ...
    MATERIAL_CHECK(Runner,
        Explicit.Master.Id.Value ==
            "67e3bc21-9df5-4fc2-ab60-1ad7d02ad6e3");
    MATERIAL_CHECK(Runner,
        HasDependency(Explicit, "example_red.material-instance.json"));
    MATERIAL_CHECK(Runner, !HasDependency(Explicit, "legacy-map.json"));
    MATERIAL_CHECK(Runner,
        std::get<FFloat4>(Explicit.Resolved.Parameters.at(
            FMaterialParameterId{
                "915ce004-8c2f-47ce-87c7-b4af787b835e"}))[0] == 1.0f);

    TiramisuEditorViewportMaterialResolver MissingResolver;
    MATERIAL_CHECK(Runner,
        !MissingResolver.Load(Root / "missing", &LoadDiagnostics));
...
void TestResolverDependencyReloadKeepsLastGood(TiramisuMaterialTestRunner& Runner)
{
	...
	std::error_code Error;
	std::filesystem::copy(SourceRoot, TemporaryRoot,
	...
	MATERIAL_CHECK(Runner,
		!ValidChanges.empty() && ValidChanges.front().Path.filename() ==
			"legacy-map.json");
	...
	MATERIAL_CHECK(Runner,
		!InvalidCandidate.Load(TemporaryRoot, &Diagnostics));
