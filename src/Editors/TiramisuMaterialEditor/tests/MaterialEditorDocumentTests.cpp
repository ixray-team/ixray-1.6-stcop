...
void TestNewGraphAndCompile(TiramisuMaterialTestRunner& Runner)
{
	...
	MATERIAL_CHECK(Runner, Document.AddNode("constant", {"roughness-constant"}, {20.0f, 40.0f}).Succeeded());
	MATERIAL_CHECK(Runner, Document.SetNodeProperty({"roughness-constant"}, "value", 0.65f).Succeeded());
...
{
	TiramisuMaterialEditorDocument Document;
	MATERIAL_CHECK(Runner, Document.AddNode("constant", {"scalar-constant"}).Succeeded());

	Document.OpenGraph(Document.GetGraph());
	MATERIAL_CHECK(Runner, !Document.CanUndo());
	MATERIAL_CHECK(Runner, !Document.IsDirty());
	MATERIAL_CHECK(Runner, Document.SetNodeProperty({"scalar-constant"}, "value", 0.0f).Succeeded());
	MATERIAL_CHECK(Runner, !Document.CanUndo());
	MATERIAL_CHECK(Runner, !Document.IsDirty());

	MATERIAL_CHECK(Runner, Document.SetNodeProperty({"scalar-constant"}, "value", 0.75f).Succeeded());
	MATERIAL_CHECK(Runner, Document.CanUndo());
	MATERIAL_CHECK(Runner, Document.IsDirty());
	MATERIAL_CHECK(Runner, std::get<float>(Document.GetGraph().Nodes.back().Properties.at("value")) == 0.75f);

	MATERIAL_CHECK(Runner, Document.Undo());
	MATERIAL_CHECK(Runner, std::get<float>(Document.GetGraph().Nodes.back().Properties.at("value")) == 0.0f);
	MATERIAL_CHECK(Runner, Document.Redo());
	MATERIAL_CHECK(Runner, std::get<float>(Document.GetGraph().Nodes.back().Properties.at("value")) == 0.75f);

	MATERIAL_CHECK(Runner, HasDiagnostic(Document.SetNodeProperty({"scalar-constant"}, "value", xr_string{"wrong"}).Diagnostics, "graph.node_property_type_mismatch"));
	MATERIAL_CHECK(Runner, HasDiagnostic(Document.SetNodeProperty({"scalar-constant"}, "unknown", 1.0f).Diagnostics, "graph.unknown_node_property"));
	MATERIAL_CHECK(Runner, HasDiagnostic(Document.SetNodeProperty({"missing"}, "value", 1.0f).Diagnostics, "editor.missing_node"));

	MATERIAL_CHECK(Runner, Document.AddNode("constant", {"vector-constant"}, {}, EMaterialValueType::Float3).Succeeded());
	MATERIAL_CHECK(Runner, HasDiagnostic(Document.SetNodeProperty({"vector-constant"}, "value", 1.0f).Diagnostics, "graph.node_property_type_mismatch"));
	MATERIAL_CHECK(Runner, Document.SetNodeProperty({"vector-constant"}, "value", FFloat3{1.0f, 2.0f, 3.0f}).Succeeded());
}

void TestRejectedOperations(TiramisuMaterialTestRunner& Runner)
{
	TiramisuMaterialEditorDocument Document;
	MATERIAL_CHECK(Runner, HasDiagnostic(Document.AddNode("unknown", {"unknown"}).Diagnostics, "editor.unknown_node_type"));
	MATERIAL_CHECK(Runner, HasDiagnostic(Document.AddNode("material_output", {"second-output"}).Diagnostics, "editor.duplicate_material_output"));
	MATERIAL_CHECK(Runner, HasDiagnostic(Document.RemoveNode(Document.GetGraph().Nodes.front().Id).Diagnostics, "editor.cannot_remove_material_output"));
	MATERIAL_CHECK(Runner, HasDiagnostic(Document.Disconnect("missing").Diagnostics, "editor.missing_link"));
}
...
void TestTypedLinksAndOpenGraph(TiramisuMaterialTestRunner& Runner)
{
	...
	MATERIAL_CHECK(Runner, HasDiagnostic(Document.Connect("invalid", Uv.Id, Texture.Id).Diagnostics, "graph.link_type_mismatch"));
	MATERIAL_CHECK(Runner, Document.GetGraph().Links.empty());
...
void TestMasterMaterialDocument(TiramisuMaterialTestRunner& Runner)
{
	...
	MATERIAL_CHECK(Runner, OriginalId[8] == '-' && OriginalId[13] == '-' && OriginalId[18] == '-' && OriginalId[23] == '-');
	MATERIAL_CHECK(Runner, Document.GetMaterial().Implementation.Type == EMaterialImplementationType::Graph);
	MATERIAL_CHECK(Runner, !Document.IsDirty());
	...
	MATERIAL_CHECK(Runner, Document.SetMaterialShadingModel(EMaterialShadingModel::Unlit));
	MATERIAL_CHECK(Runner, Document.SetMaterialTwoSided(true));
	MATERIAL_CHECK(Runner, Document.SetMaterialHlslTemplate("materials/TestTemplate.hlsl"));
	MATERIAL_CHECK(Runner, Document.IsDirty());
	MATERIAL_CHECK(Runner, std::ranges::contains(Document.GetMaterial().Dependencies, xr_string{"materials/TestTemplate.hlsl"}));
	...
	MATERIAL_CHECK(Runner, HasDiagnostic(Document.OpenMaterialJson("{ invalid json").Diagnostics, "asset.invalid_json"));
	MATERIAL_CHECK(Runner, Document.SerializeMaterial() == BeforeInvalidOpen);

	TiramisuMaterialEditorDocument Reopened;
	MATERIAL_CHECK(Runner, Reopened.OpenMaterialJson(Document.SerializeMaterial(), "memory.material.json").Succeeded());
...
void TestMaterialFileRoundTrip(TiramisuMaterialTestRunner& Runner)
{
	...
	MATERIAL_CHECK(Runner, Recovered.OpenRecoveryFile(RecoveryPath, Path).Succeeded());
...
void TestMigrationMarksDocumentDirty(TiramisuMaterialTestRunner& Runner)
{
	...
	MATERIAL_CHECK(Runner, HasDiagnostic(Result.Diagnostics, "asset.migrated_version_field"));
	MATERIAL_CHECK(Runner, HasDiagnostic(Result.Diagnostics, "asset.migrated_id_field"));
	MATERIAL_CHECK(Runner, Document.IsDirty());
	MATERIAL_CHECK(Runner, Document.SerializeMaterial().find("\"asset_version\"") != xr_string::npos);
}
...
void TestHlslMasterRejectsGraphEditing(TiramisuMaterialTestRunner& Runner)
{
	...
	MATERIAL_CHECK(Runner, HasDiagnostic(Document.AddNode("constant", {"not-allowed"}).Diagnostics, "editor.graph_implementation_required"));
	MATERIAL_CHECK(Runner, HasDiagnostic(Document.Compile().Diagnostics, "editor.graph_implementation_required"));
}
...
void TestParameterEditingAndReferences(TiramisuMaterialTestRunner& Runner)
{
	...
	MATERIAL_CHECK(Runner, HasDiagnostic(Document.AddParameter(Scalar).Diagnostics, "editor.duplicate_parameter_id"));

	FMaterialParameterDefinition Invalid = Scalar;
	Invalid.Id.Value = "invalid-default-guid";
	Invalid.DefaultValue = xr_string{"wrong"};
	MATERIAL_CHECK(Runner, HasDiagnostic(Document.AddParameter(Invalid).Diagnostics, "editor.parameter_default_type_mismatch"));

	FMaterialParameterDefinition Renamed = Scalar;
	Renamed.Name = "Microsurface";
	MATERIAL_CHECK(Runner, Document.UpdateParameter(Scalar.Id, Renamed).Succeeded());
	MATERIAL_CHECK(Runner, Document.GetMaterial().FindParameter(Scalar.Id)->Name == "Microsurface");

	MATERIAL_CHECK(Runner, Document.AddNode("parameter", {"roughness-parameter"}, {}, EMaterialValueType::Float1).Succeeded());
	MATERIAL_CHECK(Runner, Document.SetNodeProperty({"roughness-parameter"}, "parameter_id", Scalar.Id.Value).Succeeded());
	MATERIAL_CHECK(Runner, HasDiagnostic(Document.RemoveParameter(Scalar.Id).Diagnostics, "editor.parameter_in_use"));
	...
	MATERIAL_CHECK(Runner, HasDiagnostic(Document.UpdateParameter(Scalar.Id, ChangedType).Diagnostics, "editor.parameter_type_in_use"));

	MATERIAL_CHECK(Runner, Document.RemoveNode({"roughness-parameter"}).Succeeded());
	MATERIAL_CHECK(Runner, Document.UpdateParameter(Scalar.Id, ChangedType).Succeeded());
...
void TestTypedNodeParameterBindings(TiramisuMaterialTestRunner& Runner)
{
	...
	FMaterialParameterDefinition Scalar{{"binding-scalar-guid"}, "Scalar", EMaterialParameterType::Scalar, 0.5f};
	FMaterialParameterDefinition Texture{{"binding-texture-guid"}, "Texture", EMaterialParameterType::Texture2D, xr_string{"textures/default"}};
	FMaterialParameterDefinition Switch{{"binding-switch-guid"}, "Switch", EMaterialParameterType::StaticBool, false};
	MATERIAL_CHECK(Runner, Document.AddParameter(Scalar).Succeeded());
	...
	MATERIAL_CHECK(Runner, Document.AddNode("parameter", {"binding-parameter-node"}, {}, EMaterialValueType::Float1).Succeeded());
	MATERIAL_CHECK(Runner, Document.SetNodeProperty({"binding-parameter-node"}, "parameter_id", Scalar.Id.Value).Succeeded());
	MATERIAL_CHECK(Runner, HasDiagnostic(Document.SetNodeProperty({"binding-parameter-node"}, "parameter_id", Texture.Id.Value).Diagnostics, "editor.node_parameter_type_mismatch"));
	MATERIAL_CHECK(Runner, HasDiagnostic(Document.SetNodeProperty({"binding-parameter-node"}, "parameter_id", xr_string{"missing-parameter"}).Diagnostics, "editor.unknown_node_parameter"));

	MATERIAL_CHECK(Runner, Document.AddNode("static_switch", {"binding-switch-node"}).Succeeded());
	MATERIAL_CHECK(Runner, Document.SetNodeProperty({"binding-switch-node"}, "parameter_id", Switch.Id.Value).Succeeded());
	MATERIAL_CHECK(Runner, HasDiagnostic(Document.SetNodeProperty({"binding-switch-node"}, "parameter_id", Scalar.Id.Value).Diagnostics, "editor.node_parameter_type_mismatch"));

	MATERIAL_CHECK(Runner, Document.AddNode("texture_sample", {"binding-texture-node"}).Succeeded());
	MATERIAL_CHECK(Runner, Document.SetNodeProperty({"binding-texture-node"}, "texture_parameter_id", Texture.Id.Value).Succeeded());
	MATERIAL_CHECK(Runner, HasDiagnostic(Document.SetNodeProperty({"binding-texture-node"}, "texture_parameter_id", Scalar.Id.Value).Diagnostics, "editor.node_parameter_type_mismatch"));
	MATERIAL_CHECK(Runner, HasDiagnostic(Document.RemoveParameter(Texture.Id).Diagnostics, "editor.parameter_in_use"));
}

void TestGraphCopyPasteAndSingleUndo(TiramisuMaterialTestRunner& Runner)
{
	TiramisuMaterialEditorDocument Document;
	MATERIAL_CHECK(Runner, Document.AddNode("constant", {"clipboard-constant"}, {10.0f, 20.0f}).Succeeded());
	MATERIAL_CHECK(Runner, Document.SetNodeProperty({"clipboard-constant"}, "value", 0.25f).Succeeded());
	MATERIAL_CHECK(Runner, Document.AddNode("add", {"clipboard-add"}, {50.0f, 60.0f}, EMaterialValueType::Float1).Succeeded());
	...
	MATERIAL_CHECK(Runner, Document.Connect("clipboard-internal-link", Pin(*Constant, "Value", EMaterialPinDirection::Output).Id, Pin(*Add, "A", EMaterialPinDirection::Input).Id).Succeeded());
	...
	MATERIAL_CHECK(Runner, Document.PasteNodes(ClipboardJson, {100.0f, 200.0f}, Pasted).Succeeded());
	...
	MATERIAL_CHECK(Runner, std::get<float>(PastedConstant->Properties.at("value")) == 0.25f);
	...
	MATERIAL_CHECK(Runner, HasDiagnostic(Document.CopyNodes(OutputOnly, OutputClipboard).Diagnostics, "editor.clipboard_empty_selection"));
