...
FMaterialPreviewCompileRequest RequestFor(const EMaterialShaderBackend Backend)
{
	...
	Request.IncludeDirectories = {
		"gamedata/shaders/r5", "gamedata/shaders/r5/common", "gamedata/shaders/r5/materials", "gamedata/shaders/r5/materials/passes", "gamedata/shaders/r5/materials/vertex"
	};
	return Request;
}
...
float ReadFloat(const FMaterialPackedParameterBlock& Block, const FMaterialParameterLayoutField& Field)
{
	float Value = 0.0f;
...
int main()
{
	if (NormalizeMaterialPreviewTexturePath(
		...
		NormalizeMaterialPreviewTexturePath(
			"$game_textures$/act/act_arm_1.tga"
		) != "act/act_arm_1" ||
	...
	{
		ConcurrentRequests.push_back(std::async(std::launch::async, [D3DRequest]
	...
	for (size_t Index = 1; Index < ConcurrentRequests.size(); ++Index)
	{
		...
		{
			return Fail(Concurrent, "Concurrent identical preview compile was not coalesced");
		}
	}
	if (!D3D.Succeeded())
	{
	...
	{
		return Fail(RuntimeOverride, "Runtime parameter override created a duplicate shader permutation");
	}
	...
	{
		return Fail(GraphPreview, "Graph-generated HLSL did not compile through the live preview path");
	}
	...
	{
		return Fail(ForwardVulkan, "Vulkan editor Forward material permutation failed");
	}
	...
	if (!ChildDocument.SetParentResolution(
						  Master.Value, ResolvedParent.Value
		)
			 .Succeeded())
	{
		return 1;
	...
	if (!ParentChainPreview.Succeeded() ||
		...
		std::abs(ReadFloat(ParentChainPreview.ParameterBlock, *Roughness) - 0.12f) > 0.0001f)
	{
		return Fail(ParentChainPreview, "Flattened parent instance chain did not reach live preview");
	}
