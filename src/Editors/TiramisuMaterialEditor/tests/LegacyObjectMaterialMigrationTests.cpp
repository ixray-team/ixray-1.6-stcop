...
void TestDeduplicatedFirstLoadMigration(TiramisuMaterialTestRunner& Runner)
{
    ...
    std::error_code Error;
    std::filesystem::copy("gamedata/render_materials", TemporaryRoot,
    ...
    MATERIAL_CHECK(Runner,
        First.Bindings[0].MaterialInstance ==
            First.Bindings[1].MaterialInstance);
    MATERIAL_CHECK(Runner,
        First.Bindings[0].MaterialAsset ==
            "generated/legacy_objects/" +
                First.Bindings[0].MaterialInstance +
                ".material-instance.json");
    MATERIAL_CHECK(Runner,
        First.Bindings[0].MaterialAsset ==
            First.Bindings[1].MaterialAsset);
    MATERIAL_CHECK(Runner, First.DatabaseChanged);
    MATERIAL_CHECK(Runner,
        std::filesystem::is_regular_file(Service.GetDatabasePath()));
    ...
    MATERIAL_CHECK(Runner,
        Instance.Value.Parent ==
            "ee5ffbc0-bd24-4aa8-9e16-50651ca1c269");
    MATERIAL_CHECK(Runner,
        std::get<xr_string>(Instance.Value.Overrides.at(
    ...
    MATERIAL_CHECK(Runner,
        Second.Bindings[0].MaterialInstance ==
            First.Bindings[0].MaterialInstance);
    MATERIAL_CHECK(Runner,
        Second.Bindings[0].MaterialAsset ==
            First.Bindings[0].MaterialAsset);
    ...
    MATERIAL_CHECK(Runner,
        Variant.Bindings[0].MaterialInstance !=
            First.Bindings[0].MaterialInstance);
    ...
    const FLegacyObjectMaterialMigrationResult PhysicalOnly =
        Service.Migrate("objects/buildings/wall-metal-physics.object",
            {DifferentGameMaterial});
    MATERIAL_CHECK(Runner, PhysicalOnly.Succeeded());
    MATERIAL_CHECK(Runner,
        PhysicalOnly.Bindings[0].MaterialInstance ==
            First.Bindings[0].MaterialInstance);
    ...
    MATERIAL_CHECK(Runner,
        ReadText(Service.GetDatabasePath()) == BeforeBatch);
    Diagnostics.clear();
    ...
    MATERIAL_CHECK(Runner,
        AfterBatch.find("levels/zaton#wall_0001") !=
            xr_string::npos);
    Diagnostics.clear();
    MATERIAL_CHECK(Runner, Service.FlushDatabase(Diagnostics));
    MATERIAL_CHECK(Runner,
        ReadText(Service.GetDatabasePath()) == AfterBatch);
    ...
    Diagnostics.clear();
    MATERIAL_CHECK(Runner,
        ReloadedService.Initialize(TemporaryRoot, &Diagnostics));
    ...
    MATERIAL_CHECK(Runner,
        Reloaded.Bindings[0].MaterialInstance ==
            First.Bindings[0].MaterialInstance);
    MATERIAL_CHECK(Runner,
        Reloaded.Bindings[0].MaterialAsset ==
            First.Bindings[0].MaterialAsset);
}
} // namespace
