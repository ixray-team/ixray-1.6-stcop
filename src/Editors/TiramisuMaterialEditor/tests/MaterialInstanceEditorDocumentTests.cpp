...
void TestInstanceEditing(TiramisuMaterialTestRunner& Runner)
{
	...
	MATERIAL_CHECK(Runner, HasDiagnostic(Document.SetParent("").Diagnostics, "editor.instance_parent_missing"));
	MATERIAL_CHECK(Runner, Document.SetParent("instance-parent-master").Succeeded());
	Document.SetParentMaterial(MakeParent());

	MATERIAL_CHECK(Runner, Document.SetOverride({"roughness-guid"}, 0.25f, false).Succeeded());
	MATERIAL_CHECK(Runner, Document.SetOverride({"feature-guid"}, true, true).Succeeded());
	...
	MATERIAL_CHECK(Runner, HasDiagnostic(Document.SetOverride({"roughness-guid"}, true, false).Diagnostics, "editor.instance_override_type_mismatch"));
	MATERIAL_CHECK(Runner, HasDiagnostic(Document.SetOverride({"roughness-guid"}, 0.25f, true).Diagnostics, "editor.instance_override_storage_mismatch"));
	MATERIAL_CHECK(Runner, HasDiagnostic(Document.SetOverride({"missing-guid"}, 1.0f, false).Diagnostics, "editor.instance_unknown_parameter"));
	MATERIAL_CHECK(Runner, HasDiagnostic(Document.SetParent("different-parent").Diagnostics, "editor.instance_parent_has_overrides"));

	MATERIAL_CHECK(Runner, Document.RemoveOverride({"roughness-guid"}).Succeeded());
...
void TestInstanceJsonAndFileRoundTrip(TiramisuMaterialTestRunner& Runner)
{
	...
	MATERIAL_CHECK(Runner, Document.SetParent("instance-parent-master").Succeeded());
	Document.SetParentMaterial(MakeParent());
	MATERIAL_CHECK(Runner, Document.SetOverride({"tint-guid"}, FFloat4{0.2f, 0.3f, 0.4f, 1.0f}, false).Succeeded());

	const xr_string Json = Document.SerializeInstance();
	TiramisuMaterialInstanceEditorDocument Parsed;
	MATERIAL_CHECK(Runner, Parsed.OpenInstanceJson(Json, "memory.material-instance.json").Succeeded());
	MATERIAL_CHECK(Runner, Parsed.GetInstance().Id == Document.GetInstance().Id);
	MATERIAL_CHECK(Runner, !Parsed.IsDirty());
	MATERIAL_CHECK(Runner, HasDiagnostic(Parsed.OpenInstanceJson("{ invalid json").Diagnostics, "asset.invalid_json"));
...
void TestInstanceRequiresParentSchemaForOverrides(TiramisuMaterialTestRunner& Runner)
{
	TiramisuMaterialInstanceEditorDocument Document;
	MATERIAL_CHECK(Runner, Document.SetParent("instance-parent-master").Succeeded());
	MATERIAL_CHECK(Runner, HasDiagnostic(Document.SetOverride({"roughness-guid"}, 0.5f, false).Diagnostics, "editor.instance_parent_schema_missing"));
	...
	MATERIAL_CHECK(Runner, HasDiagnostic(Document.ValidateOverrides().Diagnostics, "editor.instance_parent_schema_missing"));
}
...
void TestParentChainResolutionAndFlattening(TiramisuMaterialTestRunner& Runner)
{
	...
	MATERIAL_CHECK(Runner, WriteTextFileAtomically(MasterPath, SerializeMaterialAssetJson(Master)).Success);
	...
	MATERIAL_CHECK(Runner, WriteTextFileAtomically(ParentPath, SerializeMaterialInstanceJson(Parent)).Success);

	FMaterialInstanceAsset Child;
	Child.Id.Value = "parent-chain-child";
	Child.Name = "Child";
	Child.Parent = "middle.material-instance.json";
	Child.Overrides.emplace(FMaterialParameterId{"tint-guid"}, FFloat4{0.1f, 0.2f, 0.3f, 1.0f});
	...
	if (Resolution.Succeeded())
	{
		...
		MATERIAL_CHECK(Runner, Resolution.Parent.ParentChain.front() == Parent.Id);
		MATERIAL_CHECK(Runner, std::get<float>(Resolution.Parent.Parameters.at(FMaterialParameterId{"roughness-guid"})) == 0.2f);
		MATERIAL_CHECK(Runner, std::get<bool>(Resolution.Parent.StaticParameters.at(FMaterialParameterId{"feature-guid"})));
		MATERIAL_CHECK(Runner, Resolution.Instance.ParentChain.size() == 2);
		MATERIAL_CHECK(Runner, Resolution.AssetDependencies.size() == 2);
		MATERIAL_CHECK(Runner, std::ranges::find(Resolution.AssetDependencies, MasterPath) != Resolution.AssetDependencies.end());
		MATERIAL_CHECK(Runner, std::ranges::find(Resolution.AssetDependencies, ParentPath) != Resolution.AssetDependencies.end());

		TiramisuMaterialInstanceEditorDocument Document;
		Document.OpenInstance(Child);
		MATERIAL_CHECK(Runner, Document.SetParentResolution(Resolution.Master, Resolution.Parent).Succeeded());
		...
		MATERIAL_CHECK(Runner, std::get<bool>(Document.GetEffectiveStaticParameters().at(FMaterialParameterId{"feature-guid"})));
		...
		if (Flattened.Succeeded())
		{
			...
			MATERIAL_CHECK(Runner, std::get<float>(Flattened.Value.Overrides.at(FMaterialParameterId{"roughness-guid"})) == 0.2f);
			MATERIAL_CHECK(Runner, std::get<FFloat4>(Flattened.Value.Overrides.at(FMaterialParameterId{"tint-guid"})) == FFloat4({0.1f, 0.2f, 0.3f, 1.0f}));
			MATERIAL_CHECK(Runner, std::get<bool>(Flattened.Value.StaticOverrides.at(FMaterialParameterId{"feature-guid"})));
		}
	}
