...
public:
	...
	[[nodiscard]] const FMaterialValue* GetInheritedValue(
		const FMaterialParameterId& ParameterId, bool Static
	) const noexcept;
...
private:
	void RecordMutation();
	static void AddDiagnostic(FMaterialEditorOperationResult& Result, xr_string_view Code, xr_string Message);
