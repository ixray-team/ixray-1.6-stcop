...
namespace
{
bool EqualsCaseInsensitive(const xr_string_view Left, const xr_string_view Right)
{
...
bool StartsWithCaseInsensitive(const xr_string_view Text, const xr_string_view Prefix)
{
	return Text.size() >= Prefix.size() &&
		   EqualsCaseInsensitive(Text.substr(0, Prefix.size()), Prefix);
}

bool EndsWithCaseInsensitive(const xr_string_view Text, const xr_string_view Suffix)
{
...
xr_string_view MaterialPreviewEnvironmentAsset(
	const xr_string_view Environment
) noexcept
{
