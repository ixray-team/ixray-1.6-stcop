...
{
public:
    explicit FTemporaryFileGuard(std::filesystem::path Path) : FilePath(std::move(Path)) {}
    ~FTemporaryFileGuard()
    {
        if (!Released)
        {
...
xr_string FormatError(const std::filesystem::path& Path, const xr_string_view Operation,
