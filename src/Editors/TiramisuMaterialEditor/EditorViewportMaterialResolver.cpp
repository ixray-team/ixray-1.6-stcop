...
xr_string SourceReference(const std::filesystem::path& Root,
...
xr_string MakeDynamicInstanceId(const u64 MaterialSlot)
{
    ...
    Stream << "editor-legacy-" << std::hex << std::setfill('0') <<
        std::setw(16) << MaterialSlot;
    return Stream.str();
}
} // namespace
    ...
    xr_vector<FMaterialDiagnostic>* OutDiagnostics)
{
    ...
    {
        AddError(Diagnostics, "editor.material_root_missing",
            "Material asset directory was not found: '" +
                ToXrString(MaterialRoot.string()) + "'.");
        if (OutDiagnostics)
    ...
    if (Error)
    {
        AddError(Diagnostics, "editor.material_scan_failed",
            "Failed to enumerate material assets under '" + ToXrString(Root.string()) +
                "': " + ToXrString(Error.message()));
    }
    ...
    for (const std::filesystem::path& Path : Masters)
    {
        ...
        if (Json.empty())
        {
            AddError(Diagnostics, "editor.material_read_failed",
                "Cannot read master material '" + ToXrString(Path.string()) + "'.");
            continue;
        }
    ...
    for (const std::filesystem::path& Path : Instances)
    {
        ...
        if (Json.empty())
        {
            AddError(Diagnostics, "editor.instance_read_failed",
                "Cannot read material instance '" + ToXrString(Path.string()) + "'.");
            continue;
        }
    ...
    if (LegacyJson.empty())
    {
        AddError(Diagnostics, "editor.legacy_map_read_failed",
            "Cannot read legacy material map '" + ToXrString(LegacyMapPath.string()) + "'.");
    }
    else
    {
    ...
    if (!Loaded)
    {
        AddError(Diagnostics, "editor.legacy_fallback_unavailable",
            "legacy-map.json standard/error materials are not resolvable.");
    }
    if (OutDiagnostics)
        *OutDiagnostics = std::move(Diagnostics);
    return Loaded;
...
FEditorViewportMaterialResolution TiramisuEditorViewportMaterialResolver::Resolve(
    const FEditorViewportLegacyMaterialSource& Source) const
{
    FEditorViewportMaterialResolution Result;
    Result.TwoSided = Source.TwoSided;
    if (!Loaded)
    {
        AddError(Result.Diagnostics, "editor.material_resolver_unavailable",
            "Editor viewport material resolver is not loaded.");
        return Result;
    }
    ...
    if (!Resolved.Succeeded() && Result.Legacy.Material != LegacyMap.ErrorMaterial)
    {
        ...
        AddError(Result.Diagnostics, "editor.legacy_mapping_unresolvable",
            "Mapped legacy material could not be resolved; using error material.");
    ...
    if (!Master)
    {
        AddError(Result.Diagnostics, "editor.material_master_missing",
            "Resolved editor viewport material has no live master.");
        return Result;
    }
    ...
    Result.FlattenedInstance.Name = Source.SurfaceName.empty()
        ? "Editor legacy material" : Source.SurfaceName;
