...
void AddError(FMaterialPreviewCompileResult& Result,
...
xr_string SourceHash(const FMaterialPreviewCompileRequest& Request,
    const xr_string_view Implementation, const xr_string_view PassSource)
{
...
FMaterialShaderCompileRequest MakeShaderRequest(
    ...
    xr_string Source, const char* SourceName,
    const char* TargetProfile, const bool VertexStage)
{
...
struct FMaterialPreviewShaderCache
{
    std::mutex Mutex;
    xr_hash_map<u64,
        std::shared_future<FMaterialPreviewShaderCacheValue>> Entries;
    ...
    const FMaterialPreviewCompileRequest& Request)
{
    ...
    if (Implementation.empty())
    {
        AddError(Result, "preview.empty_implementation",
            "Material preview requires generated or loaded EvaluateMaterial HLSL.");
        return Result;
    }
    ...
    if (Request.TemplateSource.empty() || Request.VertexFactorySource.empty() ||
        PassSource.empty())
    {
        AddError(Result, "preview.missing_engine_source",
            "Material template, vertex factory and validation pass sources are required.");
        return Result;
    }
    ...
    FMaterialSourceAssemblyResult VertexSource =
        AssembleMaterialShaderSourceForPass(Master, Request.TemplateSource,
            Implementation, Result.ResolvedMaterial.StaticParameters,
            Request.VertexFactorySource,
            "materials/vertex/MaterialLevelStaticVertexFactory.hlsl");
    const FMaterialPassDefinition* PassDefinition =
        FindMaterialPassDefinition(Request.Pass);
    if (!PassDefinition)
    {
        AddError(Result, "preview.pass_missing",
            "The requested material pass is absent from the pass manifest.");
        return Result;
    }
    ...
    FMaterialSourceAssemblyResult PixelSource =
        AssembleMaterialShaderSourceForPass(Master, Request.TemplateSource,
            Implementation, Result.ResolvedMaterial.StaticParameters,
            PassSource, PassDefinition->ShaderSource);
    ...
    FMaterialPipelineKey Pipeline = MakeCookedMaterialPipelineKey(
        Result.ResolvedMaterial, *PassDefinition,
        Request.Backend == EMaterialShaderBackend::D3D12 ? "d3d12" : "vulkan");
    ...
    {
        AddError(Result, "preview.invalid_pipeline_key",
            "The material preview pipeline key resolved to zero.");
        return Result;
    }
    ...
    if (CompileThisRequest)
    {
        ...
        {
            Value.Diagnostics.push_back({EMaterialDiagnosticSeverity::Error,
                "preview.dxc_unavailable",
                "DXC is unavailable for material preview compilation.", {}, {}});
        }
        else
        {
            FMaterialShaderCompileResult Vertex = Compiler.Compile(
                MakeShaderRequest(Request, std::move(VertexSource.Source),
                    "material-preview.vertex.hlsl", "vs_6_6", true));
            Append(Value.Diagnostics, Vertex.Diagnostics);
            FMaterialShaderCompileResult Pixel = Compiler.Compile(
                MakeShaderRequest(Request, std::move(PixelSource.Source),
                    "material-preview.pixel.hlsl", "ps_6_6", false));
