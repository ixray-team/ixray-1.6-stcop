...
int main()
{
	...
	for (const auto Backend : Backends)
	{
		for (const auto [EntryPoint, Profile] : {
			xr_pair{"VSMain", "vs_6_6"}, xr_pair{"PSMain", "ps_6_6"},
			xr_pair{"VSDebug", "vs_6_6"}, xr_pair{"PSDebug", "ps_6_6"},
			xr_pair{"VSOverlay", "vs_6_6"}})
		{
			...
			if (!Result.Succeeded())
				return Fail("The editor viewport shader did not compile for both APIs",
					Result.Diagnostics);
		}
	}
	return 0;
