...
int main()
{
	...
	OverlayText[0].Text = "owned viewport label";
	if (!Mailbox.Submit(MakeSnapshot(xr_span(&Mesh, 1), xr_span(&Instance, 1), {}, Materials, DebugLines, DebugTriangles, 23, OverlayLines, OverlayTriangles, OverlayText, Lights)))
	{
	...
	if (!Mailbox.Submit(MakeSnapshot({}, MultiMeshObject, {}, Materials), &Diagnostic) ||
	...
	const xr_array OverrideMaterials = {
		Materials[0],
		FEditorMaterialSlotSource{{10}, {}, {}, "override surface", EEditorMaterialSlotFlags::None, "74dfac8a-6739-4253-804a-5d3369df759b"}
	};
	FEditorStaticMeshInstance OverriddenInstance = Instance;
	OverriddenInstance.MaterialOverrides.push_back({{9}, {10}});
	if (!Mailbox.Submit(MakeSnapshot({}, xr_span(&OverriddenInstance, 1), {}, OverrideMaterials), &Diagnostic) ||
	...
	if (Mailbox.Submit(MakeSnapshot({}, xr_span(&OverriddenInstance, 1), {}, Materials), &Diagnostic))
	{
	...
	if (Mailbox.Submit(MakeSnapshot({}, xr_span(&MissingBaseOverride, 1), {}, OverrideMaterials), &Diagnostic))
	{
	...
	if (Mailbox.Submit(MakeSnapshot({}, xr_span(&DuplicateOverride, 1), {}, OverrideMaterials), &Diagnostic))
	{
	...
	if (Mailbox.Submit(MakeSnapshot(xr_span(&InvalidIndexMesh, 1), {}, {}, Materials), &Diagnostic))
	{
		return Fail("An out-of-range mesh index was accepted");
	}

	const xr_array MissingMaterials = {FEditorMaterialSlotSource{{10}, "default", "textures/other", "other", EEditorMaterialSlotFlags::None}};
	if (Mailbox.Submit(MakeSnapshot(xr_span(&Mesh, 1), {}, {}, MissingMaterials), &Diagnostic))
	{
	...
	if (Mailbox.Submit(MakeSnapshot({}, xr_span(&MissingMeshInstance, 1)), &Diagnostic))
	{
	...
	if (Mailbox.Submit(MakeSnapshot(xr_span(&TransactionMesh, 1), xr_span(&InvalidTransactionInstance, 1), {}, Materials), &Diagnostic))
	{
	...
	if (Mailbox.Submit(MakeSnapshot({}, xr_span(&LeakedTransactionInstance, 1), {}, Materials), &Diagnostic))
	{
	...
	InvalidOverlayLines[0].Vertices[0].Position[0] =
		std::numeric_limits<float>::quiet_NaN();
	FEditorViewportSceneSnapshot InvalidOverlay = MakeSnapshot({}, {}, {}, {}, {}, {}, 25, InvalidOverlayLines);
	...
	InvalidOverlayText[0].Position[1] =
		std::numeric_limits<float>::quiet_NaN();
	FEditorViewportSceneSnapshot InvalidText = MakeSnapshot({}, {}, {}, {}, {}, {}, 26, {}, {}, InvalidOverlayText);
	...
	InvalidText = MakeSnapshot({}, {}, {}, {}, {}, {}, 27, {}, {}, InvalidOverlayText);
	if (Mailbox.Submit(InvalidText, &Diagnostic))
	{
	...
	if (Mailbox.Submit(MakeSnapshot({}, {}, {}, {}, {}, {}, 0, {}, {}, {}, InvalidLights), &Diagnostic))
	{
