...
CEnvironment::CEnvironment()
	: CurrentEnv(0), m_ambients_config(0)
{
	...
	{
		config = new CInifile(FS.update_path(filePath, _game_config_, "environment\\environment.ltx"), true, true, false);
		section = "environment";
	}
	else
	{
		R_ASSERT4(config->section_exist(section), "Path (which is [Argument 1]) doesn't exist and section (which is [Argument 2]) is missing. "
												  "Please, use either COP or SOC weather config",
				  filePath,
				  section);
	}
...
void CEnvironment::calculate_dynamic_sun_dir()
{
	...
	float cosSZA = std::sin(LatitudeR) * std::sin(deg2rad(D)) + std::cos(LatitudeR) *
