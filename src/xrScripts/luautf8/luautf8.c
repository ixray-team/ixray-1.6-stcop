            ...
            process_combining_marks:
                ...
                for (unsigned int i = 1; i < vec_size; i++) {
                    if ((vector[i - 1] & 0xFF) > (vector[i] & 0xFF)) {
                        ...
                        uint32_t* scratch = (uint32_t*)malloc(vec_size * sizeof(uint32_t));
                ...
                while (i < vec_size) {
                    ...
                    if (mark_entry) {
                        if (mark_entry->reason == REASON_MUST_CONVERT_1) {
                            ...
                            if (vec_size == vec_max) {
                                vec_max *= 2;
                                vector = (uint32_t*)realloc((vector == onstack) ? NULL : vector, vec_max * sizeof(uint32_t));
                            }
