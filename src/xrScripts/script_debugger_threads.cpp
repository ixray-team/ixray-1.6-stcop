...
u32 CDbgScriptThreads::Fill()
{
	...
	CScriptProcess* sp = g_pScriptEngine->script_process(ScriptEngine::eScriptProcessorGame);

	if (sp)
		res += FillFrom(sp);
	
	sp = g_pScriptEngine->script_process(ScriptEngine::eScriptProcessorLevel);
	if (sp)
		res += FillFrom(sp);
