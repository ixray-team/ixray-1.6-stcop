...
namespace EPALegacy
{
	...
	public:
		...
		SOrder&	appendInt	(const char* name, int v, int mn=-P_MAXINT, int mx=P_MAXINT);
		SOrder&	appendVector(const char* name, PVector::EType type, float vx, float vy, float vz, float mn=-P_MAXFLOAT, float mx=P_MAXFLOAT);
		SOrder&	appendDomain(const char* name, PDomain v);
		SOrder&	appendBool	(const char* name, bool b);
		SOrder&	appendString(const char* name, const shared_str& v);
		SOrder&	appendString(const char* name, const char* v);
		...
		PString& _string(const char* name){auto	it=strings.find(name);	R_ASSERT2(it!=strings.end(),name);	return it->second;}
		PEnum& _enum(const char* name){auto it = enums.find(name); bool found = it!=enums.end(); R_ASSERT(found,name); return it->second;}
	public:
	...
	struct EPACreator // the only reason I created this struct is to export this fucking function, because for some reason it didn't do that as normal function!
	{
