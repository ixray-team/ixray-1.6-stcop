...
void* _VertexStream::Lock	( u32 vl_Count, u32 Stride, u32& vOffset )
{
	...
	if ((vl_Count+vl_mPosition) >= vl_mSize)
	{
		...
		bool res = pVB->Lock( mPosition, bytes_need, (void**)&pData, eLOCK_DISCARD);
		if( !res )
			Msg( " pVB->Lock - failed: res = %d,mPosition = %d, bytes_need = %d, &pData = %x, eLOCK_DISCARD", res, mPosition, bytes_need, (void**)&pData );
...
//#endif
		...
		bool res = pVB->Lock			( mPosition, bytes_need, (void**)&pData, eLOCK_NOOVERWRITE);
		if( !res )
			Msg( " pVB->Lock - failed: res = %d,mPosition = %d, bytes_need = %d, &pData = %x, eLOCK_NOOVERWRITE", res, mPosition, bytes_need, (void**)&pData );
...
u16*	_IndexStream::Lock	( u32 Count, u32& vOffset )
{
	...
	u32 dwFlags = eLOCK_NOOVERWRITE;
	if ( 2*( Count + mPosition ) >= mSize )
	{
		mPosition	= 0;						// clear position
		dwFlags		= eLOCK_DISCARD;			// discard it's contens
		mDiscardID	++;
	}

	pIB->Lock				( mPosition * 2, Count * 2, (void**) &pLockedData, (eLockType)dwFlags);
