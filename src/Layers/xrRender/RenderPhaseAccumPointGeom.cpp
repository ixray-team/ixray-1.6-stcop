...
void CRenderTarget::accum_point_geom_create()
{
	...
	u32 vSize = 3 * 4;
	VERIFY(RHIUtils::CreateVertexBuffer(&g_accum_point_vb, du_sphere_vertices, vCount * vSize));

	// Indices
	u32	iCount = DU_SPHERE_NUMFACES * 3;
	VERIFY(RHIUtils::CreateIndexBuffer(&g_accum_point_ib, du_sphere_faces, iCount * 2));
}
