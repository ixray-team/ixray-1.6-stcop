...
// calculate
//------------------------------------------------------------------------------
ICF void	KEY_Interp	(CKey& D,const CKey& K1,const CKey& K2, float delta)
{
	VERIFY			(_valid(delta));
...
ICF bool operator < (const ConsistantKey& A, const ConsistantKey& B)	// note: inverse operator
{	return A.w>B.w; }
/*
...
*/
ICF void MakeKeysSelected(ConsistantKey *keys, int count)
{
	// sort in decreasing order
	std::sort(keys,keys+count);
}
...
*/
ICF	void QR2Quat(const CKeyQR &K,Fquaternion &Q)
{
...
ICF void QT8_2T(const CKeyQT8& K, const CMotion& M, Fvector &T)
{
...
ICF void QT16_2T(const CKeyQT16& K, const CMotion& M, Fvector &T)
{
	T.x		= float(K.x1)*M._sizeT.x+M._initT.x;
	T.y		= float(K.y1)*M._sizeT.y+M._initT.y;
	T.z		= float(K.z1)*M._sizeT.z+M._initT.z;
...
ICF void QT_FFT_l(const CKeyQT32& K, Fvector& T)
{
	T.x = K.x1;
	T.y = K.y1;
	T.z = K.z1;
...
ICF void QuatL(const CKeyQR32& K, Fquaternion& Q)
{
	Q.x = K.x;
	Q.y = K.y;
	Q.z = K.z;
...
ICF void Dequantize(CKey& K, const CBlend& BD, const CMotion& M)
{
	CKey* D = &K;
	const CBlend* B = &BD;
...
ICF void MixInterlerp( CKey &Result, const CKey	*R, const CBlend* const BA[MAX_BLENDED], int b_count )
{
...
ICF void key_sub(CKey &rk, const CKey &k0, const CKey& k1)//sub right
{
	Fquaternion q;
	q.inverse(k1.Q);
	rk.Q.mul(k0.Q,q);
...
ICF void key_identity(CKey &k)
{
	k.Q.identity();
	k.T.set(0,0,0);
}
ICF void key_add(CKey &res, const CKey &k0, const CKey &k1)//add right
{
...
ICF void q_scale(Fquaternion &q, float v)
{
...
ICF void key_scale(CKey &res, const CKey &k, float v)
{
	res = k;
	q_scale(res.Q,v);
	res.T.mul(v);
}
ICF void key_mad(CKey &res, const CKey &k0, const CKey& k1, float v)
{
	CKey k ;
	key_scale(k,k1,v);
	key_add(res,k,k0);
...
ICF void keys_substruct(CKey	*R, const CKey	*BR, int b_count )
{
	for (int i=0; i<b_count; i++)
	{
		CKey r;
...
ICF void q_scalem(Fmatrix &m, float v)
{
	Fquaternion q;
	q.set(m);
	q_scale(q,v);
...
ICF void q_scale_vs_basem(Fmatrix &m_res,const Fquaternion &q, const Fquaternion &base,float scale_factor)
{
	Fmatrix mb,imb;
	mb.rotation(base);
	imb.invert(mb);
...
ICF void q_add_scaled_basem( Fquaternion &q, const Fquaternion &base, const Fquaternion &q0, const Fquaternion &q1, float v1 )
{
	//VERIFY(0.f =< v && 1.f >= v );
...
ICF bool check_scale(const Fmatrix &m)
{
	float det = DET(m);
	return (0.8f<det&&det<1.3f);
}

ICF bool check_scale(const Fquaternion &q)
{
	Fmatrix m;
	m.rotation(q);
...
ICF void MixFactors(float *F,int b_count)
{
	float sum = 0;
	for (int i=0; i<b_count; i++)
		sum+=F[i];
...
ICF void MixinAdd(CKey &Result,const CKey	*R,const float* BA,int b_count)
{
	for (int i=0; i<b_count; i++)
		key_mad(Result,Result,R[i],BA[i]);
}
ICF void MixAdd(CKey &Result,const CKey	*R,const float* BA,int b_count)
{
	key_identity(Result);
	MixinAdd(Result,R,BA,b_count);
}
ICF void process_single_channel( CKey &Result,const animation::channel_def &ch, const CKey *R, const CBlend* const BA[MAX_BLENDED], int b_count )
{
...
ICF void MixChannels(CKey &Result,const CKey	*R,const animation::channel_def* BA,int b_count)
{
	VERIFY(b_count>0);
	Result = R[0];
