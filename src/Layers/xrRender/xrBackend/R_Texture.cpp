...
inline PixelFormat GetPixelFormatFromDXGI(DXGI_FORMAT format)
{
    switch (format)
    {
    ...
    case DXGI_FORMAT_R32G32B32_FLOAT:
        return FMT_R32G32B32F;
    case DXGI_FORMAT_R32G32B32A32_FLOAT:
        return FMT_R32G32B32A32F;
    case DXGI_FORMAT_D32_FLOAT:
        return FMT_DEPTH32F;
    case DXGI_FORMAT_D24_UNORM_S8_UINT:
        return FMT_DEPTH24_STENCIL_8;
    default:
        break;
    }

    FATAL("Unkonwed DXGI_FORMAT");
    return FMT_UNKNOWN;
}

HRESULT CreateTextureRHI(
    ...
    IBaseTexture** ppResource) noexcept
{
    ...
    *ppResource = nullptr;
    ...
    std::unique_ptr<SUBRESOURCE_DATA[]> initData(new (std::nothrow) SUBRESOURCE_DATA[metadata.mipLevels * metadata.arraySize]);
    if (!initData)
    ...
    // Our texture desc are incapsulate all texture dimensions
    TextureDesc desc = {};

    switch (metadata.dimension)
    {
    case TEX_DIMENSION_TEXTURE1D:
        desc.textureType = TextureType::TEXTURE_1D;
        break;
    case TEX_DIMENSION_TEXTURE2D:
        desc.textureType = TextureType::TEXTURE_2D;
        break;
    case TEX_DIMENSION_TEXTURE3D:
        desc.textureType = TextureType::TEXTURE_3D;
        break;
    default:
        Msg("Unknowed texture dimension");
        return E_ABORT;
    }
    ...
    desc.mipmapLevel = static_cast<UINT>(metadata.mipLevels);
    desc.arraySize = static_cast<UINT>(metadata.arraySize);
    desc.format = GetPixelFormatFromDXGI(format);
    desc.usage = usage;
    desc.dynamic = usage == ResourceUsage::DYNAMIC;
    desc.cubemap = metadata.IsCubemap();

    *ppResource = g_rbackend->CreateTexture(&desc, initData.get());
    hr = S_OK;
    return hr;
}

IBaseTexture* CRender::texture_load(LPCSTR fRName, u32& ret_msize, bool bStaging /* = false*/)
{
    ...
    IBaseTexture* pTexture2D = nullptr;
    string_path fn;
    u32 img_size = 0;
    ...
    _DDS_CUBE: {
        ...
        auto usage = (bStaging) ? ResourceUsage::STAGING : ResourceUsage::IMMUTABLE;
        ...
        hr = CreateTextureRHI(scratchImage->GetImages(), scratchImage->GetImageCount(),
    ...
    _DDS_2D: {
        ...
        hr = CreateTextureRHI(scratchImage->GetImages() + mip_lod, scratchImage->GetImageCount(),
            imageInfo, usage, bindFlags, cpuAccessFlags, miscFlags, CREATETEX_FLAGS::CREATETEX_DEFAULT, &pTexture2D);
        FS.r_close(reader);
