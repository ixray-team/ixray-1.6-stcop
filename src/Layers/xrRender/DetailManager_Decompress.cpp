...
void CDetailManager::UnpackSlotItems(Slot* S)
{
	...
	extern bool ps_r__detail_use_perlin_noise;
	extern float ps_r__detail_perlin_noise_frequency;
	extern float ps_r__detail_perlin_noise_amplitude;
	extern float ps_r__detail_perlin_noise_random_height;
	...
	for (u32 z = 0; z <= d_size; z++)
	{
		for (u32 x = 0; x <= d_size; x++)
		{
			...
			const CDetail& Dobj = *objects[obj_id];
#endif
			if (!ps_r__detail_use_perlin_noise)
			{
			...
			else
			{
				...
				noise = fastNoise2D(Item_P.x * ps_r__detail_perlin_noise_frequency, Item_P.z * ps_r__detail_perlin_noise_frequency);
				baseScale = minScale + (maxScale - minScale) * (noise * ps_r__detail_perlin_noise_amplitude + (1 - ps_r__detail_perlin_noise_amplitude) * 0.5f);
				offsetRange = (maxScale - minScale) * ps_r__detail_perlin_noise_random_height;
				randomOffset = r_scale.randF(-offsetRange, offsetRange);
				scale = baseScale + randomOffset;
