...
void	light::vis_prepare()
{
	...
	//log_cryray_engine::Msg	("sc[%f,%f,%f]/c[%f,%f,%f] - sr[%f]/r[%f]",VPUSH(spatial.center),VPUSH(position),spatial.radius,range);
	//log_cryray_engine::Msg	("dist:%f, sa:%f",Device.vCameraPosition.distance_to(spatial.center),safe_area);
	bool	skiptest = true;
...
void light::vis_update()
{
	...
	//log_cryray_engine::Log					("",fragments);
