...
FHierrarhyVisual::~FHierrarhyVisual()
{
	if (!bDontDelete) {
		for (u32 i=0; i<children.size(); i++)	
			xrAPI.Render->model_Delete((IRenderVisual *&)children[i]);
	}
	children.clear();
}
...
#else
			u32 ID	= data->r_u32();
			children[i]	= (dxRender_Visual*)xrAPI.Render->getVisual(ID);
#endif
		}
		bDontDelete = TRUE;
	}
    	...
    	if (data->find_chunk(OGF_CHILDREN))
		{
            ...
            if (OBJ){
                ...
                for (int count=1; O; count++) {
					...
					children.push_back	((dxRender_Visual*)xrAPI.Render->model_CreateChild(name_load,O));
                    O->close			();
...
void	FHierrarhyVisual::Copy(dxRender_Visual *pSrc)
{
	...
	for (u32 i=0; i<pFrom->children.size(); i++) {
		dxRender_Visual *p = (dxRender_Visual*)xrAPI.Render->model_Duplicate	(pFrom->children[i]);
