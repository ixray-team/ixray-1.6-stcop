...
template<class C>	void mdump(C c)
{
	...
	for (C::iterator I=c.begin(); I!=c.end(); I++)
		log_cryray_engine::Msg	("*        : %3d: %s",I->second->dwReference, I->second->cName.c_str());
}
...
{
	log_cryray_engine::Msg		("* RM_Dump: textures  : %d",		m_textures.size());		if(!bBrief) mdump(m_textures);
	log_cryray_engine::Msg		("* RM_Dump: rtargets  : %d",		m_rtargets.size());		if(!bBrief) mdump(m_rtargets);
//	DX10 cut 	log_cryray_engine::Msg		("* RM_Dump: rtargetsc : %d",		m_rtargets_c.size());	if(!bBrief) mdump(m_rtargets_c);
	log_cryray_engine::Msg		("* RM_Dump: vs        : %d",		m_vs.size());			if(!bBrief) mdump(m_vs);
	log_cryray_engine::Msg		("* RM_Dump: ps        : %d",		m_ps.size());			if(!bBrief) mdump(m_ps);
	log_cryray_engine::Msg		("* RM_Dump: dcl       : %d",		v_declarations.size());
	log_cryray_engine::Msg		("* RM_Dump: states    : %d",		v_states.size());
	log_cryray_engine::Msg		("* RM_Dump: tex_list  : %d",		lst_textures.size());
	log_cryray_engine::Msg		("* RM_Dump: matrices  : %d",		lst_matrices.size());
	log_cryray_engine::Msg		("* RM_Dump: lst_constants: %d",	lst_constants.size());
	log_cryray_engine::Msg		("* RM_Dump: v_passes  : %d",		v_passes.size());
	log_cryray_engine::Msg		("* RM_Dump: v_elements: %d",		v_elements.size());
	log_cryray_engine::Msg		("* RM_Dump: v_shaders : %d",		v_shaders.size());
}
