...
#ifndef _EDITOR
#include "../../xrGameAPI/smart_cast.h"
#else
...
//.         strconcat		(sizeof(name_load),name_load, short_name, ":lod:", lod_name.c_str());
            m_lod 			= (dxRender_Visual*) xrAPI.Render->model_CreateChild(lod_name, NULL);
...
void CKinematics::Copy(dxRender_Visual *P) 
{
    ...
    m_lod 	   = (pFrom->m_lod)?(dxRender_Visual*)xrAPI.Render->model_Duplicate	(pFrom->m_lod):0;
}
	...
	if (!wallmarks.empty()&&(wm_frame!=Device.dwFrame)){
		...
		for (SkeletonWMVecIt it=wallmarks.begin(); it!=wallmarks.end(); it++){
			...
			if (w<1.f){
				...
				if (xrAPI.Render->ViewBase.testSphere_dirty(wm->m_Bounds.P,wm->m_Bounds.R))
