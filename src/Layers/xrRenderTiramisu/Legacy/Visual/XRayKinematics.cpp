...
void CDS0_Kinematics::Visibility_Update()
{
	...
	{
		CDS0_SkeletonX* _c = dynamic_cast<CDS0_SkeletonX*>(children[c_it]);
		VERIFY(_c);
	...
	{
		CDS0_SkeletonX* _c = dynamic_cast<CDS0_SkeletonX*>(children_invisible[_it]);
		VERIFY(_c);
		if (_c->has_visible_bones())
		{
...
void RecursiveBindTransform(CDS0_Kinematics* K, buffer_vector<Fmatrix>& matrices, u16 bone_id, const Fmatrix& parent)
{
	CBoneData& BD = K->LL_GetData(bone_id);
	Fmatrix& BM = matrices[bone_id];
	// Build matrix
...
void CDS0_Kinematics::LL_GetBindTransform(buffer_vector<Fmatrix>& matrices)
{
	matrices.resize(LL_BoneCount());
	RecursiveBindTransform(this, matrices, iRoot, Fidentity);
}
...
void CDS0_Kinematics::CalculateBones(bool bForceExact)
{
	...
	if (DevicePtr->dwTimeGlobal == UCalc_Time)
	{
	...
	UCalc_Time = DevicePtr->dwTimeGlobal;
