...
void CDS0_KinematicsAnimated::LL_UpdateTracks(float dt, bool b_force, bool leave_blends)
{
	...
	for (u16 part = 0; part < MAX_PARTS; part++)
	{
		...
		{
			CBlend& B = *(*I);
			if (!b_force && B.dwFrame == DevicePtr->dwFrame)
			{
				continue;
			}
			B.dwFrame = DevicePtr->dwFrame;
...
{
	;
	if (Update_LastTime == DevicePtr->dwTimeGlobal)
	{
		return;
	}
	u32 DT = DevicePtr->dwTimeGlobal - Update_LastTime;
	if (DT > 66)
	{
		DT = 66;
	}
	...
	{
		if ((*GetUpdateTracksCalback())(float(DevicePtr->dwTimeGlobal - Update_LastTime) / 1000.f, *this))
		{
			Update_LastTime = DevicePtr->dwTimeGlobal;
		}
		return;
	}
	Update_LastTime = DevicePtr->dwTimeGlobal;
...
CDS0_KinematicsAnimated::CDS0_KinematicsAnimated()
	: ::CDS0_Kinematics(), IKinematicsAnimated(), blend_instances(NULL), m_Partition(NULL), m_blend_destroy_callback(0), m_update_tracks_callback(0), Update_LastTime(0)
{
}
