...
#include "Legacy/Visual/XRayFHierrarhyVisual.h"


void TiramisuLegacyRenderGraph::Clear()
{
	RenderList.clear();
}

void TiramisuLegacyRenderGraph::ClearViews()
{
	Views.clear();
	ViewMasks.clear();
}

void TiramisuLegacyRenderGraph::PushView(CFrustum* View)
{
	Views.push_back(View);
	ViewMasks.push_back(View->getMask());
}

void TiramisuLegacyRenderGraph::SetSsaThresholds(float DiscardThreshold, float GlodStartThreshold, float GlodEndThreshold)
{
...
bool TiramisuLegacyRenderGraph::IsValuableToRender(CDS0_RenderVisual* Visual) const
{
...
bool TiramisuLegacyRenderGraph::IsVisible(CDS0_RenderVisual* Visual, bool& OutPartial) const
{
	if (Views.empty())
	{
		OutPartial = false;
...
bool TiramisuLegacyRenderGraph::ShouldAddLeaf(CDS0_RenderVisual* Visual) const
{
...
void TiramisuLegacyRenderGraph::AddStatic(CDS0_RenderVisual* Visual)
{
	if (!Visual)
	{
		return;
...
void TiramisuLegacyRenderGraph::AddStaticLeafs(CDS0_RenderVisual* Visual)
{
	if (!Visual)
	{
		return;
...
void TiramisuLegacyRenderGraph::InsertStatic(CDS0_RenderVisual* Visual)
{
	float DistanceSq = 0.f;
	const float Ssa = CalculateSsa(Visual, DistanceSq);
...
float TiramisuLegacyRenderGraph::CalculateSsa(CDS0_RenderVisual* Visual, float& OutDistanceSq) const
{
	OutDistanceSq = DevicePtr->vCameraPosition.distance_to_sqr(Visual->Vis.sphere.P) + EPS;
	return Visual->Vis.sphere.R / OutDistanceSq;
}

float TiramisuLegacyRenderGraph::CalculateLod(float Ssa) const
{
