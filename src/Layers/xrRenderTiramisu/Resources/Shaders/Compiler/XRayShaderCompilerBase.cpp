...
XRayShaderCompilerBase* XRayShaderCompilerBase::Create(nri::GraphicsAPI GraphicsAPI, bool NeedCreateShaderPDB, bool DebugShader)
{
	return new XRayShaderCompilerDesktop(GraphicsAPI,NeedCreateShaderPDB, DebugShader);
}

XRayShaderCompilerBase::XRayShaderCompilerBase(bool NeedCreateShaderPDB, bool DebugShader) : 
	bNeedCreateShaderPDB(NeedCreateShaderPDB), bDebugShader(DebugShader), IncludeCrc32(0), IncludeCount(0), IncludeSize(0)
{
}

bool XRayShaderCompilerBase::Check(const char* SourceFileName, const XRayShaderDefinesContainer& Defines, const char* ResultFileName)
{
...
bool XRayShaderCompilerBase::Check(const char* RootSignature, const char* ResultFileName)
{
...
void XRayShaderCompilerBase::RescanIncludes()
{
	FS_FileSet Files;
...
bool XRayShaderCompilerBase::CheckIncludes()
{
	FS_FileSet Files;
...
void XRayShaderCompilerBase::ReadBinaryFile(const char* FileName, xr_vector<char>& Data)
{
	ReadFile(FileName, Data);
	Data.erase(Data.begin(), Data.begin());
}

void XRayShaderCompilerBase::ReadRootSignatureFile(const char* FileName, xr_vector<char>& Data)
{
	ReadFile(FileName, Data);
	Data.erase(Data.begin(), Data.begin() + 4 * 2);
}

void XRayShaderCompilerBase::ReadTextFile(const char* FileName, xr_vector<char>& Data, bool& bIsUTF8)
{
	ReadFile(FileName, Data);
