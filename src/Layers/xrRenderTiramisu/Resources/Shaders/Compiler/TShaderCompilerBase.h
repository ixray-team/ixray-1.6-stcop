...
{
public:
	static TShaderCompilerBase*		Create					(nri::GraphicsAPI GraphicsAPI, bool NeedCreateShaderPDB, bool DebugShader);
									TShaderCompilerBase		(bool NeedCreateShaderPDB, bool DebugShader);
	virtual							~TShaderCompilerBase	() = default;

	virtual bool					Compile					(const TShaderDefinesContainer& Defines, EShaderType ShaderType, const xr_vector<xr_string>& IncludePaths, const char* source_file_name, const char* result_file_name, xr_string& OutMessage) = 0;
	//virtual bool					CompileRootSignature	(const char* RootSignature, const char* result_file_name, xr_string& OutMessage) = 0;

	bool							Check					(const char* SourceFileName, const TShaderDefinesContainer& Defines, const char* ResultFileName);
