...
TShaderCompilerBase* TShaderCompilerBase::Create(nri::GraphicsAPI GraphicsAPI, bool NeedCreateShaderPDB, bool DebugShader)
{
	return new TShaderCompilerDesktop(GraphicsAPI,NeedCreateShaderPDB, DebugShader);
}

TShaderCompilerBase::TShaderCompilerBase(bool NeedCreateShaderPDB, bool DebugShader) : 
	bNeedCreateShaderPDB(NeedCreateShaderPDB), bDebugShader(DebugShader), IncludeCrc32(0), IncludeCount(0), IncludeSize(0)
{
}

bool TShaderCompilerBase::Check(const char* SourceFileName, const TShaderDefinesContainer& Defines, const char* ResultFileName)
{
...
bool TShaderCompilerBase::Check(const char* RootSignature, const char* ResultFileName)
{
...
void TShaderCompilerBase::RescanIncludes()
{
	FS_FileSet Files;
...
bool TShaderCompilerBase::CheckIncludes()
{
	FS_FileSet Files;
...
void TShaderCompilerBase::ReadBinaryFile(const char* FileName, xr_vector<char>& Data)
{
	ReadFile(FileName, Data);
	Data.erase(Data.begin(), Data.begin());
}

void TShaderCompilerBase::ReadRootSignatureFile(const char* FileName, xr_vector<char>& Data)
{
	ReadFile(FileName, Data);
	Data.erase(Data.begin(), Data.begin() + 4 * 2);
}

void TShaderCompilerBase::ReadTextFile(const char* FileName, xr_vector<char>& Data, bool& bIsUTF8)
{
	ReadFile(FileName, Data);
