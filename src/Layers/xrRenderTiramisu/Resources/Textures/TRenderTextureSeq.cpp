...
TRenderTextureSeq::~TRenderTextureSeq()
{
	for (TRenderTexture*& Textxure : FrameTextures)
	{
		Owner->Free(Textxure);
	}
...
bool TRenderTextureSeq::LoadFrameTexture(const char* TextureName)
{
	if (TRenderTexture* ExistingTexture = Owner->GetTexture(TextureName))
	{
