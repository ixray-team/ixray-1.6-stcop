...
EXRayLegacyLevelVertexType TRenderLegacyScene::GetAndConvertFVF(CStreamReader* base_fs, uint32_t& OutSize)
{
	...
	if (memcmp(TempArray,LDeclR1LMap,sizeof(LDeclR1LMap)) == 0)
	{
		...
		OutSize = sizeof(FXRayLegacyLevelVertex_BaseWithLightMap);
		return EXRayLegacyLevelVertexType::BaseWithLightMap;
	}
	if (memcmp(TempArray,LDeclR1Vert,sizeof(LDeclR1Vert)) == 0)
	{
		constexpr int32_t Offset = sizeof(LDeclR1Vert) - sizeof(TempArray);
		base_fs->advance(Offset);
		OutSize = sizeof(FXRayLegacyLevelVertex_BaseWithLightColor);
		return EXRayLegacyLevelVertexType::BaseWithLightColor;
	}
	if (memcmp(TempArray,LDeclMUModel,sizeof(LDeclMUModel)) == 0)
	{
		constexpr int32_t Offset = sizeof(LDeclMUModel) - sizeof(TempArray);
		base_fs->advance(Offset);
		OutSize = sizeof(FXRayLegacyLevelVertex_MultipleUsageModel);
		return EXRayLegacyLevelVertexType::MultipleUsageModel;
	}
	if (memcmp(TempArray,LDeclXVert,sizeof(LDeclXVert)) == 0)
	{
		constexpr int32_t Offset = sizeof(LDeclXVert) - sizeof(TempArray);
		base_fs->advance(Offset);
		OutSize = sizeof(FXRayLegacyLevelVertex_OnlyVertex);
		return EXRayLegacyLevelVertexType::VertexOnly;
	}
	NODEFAULT;
    return EXRayLegacyLevelVertexType::VertexOnly;
}
