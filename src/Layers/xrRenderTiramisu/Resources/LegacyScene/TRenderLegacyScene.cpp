...
TRenderLegacyScene::~TRenderLegacyScene()
{
	Clear();
}

void TRenderLegacyScene::LoadLevel(IReader* FileReader)
{
	Clear();
    ...
    IReader* ShadersChunk;
    {
        ...
        uint32_t Count = ShadersChunk->r_u32();
...
void TRenderLegacyScene::Clear()
{
	...
	{
		for (XRayTexture* Texture : Shader.Textures)
		{
...
TLegacySceneSector* TRenderLegacyScene::GetSectorByRay(const Fvector& Position, const Fvector& Direction)
{
...
TLegacySceneSector* TRenderLegacyScene::GetSector(Fvector Position)
{
	TLegacySceneSector* Result = nullptr;
	Fvector dir;
...
void TRenderLegacyScene::Calculate()
{
	RenderGraph.Clear();
	...
	{
		TLegacySceneSector* NewCurrentSector = GetSector(DevicePtr->vCameraPosition);
	...
	if (PortalsCollisionModel)
	{
		...
		for (int32_t i = 0; i < PortalsCollider.r_count(); i++)
		{
	...
	if (LastSector)
	{
		...
		if (psDeviceFlags.test(rsDrawStatic))
		{
			...
			{
				TLegacySceneSector* Sector = GPortalTraverser.r_sectors[s_it];
...
void TRenderLegacyScene::LoadBuffers(CStreamReader* Reader)
{
    xr_vector<uint8_t> MegaBuffer;
	...
	R_ASSERT(Reader->find_chunk(fsL_VB));
    {
        ...
        uint32_t CountBuffers = BuffersStreamReader->r_u32();
        for (uint32_t i = 0; i < CountBuffers; i++)
        {
            ...
            uint32_t CountVertex = BuffersStreamReader->r_u32();
            uint32_t Offset = MegaBuffer.size();
    ...
    R_ASSERT(Reader->find_chunk(fsL_IB));
    {
    	...
    	uint32_t CountIndexBuffers = BufferStreamReader->r_u32();
    	for (uint32_t i = 0; i < CountIndexBuffers; i++)
    	{
    		uint32_t IndexCount = BufferStreamReader->r_u32();
...
void TRenderLegacyScene::LoadVisuals(IReader* Reader)
{
...
void TRenderLegacyScene::LoadSectors(IReader* Reader)
{
	uint32_t HeaderSize = Reader->find_chunk(fsL_PORTALS);
	R_ASSERT(0 == HeaderSize % sizeof(FPortalHeader));
	uint32_t HeaderCount = HeaderSize / sizeof(FPortalHeader);
	
	Portals.resize(HeaderCount);
	for (uint32_t i = 0; i < HeaderCount; i++)
	{
		Portals[i] = xr_make_unique<TLegacyScenePortal>();
	}
	...
	if (HeaderCount)
	{
		...
		for (uint32_t i = 0; i < HeaderCount; i++)
		{
			...
			for (uint32_t j = 2; j < P.vertices.size(); j++)
			{
...
void TRenderLegacyScene::LoadSWIs(CStreamReader* base_fs)
{
}
