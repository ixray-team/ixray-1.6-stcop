...
TLegacyScenePortal::TLegacyScenePortal		()
{
#ifdef DEBUG
...
#ifdef DEBUG
void TLegacyScenePortal::OnRender	()
{
...
#endif
//
void	TLegacyScenePortal::Setup	(Fvector* V, size_t vcnt, TLegacySceneSector* face, TLegacySceneSector* back)
{
	// calc sphere
	Fbox				BB;
	BB.invalidate		();
...
//
TLegacySceneSector::~TLegacySceneSector()
{
}


void TLegacySceneSector::traverse			(CFrustum &F, _scissor& R_scissor)
{
	...
	for	(u32 I=0; I<m_portals.size(); I++)
	{
		...
		TLegacyScenePortal* PORTAL = m_portals[I];
		TLegacySceneSector* pSector;
...
void TLegacySceneSector::load(IReader& fs)
{
...
void CPortalTraverser::traverse			(TLegacySceneSector* start, CFrustum& F, Fvector& vBase, Fmatrix& mXFORM, u32 options)
{
	...
	i_start				= (TLegacySceneSector*)start;
	r_sectors.clear		();
	_scissor			scissor;
	scissor.set			(0,0,1,1);
	scissor.depth		= 0;
	...
	if (options & VQ_SCISSOR)		{
		...
		{
			TLegacySceneSector*	S		= (TLegacySceneSector*)r_sectors[s];
...
void CPortalTraverser::fade_portal	(TLegacyScenePortal* _p, float ssa)
{
...
ICF		bool	psort_pred			(const std::pair<TLegacyScenePortal*, float>& _1, const std::pair<TLegacyScenePortal*, float>& _2)
{
