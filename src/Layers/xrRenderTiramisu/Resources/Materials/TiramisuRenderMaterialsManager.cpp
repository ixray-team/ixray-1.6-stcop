...
xr_optional<xr_string> ReadMaterialAssetSource(const char* FileName)
{
	...
	xr_string Json(static_cast<const char*>(Reader->pointer()), static_cast<size_t>(Reader->length()));
	FS.r_close(Reader);
	return Json;
}
...
)
{
	...
	for (const auto& [Parameter, Value] : RuntimeOverrides)
	{
		...
		Instance->SetTextureParameter(Parameter, GRenderResourcesManager->TexturesManager->GetTexture(std::get<xr_string>(Value).c_str()));
	}
	...
	Msg("* Tiramisu legacy material '%s' -> parent instance '%s' (%s)", ShaderName.c_str(), Resolved.Material.c_str(), InName.c_str());
	return Instance;
}
...
void TiramisuRenderMaterialsManager::LoadSourceMaterialAssets()
{
	...
	FS.file_list(MasterFiles, "$game_render_materials$", FS_ListFiles, "*.material.json");
	FS.file_list(InstanceFiles, "$game_render_materials$", FS_ListFiles, "*.material-instance.json");
...
void TiramisuRenderMaterialsManager::LoadLegacyMaterialMap()
{
	...
	const xr_string Json(static_cast<const char*>(Reader->pointer()), static_cast<size_t>(Reader->length()));
	FS.r_close(Reader);
