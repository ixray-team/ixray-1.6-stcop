    ...
    const char* Stage)
{
    ...
    {
        Msg("! Tiramisu material GPU %s [%s]: %s", Stage,
...
void TiramisuRenderMaterialGpuStorage::CreateBuffer_RenderThread(
    const u64 Size, nri::Buffer*& Buffer,
    nri::Descriptor*& Descriptor, u32& DescriptorIndex)
{
    CheckIsRenderThread();
    ...
    NRI_CHECK(GRenderDevice.CoreInterface.CreateCommittedBuffer(
        *GRenderDevice.Device, nri::MemoryLocation::DEVICE_UPLOAD, 0.5f,
...
void TiramisuRenderMaterialGpuStorage::DestroyBuffer_RenderThread(
    nri::Buffer*& Buffer, nri::Descriptor*& Descriptor, u32& DescriptorIndex)
{
    CheckIsRenderThread();
...
bool TiramisuRenderMaterialGpuStorage::WriteBuffer_RenderThread(
    nri::Buffer& Buffer, const u64 Offset, const void* Data, const u64 Size)
{
    CheckIsRenderThread();
    if (!Data || Size == 0)
        return false;
...
u32 TiramisuRenderMaterialGpuStorage::CreateMaterialInstance_RenderThread(
    const xr_span<const u8> ParameterData, const u64 LayoutHash)
{
    CheckIsRenderThread();
    ...
    if (ParameterOffset > MaterialParameterCapacity ||
        AlignedParameterSize > MaterialParameterCapacity - ParameterOffset)
        return INDEX_NONE;

    std::memset(MaterialParameterMirror.data() + ParameterOffset, 0, AlignedParameterSize);
    std::memcpy(MaterialParameterMirror.data() + ParameterOffset,
        ParameterData.data(), ParameterData.size());
    if (!WriteBuffer_RenderThread(*MaterialParameterBuffer, ParameterOffset,
            MaterialParameterMirror.data() + ParameterOffset, AlignedParameterSize))
        return INDEX_NONE;
    ...
    if (!WriteBuffer_RenderThread(*MaterialInstanceBuffer,
            u64(MaterialInstanceIndex) * sizeof(InstanceData),
...
bool TiramisuRenderMaterialGpuStorage::UpdateMaterialInstance_RenderThread(
    const u32 MaterialInstanceIndex, const xr_span<const u8> ParameterData)
{
    CheckIsRenderThread();
    ...
    return WriteBuffer_RenderThread(*MaterialParameterBuffer,
        InstanceData.ParameterDataOffset, CurrentData, ParameterData.size());
}

u32 TiramisuRenderMaterialGpuStorage::GetOrCreateMaterial_RenderThread(
    const TiramisuMaterialRenderProxy& Material)
{
    CheckIsRenderThread();
    const FMaterialAssetId& Asset =
        Material.GetAssetReference();
    if (!Asset.IsValid())
        return INDEX_NONE;

    FMaterialInstanceCacheKey CacheKey;
    CacheKey.Asset = Asset;
    for (const FMaterialTextureParameterBinding& Binding :
        Material.GetTextureParameters())
    {
        if (!Binding.Parameter.IsValid() || !Binding.Texture ||
            !Binding.Texture->Descriptor)
        {
            continue;
        }
        CacheKey.TextureParameters.emplace_back(
            Binding.Parameter, Binding.Texture->GetOrCreateHeapID());
    }
    std::ranges::sort(CacheKey.TextureParameters);

    if (const auto Existing = MaterialInstanceCache.find(CacheKey);
        Existing != MaterialInstanceCache.end())
        return Existing->second;

    const FResolvedMaterialInstance* Resolved =
        GRenderResourcesManager->MaterialShaderLibrary->
            ResolveMaterial_RenderThread(Asset);
    const FMaterialAsset* Master =
        GRenderResourcesManager->MaterialShaderLibrary->
            ResolveMaster_RenderThread(Asset);
    if (!Resolved || !Master)
    {
        Resolved = GRenderResourcesManager->MaterialsManager->
            ResolveSourceMaterial_RenderThread(Asset);
        Master = GRenderResourcesManager->MaterialsManager->
            ResolveSourceMaster_RenderThread(Asset);
    }
    if (!Resolved || !Master)
    {
        Msg("! Tiramisu material GPU: asset '%s' is absent from materials.xrm and the development source registry.",
            Asset.Value.c_str());
        return INDEX_NONE;
    }
    ...
    const FMaterialParameterPackResult Packed =
        PackMaterialParameters(
            Layout.Value, Master->Parameters, Resolved->Parameters);
    if (!Packed.Succeeded())
    {
    ...
    const FMaterialParameterPackResult Patched =
        PatchMaterialParameterResources(Packed.Value,
            [&CacheKey](const FMaterialParameterResourceReference& Reference)
                -> xr_optional<FDescriptorHeapIndex>
            {
                if (Reference.Type ==
                    EMaterialParameterType::SamplerPreset)
                {
                ...
                const auto Override = std::ranges::find_if(
                    ...
                    [&Reference](const auto& Entry)
                    {
                        return Entry.first == Reference.Parameter;
                    });
                if (Override != CacheKey.TextureParameters.end())
                {
                    return FDescriptorHeapIndex{
                        Override->second};
                }

                if (Reference.Type ==
                    EMaterialParameterType::Texture2D)
                {
                    TiramisuRenderTextureResourceProxy* Fallback =
                        GRenderResourcesManager->WhiteTexture->ResourceProxy;
                    if (Fallback && Fallback->Descriptor)
                    {
                        return FDescriptorHeapIndex{
                            Fallback->GetOrCreateHeapID()};
                    }
                }
                return std::nullopt;
            });
    ...
    const u32 MaterialInstanceIndex =
        CreateMaterialInstance_RenderThread(
            Patched.Value.Data, Patched.Value.LayoutHash);
    if (MaterialInstanceIndex != INDEX_NONE)
        MaterialInstanceCache.emplace(std::move(CacheKey), MaterialInstanceIndex);
    return MaterialInstanceIndex;
}

u32 TiramisuRenderMaterialGpuStorage::AddDraw_RenderThread(
    const FMaterialDrawGpuData& DrawData)
{
    ...
    if (!WriteBuffer_RenderThread(*DrawDataBuffer,
            u64(DrawIndex) * sizeof(DrawData), &DrawData, sizeof(DrawData)))
        return INDEX_NONE;
    ++DrawCount;
    return DrawIndex;
}
