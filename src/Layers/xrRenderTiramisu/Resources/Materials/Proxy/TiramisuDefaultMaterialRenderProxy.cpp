...
const char* GetLegacyVertexFactoryName(const EVertexType VertexType)
{
	switch (VertexType)
	{
		case EVertexType::BaseWithLightMap:
			return "legacy_level_lmap";
		case EVertexType::BaseWithLightColor:
			return "legacy_level_color";
		default:
			return "legacy_unknown";
	}
}

u64 MakeLegacyPipelineKey(const FMaterialAssetId& AssetReference, const EVertexType VertexType)
{
...
TiramisuDefaultMaterialRenderProxy::TiramisuDefaultMaterialRenderProxy(FMaterialAssetId InAssetReference)
{
	CheckIsGameThread();
	AssetReference = InAssetReference.Value == "default" ? FMaterialAssetId{"ee5ffbc0-bd24-4aa8-9e16-50651ca1c269"} : std::move(InAssetReference);
...
void TiramisuDefaultMaterialRenderProxy::Initialize_RenderThread(const FMaterialAssetId& AssetReference)
{
	CheckIsRenderThread();
	...
	auto RegisterPipeline = [&](const EVertexType VertexType, const char* ShaderName, nri::VertexAttributeDesc* Attributes)
	{
		...
		Pipelines.emplace(FPipelineMapKey{EMaterialPass::GBuffer, VertexType}, std::move(Entry));
	};
	...
	if (Program && Program->IsComplete())
	{
		...
		NRI_CHECK(GRenderDevice.CoreInterface.CreateGraphicsPipeline(*GRenderDevice.Device, PipelineDescription, Pipeline));
...
xr_optional<FMaterialPassProxy> TiramisuDefaultMaterialRenderProxy::ResolvePass(const EMaterialPass Pass, const EVertexType VertexType) const
{
	CheckIsRenderThread();
...
const FMaterialAssetId& TiramisuDefaultMaterialRenderProxy::GetAssetReference() const
{
	CheckIsRenderThread();
	return AssetReference;
}

xr_span<const FMaterialTextureParameterBinding> TiramisuDefaultMaterialRenderProxy::GetTextureParameters() const
{
	CheckIsRenderThread();
	return {};
}
