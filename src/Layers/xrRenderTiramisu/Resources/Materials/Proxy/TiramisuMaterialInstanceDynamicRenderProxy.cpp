...
xr_optional<FMaterialPassProxy> TiramisuMaterialInstanceDynamicRenderProxy::ResolvePass(
...
{
    CheckIsRenderThread();
    const auto BaseTexture = std::ranges::find_if(TextureParameters,
        [](const FMaterialTextureParameterBinding& Binding)
        {
            return Binding.Parameter.Value ==
                LegacyBaseTextureParameterId;
        });
...
TiramisuMaterialInstanceDynamicRenderProxy::GetTextureParameters() const
{
    CheckIsRenderThread();
