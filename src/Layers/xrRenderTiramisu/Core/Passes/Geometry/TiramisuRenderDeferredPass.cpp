...
void TiramisuRenderDeferredPass::Render(nri::CommandBuffer& CurrentCommandBuffer)
{
    ...
    for (TiramisuPrimitiveSceneProxy*& SceneProxy : RenderSceneProxies)
    {
        for (u32 i = 0; i < SceneProxy->GetNumMeshBatches(); ++i)
        {
            ...
            nri::Pipeline* Pipeline = GRenderResourcesManager->MaterialPipelineRegistry->
                ResolvePipeline_RenderThread(PassProxy->Pipeline);
            if (!Pipeline)
                continue;
            GRenderDevice.CoreInterface.CmdSetPipeline(CurrentCommandBuffer, *Pipeline);

            GRenderDevice.CoreInterface.CmdSetIndexBuffer(CurrentCommandBuffer,
                *MeshBatch.IndexBuffer.IndexBuffer, MeshBatch.IndexBuffer.Offset,
                MeshBatch.IndexBuffer.IndexType);
            ...
            for (const FMeshBatchElement& Element : MeshBatch.Elements)
            {
                ...
                if (DrawIndex == INDEX_NONE)
                    break;
                GRenderDevice.CoreInterface.CmdDrawIndexed(CurrentCommandBuffer,
                    {Element.CountIndex, 1, Element.OffsetIndex,
                        Element.OffsetVertex, DrawIndex});
                if (GRender)
