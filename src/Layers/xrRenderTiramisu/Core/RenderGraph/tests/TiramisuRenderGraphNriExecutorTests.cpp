...
bool HasDiagnostic(const FNriGraphExecuteResult& Result,
    const xr_string_view Text)
{
    for (const xr_string& Diagnostic : Result.Diagnostics)
        if (Diagnostic.find(Text) != xr_string::npos) return true;
    return false;
}
    ...
    const FCompiledGraph& Graph, xr_vector<xr_string>& ExecutedPasses)
{
    ...
    {
        Callbacks.push_back({Pass.Handle,
            [&ExecutedPasses](nri::CommandBuffer&, const FCompiledPass& Executed)
            {
...
void TestAsyncExecutionAndFrameReuse()
{
    ...
    if (Test.Graph.Passes.empty()) return;
    ...
    EXECUTOR_CHECK(ExecutedPasses ==
        xr_vector<xr_string>({"Depth", "Cull", "Draw"}));
...
void TestPreflightValidationDoesNotSubmit()
{
    ...
    TiramisuNriRenderGraphExecutor Executor;
    xr_string Diagnostic;
    EXECUTOR_CHECK(Executor.Initialize(*Device, MakeCoreInterface(),
        {Graphics, nullptr, nullptr}, 1, Diagnostic));
    ...
    nri::Queue* Compute = MakeHandle<nri::Queue>();
    EXECUTOR_CHECK(Executor.Initialize(*Device, MakeCoreInterface(),
        {Graphics, Compute, nullptr}, 1, Diagnostic));
...
void TestIncompleteCoreInterfaceIsRejected()
{
    TiramisuNriRenderGraphExecutor Executor;
    xr_string Diagnostic;
    EXECUTOR_CHECK(!Executor.Initialize(*MakeHandle<nri::Device>(), {},
        {MakeHandle<nri::Queue>(), nullptr, nullptr}, 1, Diagnostic));
    EXECUTOR_CHECK(Diagnostic.find("incomplete core interface") !=
        xr_string::npos);
}
} // namespace
