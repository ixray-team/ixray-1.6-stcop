...
const FCompiledResource* FindResource(const FCompiledGraph& Graph, const FResourceHandle Handle)
{
...
const FQueueSubmission* FindSubmission(const FCompiledGraph& Graph, const xr_string_view PassName)
{
	for (const FCompiledPass& Pass : Graph.Passes)
	{
		...
		{
			if (Submission.Pass == Pass.Handle)
			{
				return &Submission;
			}
		}
...
void TestAsyncComputeFlow()
{
	...
	(void)Builder.AddPass({"DepthPrepass", EQueue::Graphics, {{Depth, EAccess::DepthStencilWrite}}});
	(void)Builder.AddPass({"BuildHiZ", EQueue::Compute, {{Depth, EAccess::ShaderRead}, {HiZ, EAccess::StorageWrite}}});
	(void)Builder.AddPass({"Cull", EQueue::Compute, {{HiZ, EAccess::StorageRead}, {Visibility, EAccess::StorageWrite}}});
	(void)Builder.AddPass({"DrawVisible", EQueue::Graphics, {{Visibility, EAccess::ShaderRead}, {BackBuffer, EAccess::ColorAttachmentWrite}}});
	(void)Builder.AddPass({"Present", EQueue::Graphics, {{BackBuffer, EAccess::Present}}});
...
void TestReadOnlyQueueTransfer()
{
	...
	(void)Builder.AddPass({"GraphicsRead", EQueue::Graphics, {{Imported, EAccess::ShaderRead}}});
	(void)Builder.AddPass({"ComputeRead", EQueue::Compute, {{Imported, EAccess::ShaderRead}}});
...
void TestTransientAliasing()
{
	...
	(void)Builder.AddPass({"WriteA", EQueue::Graphics, {{A, EAccess::ColorAttachmentWrite}, {C, EAccess::ColorAttachmentWrite}}});
	...
	(void)Builder.AddPass({"ReadBAndC", EQueue::Graphics, {{B, EAccess::ShaderRead}, {C, EAccess::ShaderRead}, {Imported, EAccess::ColorAttachmentWrite}}});
	...
	if (!CompiledA || !CompiledB || !CompiledC || !CompiledImported)
	{
		return;
	}
...
void TestValidation()
{
	...
	}
	{
		...
		const FPassHandle A = Builder.AddPass({"A", EQueue::Compute, {{Resource, EAccess::StorageWrite}}});
		const FPassHandle B = Builder.AddPass({"B", EQueue::Compute, {{Resource, EAccess::StorageRead}}});
	...
	}
	{
		...
		Builder.Reset();
		(void)Builder.AddPass({"UsesStale", EQueue::Compute, {{Stale, EAccess::StorageWrite}}});
		const FCompileResult Result = Builder.Compile();
		GRAPH_CHECK(!Result.Succeeded());
	...
	}
	{
		...
		(void)Builder.AddPass({"InvalidComputeAttachment", EQueue::Compute, {{Color, EAccess::ColorAttachmentWrite}}});
		const FCompileResult Result = Builder.Compile();
		GRAPH_CHECK(!Result.Succeeded());
...
void TestNriStateMapping()
{
	...
	GRAPH_CHECK(!ToNriAccessState(EAccess::ColorAttachmentWrite, EQueue::Compute).has_value());
...
void TestNriBarrierBatch()
{
	...
	const FNriResourceBinding Bindings[] = {
		{.Resource = Depth, .Texture = DepthTexture, .ExclusiveQueueOwnership = true},
	...
	if (!CullBatch.TextureBarriers.empty())
	{
		...
		GRAPH_CHECK(CullBatch.TextureBarriers[0].after.layout == nri::Layout::SHADER_RESOURCE);
	}
	...
	{
		GRAPH_CHECK(DrawBatch.BufferBarriers[0].after.access == nri::AccessBits::ARGUMENT_BUFFER);
		GRAPH_CHECK(DrawBatch.BufferBarriers[0].after.stages == nri::StageBits::INDIRECT);
	}
