...
bool NeedsBarrier(const EAccess Before, const EAccess After,
    const bool QueueTransfer)
{
...
xr_string_view ToString(const EAccess Access) noexcept
{
    switch (Access)
    {
    case EAccess::None: return "none";
    case EAccess::ShaderRead: return "shader_read";
    case EAccess::StorageRead: return "storage_read";
    case EAccess::StorageWrite: return "storage_write";
    case EAccess::ColorAttachmentWrite: return "color_attachment_write";
    case EAccess::DepthStencilRead: return "depth_stencil_read";
    case EAccess::DepthStencilWrite: return "depth_stencil_write";
    case EAccess::CopySource: return "copy_source";
    case EAccess::CopyDestination: return "copy_destination";
    case EAccess::IndirectArgument: return "indirect_argument";
    case EAccess::Present: return "present";
    }
    return "unknown";
}
...
FCompileResult TiramisuRenderGraphBuilder::Compile() const
{
    ...
    for (u32 PassIndex = 0; PassIndex < Passes.size(); ++PassIndex)
    {
        ...
        if (Pass.Name.empty())
            ...
            AddDiagnostic(Result, "render_graph.duplicate_pass_name",
                "Render graph contains duplicate pass name '" + Pass.Name + "'.");
        if (Pass.Uses.empty())
        ...
        for (const FResourceUse& Use : Pass.Uses)
        {
            ...
            {
                AddDiagnostic(Result, "render_graph.invalid_resource",
                    "Pass '" + Pass.Name + "' references an invalid or stale resource handle.");
                continue;
            }
            if (Use.Access == EAccess::None ||
                (!IsReadAccess(Use.Access) && !IsWriteAccess(Use.Access)))
            {
                AddDiagnostic(Result, "render_graph.invalid_access",
                    "Pass '" + Pass.Name + "' has an invalid access declaration.");
            }
            else if (!IsAccessSupportedByQueue(Use.Access, Pass.Queue))
            {
                AddDiagnostic(Result, "render_graph.queue_access_mismatch",
                    "Pass '" + Pass.Name + "' cannot use access '" +
                    xr_string(ToString(Use.Access)) + "' on the " +
                    xr_string(ToString(Pass.Queue)) + " queue.");
            }
            if (!UsedResources.insert(Use.Resource.Index).second)
            {
                AddDiagnostic(Result, "render_graph.duplicate_use",
                    "Pass '" + Pass.Name + "' declares the same resource more than once.");
            }
        }
    }
    ...
    for (const FDependency& Dependency : Dependencies)
    {
        ...
        {
            AddDiagnostic(Result, "render_graph.invalid_dependency",
                "Render graph contains an invalid or stale explicit dependency.");
            continue;
        }
    ...
    for (u32 PassIndex = 0; PassIndex < Passes.size(); ++PassIndex)
    {
        for (const FResourceUse& Use : Passes[PassIndex].Uses)
        {
            ...
            if (IsReadAccess(Use.Access))
            {
                ...
                {
                    AddDiagnostic(Result, "render_graph.read_before_write",
                        "Transient resource '" + Resources[Use.Resource.Index].Name +
    ...
    for (u32 PassIndex = 0; PassIndex < Passes.size(); ++PassIndex)
        if (Indegree[PassIndex] == 0) Ready.push(PassIndex);
    ...
    for (u32 ResourceIndex = 0; ResourceIndex < Resources.size(); ++ResourceIndex)
    {
        ...
        for (u32 PassIndex = 0; PassIndex < Passes.size(); ++PassIndex)
        {
            const auto Use = std::find_if(Passes[PassIndex].Uses.begin(), Passes[PassIndex].Uses.end(),
                [ResourceIndex](const FResourceUse& Candidate)
                {
        ...
        if (!First)
        {
            AddDiagnostic(Result, "render_graph.unused_resource",
                "Resource '" + Resources[ResourceIndex].Name + "' is never used.");
            continue;
        }
        Graph.Resources.push_back({{ResourceIndex, Generation}, *First, Last,
            FCompiledResource::NoAliasSlot});
    }
    if (!Result.Diagnostics.empty())
        return Result;
    ...
    for (const FCompiledPass& CompiledPass : Graph.Passes)
    {
        ...
        for (const FResourceUse& Use : Pass.Uses)
        {
            ...
            {
                Graph.Barriers.push_back({Use.Resource, Last.Access, Use.Access,
                    Last.Pass, CompiledPass.Handle, QueueTransfer});
            }
    ...
    for (const FCompiledPass& Pass : Graph.Passes)
    {
        ...
        auto AddWait = [&Submission](const FPassHandle SourcePass,
            const EQueue SourceQueue)
        {
            const auto Existing = std::find_if(Submission.Waits.begin(),
                Submission.Waits.end(), [SourcePass](const FQueueWait& Wait)
                {
        ...
        {
            const auto Source = std::find_if(Graph.Passes.begin(), Graph.Passes.end(),
                [Dependency](const FCompiledPass& Candidate)
                {
        ...
        for (const FBarrier& Barrier : Graph.Barriers)
        {
            if (Barrier.DestinationPass == Pass.Handle)
            {
                ...
                {
                    const auto Source = std::find_if(Graph.Passes.begin(),
                        Graph.Passes.end(), [&Barrier](const FCompiledPass& Candidate)
                        {
