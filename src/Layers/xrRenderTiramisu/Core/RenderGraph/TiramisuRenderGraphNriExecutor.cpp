...
namespace
{
const FCompiledPass* FindPass(const FCompiledGraph& Graph, const FPassHandle Handle)
{
...
{
	const auto It = std::find_if(Callbacks.begin(), Callbacks.end(), [Handle](const FNriPassCallback& Callback)
...
{
	const auto It = std::find_if(Bindings.begin(), Bindings.end(), [Handle](const FNriResourceBinding& Binding)
...
bool TiramisuNriRenderGraphExecutor::Initialize(nri::Device& InDevice, const nri::CoreInterface& CoreInterface, const FNriQueueBindings& Queues, const u32 QueuedFrameCount, xr_string& Diagnostic)
{
	if (IsInitialized())
	{
	...
	for (size_t Queue = 0; Queue < QueueCount; ++Queue)
	{
		...
		{
			if (Core.CreateCommandAllocator(*NriQueue, Frame.Queues[Queue].Allocator) != nri::Result::SUCCESS)
			{
...
)
{
	...
	for (size_t Index = 0; Index < Graph.Submissions.size(); ++Index)
	{
		...
		{
			Result.Diagnostics.push_back("Render graph requests an unavailable " + xr_string(ToString(Submission.Queue)) + " queue.");
			continue;
		}
		...
		if (!FirstSubmission[Queue])
		{
			FirstSubmission[Queue] = Index;
		}
		LastSubmission[Queue] = Index;

		BarrierBatches.push_back(BuildNriBarrierBatch(Graph, Submission, ResourceBindings, QueueBindings));
