...
xr_optional<nri::StageBits> ShaderStages(const EQueue Queue) noexcept
{
	switch (Queue)
	{
		case EQueue::Graphics:
			return nri::StageBits::GRAPHICS_SHADERS;
		case EQueue::Compute:
...
xr_optional<FNriAccessState> ToNriAccessState(const EAccess Access, const EQueue Queue) noexcept
{
	switch (Access)
	{
		case EAccess::None:
		...
		case EAccess::ShaderRead:
		{
			...
			return FNriAccessState{nri::AccessBits::SHADER_RESOURCE, nri::Layout::SHADER_RESOURCE, *Stages};
		}
		...
		case EAccess::StorageWrite:
		{
			...
			return FNriAccessState{nri::AccessBits::SHADER_RESOURCE_STORAGE, nri::Layout::SHADER_RESOURCE_STORAGE, *Stages};
		}
		case EAccess::ColorAttachmentWrite:
			if (Queue != EQueue::Graphics)
			{
				return std::nullopt;
			}
			return FNriAccessState{nri::AccessBits::COLOR_ATTACHMENT_WRITE, nri::Layout::COLOR_ATTACHMENT, nri::StageBits::COLOR_ATTACHMENT};
		case EAccess::DepthStencilRead:
			if (Queue != EQueue::Graphics)
			{
				return std::nullopt;
			}
			return FNriAccessState{nri::AccessBits::DEPTH_STENCIL_ATTACHMENT_READ, nri::Layout::DEPTH_STENCIL_READONLY, nri::StageBits::DEPTH_STENCIL_ATTACHMENT};
		case EAccess::DepthStencilWrite:
			if (Queue != EQueue::Graphics)
			{
				return std::nullopt;
			}
			return FNriAccessState{nri::AccessBits::DEPTH_STENCIL_ATTACHMENT_WRITE, nri::Layout::DEPTH_STENCIL_ATTACHMENT, nri::StageBits::DEPTH_STENCIL_ATTACHMENT};
		case EAccess::CopySource:
			return FNriAccessState{nri::AccessBits::COPY_SOURCE, nri::Layout::COPY_SOURCE, nri::StageBits::COPY};
		case EAccess::CopyDestination:
			return FNriAccessState{nri::AccessBits::COPY_DESTINATION, nri::Layout::COPY_DESTINATION, nri::StageBits::COPY};
		case EAccess::IndirectArgument:
			if (Queue != EQueue::Graphics && Queue != EQueue::Compute)
			{
				return std::nullopt;
			}
			return FNriAccessState{nri::AccessBits::ARGUMENT_BUFFER, nri::Layout::GENERAL, nri::StageBits::INDIRECT};
		case EAccess::Present:
			if (Queue != EQueue::Graphics)
			{
				return std::nullopt;
			}
			return FNriAccessState{nri::AccessBits::NONE, nri::Layout::PRESENT, nri::StageBits::NONE};
	}
	return std::nullopt;
}
...
FNriBarrierBatch BuildNriBarrierBatch(const FCompiledGraph& Graph, const FQueueSubmission& Submission, const xr_span<const FNriResourceBinding> Bindings, const FNriQueueBindings& Queues)
{
		...
		-> xr_optional<EQueue>
	{
		...
		const auto It = std::find_if(Graph.Passes.begin(), Graph.Passes.end(), [Handle](const FCompiledPass& Pass)
									 { return Pass.Handle == Handle; });
		if (It == Graph.Passes.end())
		{
			return std::nullopt;
		}
		return It->Queue;
	...
	{
		const auto Binding = std::find_if(Bindings.begin(), Bindings.end(), [&Barrier](const FNriResourceBinding& Candidate)
										  { return Candidate.Resource == Barrier.Resource; });
		if (Binding == Bindings.end())
		{
			Result.Diagnostics.push_back("Missing NRI binding for render-graph resource " + std::to_string(Barrier.Resource.Index) + ".");
			continue;
		}
		if ((Binding->Texture == nullptr) == (Binding->Buffer == nullptr))
		{
			Result.Diagnostics.push_back("Render-graph resource " + std::to_string(Barrier.Resource.Index) + " must bind exactly one NRI texture or buffer.");
			continue;
		}
		...
		if (!Before || !After)
		{
			Result.Diagnostics.push_back("Unsupported NRI access/queue mapping for resource " + std::to_string(Barrier.Resource.Index) + ".");
			continue;
		}
