...
[[nodiscard]] FVector AddScaled(const FVector& A, const FVector& B, const float Scale)
{
...
[[nodiscard]] FVector TransformPosition(const xr_array<float, 16>& Matrix, const FVector& Position)
{
	return {
...
[[nodiscard]] bool IntersectTriangle(const FVector& Origin, const FVector& Direction, const FVector& A, const FVector& B, const FVector& C, const bool CullBackFaces, float& OutDistance, FVector& OutNormal)
{
	...
	for (const FEditorStaticMeshInstance& Instance : Instances)
	{
		...
		for (const FEditorStaticMeshSection& Section : Mesh.Sections)
		{
				 ...
				 Index += 3)
			{
				...
				const FVector A = TransformPosition(Instance.LocalToWorld, Mesh.Vertices[I0].Position);
				const FVector B = TransformPosition(Instance.LocalToWorld, Mesh.Vertices[I1].Position);
				const FVector C = TransformPosition(Instance.LocalToWorld, Mesh.Vertices[I2].Position);
				float Distance = 0.0f;
				FVector Normal;
				if (!IntersectTriangle(Request.RayOrigin, Direction, A, B, C, Request.CullBackFaces, Distance, Normal) || Distance >= Closest)
				{
					continue;
				}
				Closest = Distance;
