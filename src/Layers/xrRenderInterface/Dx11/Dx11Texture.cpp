...
HRESULT CD3D11Texture3D::Create(const TextureDesc* pTextureDesc, LPSUBRESOURCE_DATA pSubresourceData)
{
	R_ASSERT(pTextureDesc);
...
bool CD3D11Texture3D::LockRect(u32 Level, LOCKED_RECT* pLockedRect, const Irect* pRect, eLockType Flags)
{
	ID3D11DeviceContext* pImmediateContext = (ID3D11DeviceContext*)HWRenderContext;
	R_ASSERT(pImmediateContext);
...
bool CD3D11Texture3D::UnlockRect(u32 Level)
{
	ID3D11DeviceContext* pImmediateContext = (ID3D11DeviceContext*)HWRenderContext;
	R_ASSERT(pImmediateContext);
...
void CD3D11Texture3D::SetStage(u32 Stage)
{
	ID3D11DeviceContext* pImmediateContext = (ID3D11DeviceContext*)HWRenderContext;
	R_ASSERT(pImmediateContext);
...
Ivector2 CD3D11Texture3D::GetTextureSize() const
{
	return Ivector2(m_TextureDesc.Width, m_TextureDesc.Height);
}

EResourceType CD3D11Texture3D::GetType()
{
	return eResourceTexture2D;
}

u32 CD3D11Texture3D::GetLevelCount()
{
	return m_TextureDesc.NumMips;
}

bool CD3D11Texture3D::GetSurfaceLevel(u32 Level, LPIRHISURFACE* ppSurfaceLevel)
{
	return false;
}

void CD3D11Texture3D::GetAPIData(SRHIAPIData* pAPIData)
{
	R_ASSERT(pAPIData);
	pAPIData->pSRV = m_pTextureSRV;
	pAPIData->pUAV = nullptr;
...
void CD3D11Texture3D::GetDesc(TextureDesc* pTextureDesc)
{
	R_ASSERT(pTextureDesc);
	*pTextureDesc = m_TextureDesc;
}
