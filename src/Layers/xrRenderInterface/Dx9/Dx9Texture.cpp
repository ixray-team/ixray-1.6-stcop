...
///////////////////////////////////////////////////////////
// CD3D9VolumeTexture implmenetatation

CD3D9VolumeTexture::CD3D9VolumeTexture() :
	m_pVolumeTexture(nullptr)
{
}
...
HRESULT CD3D9VolumeTexture::Create(const TextureDesc* pTextureDesc, LPSUBRESOURCE_DATA pSubresourceData)
{
	IDirect3DDevice9* pDevice = (IDirect3DDevice9*)HWRenderDevice;
	R_ASSERT(pDevice);
	...
	HRESULT hr = pDevice->CreateVolumeTexture(
		pTextureDesc->Width,
		pTextureDesc->Height,
		pTextureDesc->DepthOrSliceNum,
		pTextureDesc->NumMips,
		pTextureDesc->Usage,
		Format,
		pTextureDesc->DefaultPool ? D3DPOOL_DEFAULT : D3DPOOL_MANAGED,
	...
	if (FAILED(hr))
	{
		Msg("! CD3D9VolumeTexture::Create: Failed to create texture. DirectX Error: %s", Debug.dxerror2string(hr));
		return hr;
	}
...
bool CD3D9VolumeTexture::LockBox(u32 Level, LOCKED_BOX* pLockedVolume, const RHIBOX* pBox, u32 Flags)
{
	...
	*pLockedVolume = *(LOCKED_BOX*)&lockedBox;

	return true;
}

bool CD3D9VolumeTexture::UnlockBox(u32 Level)
{
	HRESULT hr = m_pVolumeTexture->UnlockBox(Level);
	if (FAILED(hr))
	{
		Msg("! CD3D9VolumeTexture::UnlockBox: Failed to unlock volume texture. DirectX Error: %s", Debug.dxerror2string(hr));
		return false;
	}
