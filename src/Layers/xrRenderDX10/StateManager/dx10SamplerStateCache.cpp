...
dx10SamplerStateCache::SHandle dx10SamplerStateCache::GetState(RHISampleDesc& desc )
{
	SHandle	hResult;
...
) const
{
	...
	ZeroMemory(pSS, sizeof(pSS[0])* RHI_COMMONSHADER_SAMPLER_SLOT_COUNT);
	...
	uiMin = 0;
	uiMax = RHI_COMMONSHADER_SAMPLER_SLOT_COUNT -1;
}
