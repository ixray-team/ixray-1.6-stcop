...
void CTexture::surface_set(IRHITexture* surf)
{
	if (surf)			surf->AddRef();
	_RELEASE(pSurface);
	_RELEASE(m_pSRView);
...
IRHITexture* CTexture::surface_get()
{
...
{
	R_ASSERT2(0, "Please implement me someday");
#if 0
	VERIFY(pSurface);
	VERIFY(flags.bLoadedAsStaging);

	IRHITexture* pTargetSurface = 0;
	...
	g_RenderRHI->CopyResource(pTargetSurface, pSurface);
...
void CTexture::apply_theora(u32 dwStage)
{
	if (pTheora->Update(m_play_time != 0xFFFFFFFF ? m_play_time : Device.dwTimeContinual))
	{
		...
		pSurface->LockRect(0, &R, &rect, eLOCK_DISCARD);
...
void CTexture::apply_avi(u32 dwStage)
{
	if (pAVI->NeedUpdate())
	{
		// AVI
		LOCKED_RECT R{};
		pSurface->LockRect(0, &R, nullptr, eLOCK_DISCARD);
