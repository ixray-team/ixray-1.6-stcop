...
public:
	...
	void	SetTexture(eVolumePrivateRT id, IRHITexture*pT) { pT->AddRef(); m_pRTTextures[id]->Release(); m_pRTTextures[id] = pT;}
	void	SetView(eVolumePrivateRT id, IRHISurface*pV) { pV->AddRef(); m_pRenderTargetViews[id]->Release(); m_pRenderTargetViews[id] = pV;}

	IRHITexture*			GetTexture(eVolumePrivateRT id) const { m_pRTTextures[id]->AddRef(); return m_pRTTextures[id];}
	IRHISurface*		GetView(eVolumePrivateRT id) const { m_pRenderTargetViews[id]->AddRef(); return m_pRenderTargetViews[id];}
...
private:
	...
	void	CreateRTTextureAndViews(int rtIndex, TextureDesc& TexDesc);
...
private:
	...
	IRHISurface				*m_pRenderTargetViews[ VP_NUM_TARGETS ];
	IRHITexture				*m_pRTTextures[ VP_NUM_TARGETS ];
};
