...
void dx103DFluidGrid::CreateVertexBuffers()
{
	...
	R_CHK(RHIUtils::CreateVertexBuffer(&m_pRenderQuadBuffer, renderQuad, vSize*m_iNumVerticesRenderQuad));
	...
	R_CHK(RHIUtils::CreateVertexBuffer(&m_pSlicesBuffer, slices, vSize*m_iNumVerticesSlices));
	...
	R_CHK(RHIUtils::CreateVertexBuffer(&m_pBoundarySlicesBuffer, boundarySlices, vSize*m_iNumVerticesBoundarySlices));
	...
	R_CHK(RHIUtils::CreateVertexBuffer(&m_pBoundaryLinesBuffer, boundaryLines, vSize*m_iNumVerticesBoundaryLines));
...
void dx103DFluidGrid::DestroyVertexBuffers() {
	delete m_pRenderQuadBuffer;
	delete m_pSlicesBuffer;
	delete m_pBoundarySlicesBuffer;
	delete m_pBoundaryLinesBuffer;
}
