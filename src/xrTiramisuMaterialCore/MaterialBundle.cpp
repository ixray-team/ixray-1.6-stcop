...
{
public:
	explicit FBinaryReader(const xr_span<const u8> InData)
		: Data(InData) {}
...
FMaterialBundleWriteResult SerializeMaterialBundle(const FMaterialBundle& Bundle)
{
	...
	try
	{
		...
		std::ranges::sort(Records, [](const FMaterialBundleRecord& Left, const FMaterialBundleRecord& Right)
		...
		std::ranges::sort(Blobs, [](const FMaterialShaderBlob& Left, const FMaterialShaderBlob& Right)
						  { return std::tie(Left.MaterialId.Value, Left.PipelineKey, Left.Format, Left.Pass, Left.Stage, Left.EntryPoint, Left.VertexFactory, Left.RenderPassSignature) <
								   std::tie(Right.MaterialId.Value, Right.PipelineKey, Right.Format, Right.Pass, Right.Stage, Right.EntryPoint, Right.VertexFactory, Right.RenderPassSignature); });
