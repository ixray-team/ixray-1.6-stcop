    ...
    FMaterialBundle InBundle, const FMaterialShaderLibraryBuildOptions& Options)
{
    ...
    {
        AddDiagnostic(Result.Diagnostics, EMaterialDiagnosticSeverity::Error, "shader_library.unsupported_version",
            "Material shader library requires bundle version " + std::to_string(MaterialBundleVersion) + ".");
    }
    if (Options.RequireCompleteShaderSet && !Library.Bundle.CompleteShaderSet)
    {
        AddDiagnostic(Result.Diagnostics, EMaterialDiagnosticSeverity::Error, "shader_library.incomplete_bundle",
            "Cooked material runtime requires a bundle marked CompleteShaderSet.");
    }
    ...
    for (const FMaterialBundleRecord& Record : Library.Bundle.Records)
    {
        ...
        {
            AddDiagnostic(Result.Diagnostics, EMaterialDiagnosticSeverity::Error, "shader_library.invalid_record_id",
                "Material bundle record has no stable asset id.");
            continue;
        }
        ...
        if (Record.Type == EMaterialBundleRecordType::FlattenedInstance)
        {
            ...
            if (!MasterId.IsValid())
            {
                AddDiagnostic(Result.Diagnostics, EMaterialDiagnosticSeverity::Error,
                    "shader_library.invalid_instance_master",
                    "Flattened material instance '" + Record.Id.Value + "' has no master id.");
                continue;
            }
        }
        ...
        {
            AddDiagnostic(Result.Diagnostics, EMaterialDiagnosticSeverity::Error, "shader_library.conflicting_alias",
                "Material asset '" + Record.Id.Value + "' resolves to more than one master material.");
        }
    }
    ...
    for (const auto& [AssetId, MasterId] : Library.MasterAliases)
    {
        ...
        {
            AddDiagnostic(Result.Diagnostics, EMaterialDiagnosticSeverity::Error, "shader_library.missing_master",
                "Material asset '" + AssetId.Value + "' references missing master '" + MasterId.Value + "'.");
        }
        else if (Master->second != MasterId)
        {
            AddDiagnostic(Result.Diagnostics, EMaterialDiagnosticSeverity::Error, "shader_library.non_flattened_instance",
                "Material instance '" + AssetId.Value + "' does not contain a flattened master reference.");
        }
    }
    ...
    for (const FMaterialBundleRecord& Record : Library.Bundle.Records)
    {
        ...
        {
            AddDiagnostic(Result.Diagnostics, EMaterialDiagnosticSeverity::Error,
                "shader_library.master_record_mismatch",
                "Master bundle record '" + Record.Id.Value +
                "' does not match its serialized asset identity.");
            continue;
        }
    ...
    for (const FMaterialBundleRecord& Record : Library.Bundle.Records)
    {
        ...
        {
            AddDiagnostic(Result.Diagnostics, EMaterialDiagnosticSeverity::Error,
                "shader_library.instance_record_mismatch",
                "Flattened instance record '" + Record.Id.Value +
                "' does not match its serialized asset identity or master.");
            continue;
        }
        FMaterialRegistrationResult Registered =
    ...
    for (const FMaterialBundleRecord& Record : Library.Bundle.Records)
    {
        ...
        {
            AddDiagnostic(Result.Diagnostics, EMaterialDiagnosticSeverity::Error,
                "shader_library.resolved_master_mismatch",
                "Material '" + Record.Id.Value + "' resolves to master '" +
                Resolved.Value.MasterId.Value + "' instead of bundle master '" +
                Record.MasterId.Value + "'.");
            continue;
        }
    ...
    for (size_t BlobIndex = 0; BlobIndex < Library.Bundle.ShaderBlobs.size(); ++BlobIndex)
    {
        ...
        {
            AddDiagnostic(Result.Diagnostics, EMaterialDiagnosticSeverity::Error, "shader_library.invalid_blob",
                "Selected backend contains a shader blob with incomplete metadata or bytecode.");
            continue;
        }
        if (!FindMaterialPassDefinition(Blob.Pass))
        {
            AddDiagnostic(Result.Diagnostics, EMaterialDiagnosticSeverity::Error, "shader_library.invalid_pass",
                "Selected backend contains a shader blob with an unknown material pass.");
            continue;
        }
        if (Blob.Stage != EMaterialShaderStage::Vertex && Blob.Stage != EMaterialShaderStage::Pixel)
        {
            AddDiagnostic(Result.Diagnostics, EMaterialDiagnosticSeverity::Error, "shader_library.invalid_stage",
                "Selected backend contains a shader blob with an unknown shader stage.");
            continue;
        }
        if (!Library.MasterAliases.contains(Blob.MaterialId))
        {
            AddDiagnostic(Result.Diagnostics, EMaterialDiagnosticSeverity::Error, "shader_library.missing_material_record",
                "Shader blob references material '" + Blob.MaterialId.Value + "' without a bundle record.");
            continue;
        }
        ...
        {
            AddDiagnostic(Result.Diagnostics, EMaterialDiagnosticSeverity::Error, "shader_library.duplicate_stage",
                "Material shader program contains more than one " + xr_string(StageName(Blob.Stage)) + " stage.");
            continue;
        }
    ...
    {
        AddDiagnostic(Result.Diagnostics, EMaterialDiagnosticSeverity::Error, "shader_library.missing_backend",
            "Material bundle contains no shader programs for the selected backend.");
    }
    ...
    for (const auto& [Key, Stages] : Library.Programs)
    {
        ...
        const EMaterialDiagnosticSeverity Severity = Library.Bundle.CompleteShaderSet ?
            EMaterialDiagnosticSeverity::Error : EMaterialDiagnosticSeverity::Warning;
        AddDiagnostic(Result.Diagnostics, Severity, "shader_library.missing_stage",
            "Material '" + Key.MaterialId.Value + "' program " + xr_string(std::to_string(Key.PipelineKey)) +
            " does not contain both vertex and pixel stages.");
    }
    ...
    const xr_span<const u8> Data, const FMaterialShaderLibraryBuildOptions& Options)
{
    ...
    Result.Diagnostics.insert(Result.Diagnostics.end(),
        std::make_move_iterator(Built.Diagnostics.begin()), std::make_move_iterator(Built.Diagnostics.end()));
...
const FMaterialAsset* TiramisuMaterialShaderLibrary::ResolveMasterMaterial(
    const FMaterialAssetId& MaterialId) const noexcept
{
    const auto Alias = MasterAliases.find(MaterialId);
    const FMaterialAssetId& MasterId =
        Alias != MasterAliases.end() ? Alias->second : MaterialId;
    const auto Material = MasterMaterials.find(MasterId);
    return Material != MasterMaterials.end() ? &Material->second : nullptr;
}

const FResolvedMaterialInstance* TiramisuMaterialShaderLibrary::ResolveMaterial(
    const FMaterialAssetId& MaterialId) const noexcept
{
    ...
    const xr_string_view VertexFactory, const xr_string_view RenderPassSignature) const
{
    ...
    if (!Material || !Definition || Definition->VertexFactory != VertexFactory ||
        Definition->RenderPassSignature != RenderPassSignature)
    {
        return std::nullopt;
    }
    ...
    const xr_string_view Backend = Format == EMaterialShaderBlobFormat::Dxil
        ? "d3d12" : "vulkan";
    const FMaterialPipelineKey Key =
        MakeCookedMaterialPipelineKey(*Material, *Definition, Backend);
    return Find(MaterialId, Key.StableHash(), Pass, VertexFactory,
        RenderPassSignature);
}
