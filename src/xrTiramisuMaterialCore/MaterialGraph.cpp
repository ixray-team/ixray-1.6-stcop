...
void AddDiagnostic(xr_vector<FMaterialDiagnostic>& Diagnostics, const xr_string_view Code,
    const xr_string& Message, const FMaterialNodeId Node = {}, const FMaterialPinId Pin = {},
    const EMaterialDiagnosticSeverity Severity = EMaterialDiagnosticSeverity::Error)
{
...
xr_string ReadGraphStringField(const Json& Object, const xr_string_view Name,
    xr_vector<FMaterialDiagnostic>& Diagnostics, const FMaterialNodeId Node = {})
{
...
u32 ReadGraphUIntField(const Json& Object, const xr_string_view Name, const u32 Default,
    xr_vector<FMaterialDiagnostic>& Diagnostics, const FMaterialNodeId Node = {})
{
    const Json* Field = MaterialJsonDetail::Find(Object, Name);
    if (!Field)
...
const Json& ReadGraphArrayField(const Json& Object, const xr_string_view Name, const Json& Empty,
    xr_vector<FMaterialDiagnostic>& Diagnostics, const FMaterialNodeId Node = {})
{
    const Json* Field = MaterialJsonDetail::Find(Object, Name);
    if (!Field)
...
const FMaterialGraphPin* FindPin(const FMaterialGraphNode& Node, const xr_string_view Name,
    const EMaterialPinDirection Direction)
{
    const auto Iterator = std::ranges::find_if(Node.Pins,
        [Name, Direction](const FMaterialGraphPin& Pin)
...
public:
    ...
    FMaterialGraphCompileResult Run()
    {
        ...
        for (const FOutputField& Field : Fields)
        {
            ...
            {
                AddDiagnostic(Result.Diagnostics, "graph.output_type_mismatch",
                    "Material output '" + xr_string(Field.Pin) + "' expects " + xr_string(ToString(Field.Type)) +
                        ", got " + xr_string(ToString(Expression->Type)) + ".",
                    Output->Id, Input->Id);
                continue;
            }
        ...
        std::ranges::sort(Result.UsedParameters,
            [](const FMaterialParameterId& Left, const FMaterialParameterId& Right) { return Left.Value < Right.Value; });
...
private:
    void ValidateAndIndex()
    {
        if (Graph.Version != MaterialGraphVersion)
            AddDiagnostic(Result.Diagnostics, "graph.unsupported_version",
                "Unsupported material graph version " + std::to_string(Graph.Version) + ".");
        ...
        for (const FMaterialGraphLink& Link : Graph.Links)
        {
            ...
            {
                AddDiagnostic(Result.Diagnostics, "graph.invalid_link_direction", "Links must connect output pins to input pins.",
                    To->second.Node->Id, To->second.Pin->Id);
                continue;
            }
            if (!TypesCompatible(From->second.Pin->Type, To->second.Pin->Type))
            {
                AddDiagnostic(Result.Diagnostics, "graph.link_type_mismatch",
                    "Cannot connect " + xr_string(ToString(From->second.Pin->Type)) + " to " +
                        xr_string(ToString(To->second.Pin->Type)) + ".",
                    To->second.Node->Id, To->second.Pin->Id);
                continue;
            }
            if (!InputLinks.emplace(Link.ToPin.Value, Link.FromPin.Value).second)
                AddDiagnostic(Result.Diagnostics, "graph.multiple_input_links", "An input pin accepts only one link.",
                    To->second.Node->Id, To->second.Pin->Id);
        }
    }
    ...
    xr_optional<FExpression> CompileInput(const FMaterialGraphNode& Node, const xr_string_view Name)
    {
        ...
        if (!Pin)
        {
            AddDiagnostic(Result.Diagnostics, "graph.missing_pin", "Node '" + Node.Type + "' is missing input pin '" +
                xr_string(Name) + "'.", Node.Id);
            return std::nullopt;
        }
        const auto Link = InputLinks.find(Pin->Id.Value);
        if (Link == InputLinks.end())
        {
            AddDiagnostic(Result.Diagnostics, "graph.unconnected_input", "Required input pin '" + xr_string(Name) +
                "' is not connected.", Node.Id, Pin->Id);
            return std::nullopt;
        }
    ...
    xr_optional<FExpression> CompileOutput(const xr_string& PinId)
    {
        ...
        if (!ActivePins.emplace(PinId).second)
        {
            ...
            AddDiagnostic(Result.Diagnostics, "graph.cycle", "Material expression graph contains a cycle.",
                Owner.Node->Id, Owner.Pin->Id);
            return std::nullopt;
        }
    ...
    xr_optional<FExpression> CompileNodeOutput(const FMaterialGraphNode& Node, const FMaterialGraphPin& Output)
    {
        ...
        if (Node.Type == "texcoord0") return FExpression{Output.Type, "Context.TexCoord0", std::nullopt};
        if (Node.Type == "vertex_color") return FExpression{Output.Type, "Context.VertexColor", std::nullopt};
        if (Node.Type == "vertex_normal") return FExpression{Output.Type, "Context.WorldNormal", std::nullopt};
        if (Node.Type == "world_position") return FExpression{Output.Type, "Context.WorldPosition", std::nullopt};
        if (Node.Type == "camera_position") return FExpression{Output.Type, "Context.CameraPosition", std::nullopt};
        if (Node.Type == "time") return FExpression{Output.Type, "Context.Time", std::nullopt};
        ...
        if (Node.Type == "add" || Node.Type == "subtract" || Node.Type == "multiply" || Node.Type == "divide")
        {
            ...
            if (!A || !B)
                return std::nullopt;
            const char* Operator = Node.Type == "add" ? "+" : Node.Type == "subtract" ? "-" : Node.Type == "multiply" ? "*" : "/";
            if (const auto* Left = A->Constant ? std::get_if<float>(&*A->Constant) : nullptr)
            {
                if (const auto* Right = B->Constant ? std::get_if<float>(&*B->Constant) : nullptr)
                {
                    ...
                    const float Folded = Node.Type == "add" ? *Left + *Right : Node.Type == "subtract" ? *Left - *Right :
                        Node.Type == "multiply" ? *Left * *Right : *Left / *Right;
        ...
        if (Node.Type == "lerp")
        {
            ...
            if (!A || !B || !Alpha) return std::nullopt;
        ...
        if (Node.Type == "fresnel")
        {
            ...
            if (!Normal || !Exponent) return std::nullopt;
            return FExpression{Output.Type,
                "pow(saturate(1.0f - dot(normalize(" + Normal->Code + "), normalize(Context.CameraVector))), " + Exponent->Code + ")",
                std::nullopt};
        }
        ...
        if (Node.Type == "texture_sample")
        {
            ...
            else
            {
                ...
                const xr_string* Id = IdValue == Node.Properties.end()
                    ? nullptr : std::get_if<xr_string>(&IdValue->second);
                if (!Id || Id->empty())
                {
                    Texture = CompileInput(Node, "Texture");
                }
                else
                {
                    ...
                    if (Parameter == Parameters.end())
                    {
                        AddDiagnostic(Result.Diagnostics, "graph.unknown_texture_parameter",
                            "Texture parameter '" + *Id + "' is not declared by the master material.",
                            Node.Id, TexturePin ? TexturePin->Id : FMaterialPinId{});
                        return std::nullopt;
                    }
                    if (Parameter->second->Type != EMaterialParameterType::Texture2D)
                    {
                        AddDiagnostic(Result.Diagnostics, "graph.texture_parameter_type",
                            "texture_sample requires a texture2d parameter.", Node.Id,
                            TexturePin ? TexturePin->Id : FMaterialPinId{});
                        return std::nullopt;
                    }
                    const FMaterialParameterId ParameterId{*Id};
                    Result.UsedParameters.push_back(ParameterId);
                    Texture = FExpression{EMaterialValueType::Texture2D,
                        "Parameters." + ParameterFieldName(ParameterId), std::nullopt};
                }
            }
            const auto Coordinates = CompileInput(Node, "UV");
            if (!Texture || !Coordinates) return std::nullopt;
            ...
            if (Output.Name == "R") Code += ".r";
            else if (Output.Name == "G") Code += ".g";
            else if (Output.Name == "B") Code += ".b";
            else if (Output.Name == "A") Code += ".a";
            else if (Output.Name == "RGB") Code += ".rgb";
        ...
        if (Node.Type == "custom_hlsl")
        {
            ...
            for (const xr_string_view Token : Forbidden)
            {
                ...
                {
                    AddDiagnostic(Result.Diagnostics, "graph.custom_hlsl_forbidden_token",
                        "Custom HLSL expression contains forbidden token '" + xr_string(Token) + "'.", Node.Id, Output.Id);
                    return std::nullopt;
                }
            }
            for (const FMaterialGraphPin& Pin : Node.Pins)
...
FMaterialGraphParseResult ParseMaterialGraphJson(const xr_string_view JsonText)
{
    ...
    try
    {
        ...
        {
            AddDiagnostic(Result.Diagnostics, "graph.invalid_json",
                "Material graph contains invalid JSON.");
            return Result;
        }
        if (!Root.is_object())
        {
        ...
        for (const Json& NodeJson : *Nodes)
        {
            ...
            if (const Json* Position = MaterialJsonDetail::Find(NodeJson, "position"))
            {
                ...
                if (!Position->is_array() || Position->size() != 2 ||
                    ...
                    AddDiagnostic(Result.Diagnostics, "graph.invalid_position",
                        "Node position must be an array of two numeric values.", Node.Id);
                else
            ...
            for (const Json& PinJson : ReadGraphArrayField(NodeJson, "pins", EmptyArray, Result.Diagnostics, Node.Id))
            {
                ...
                else
                {
                    ...
                    if (!Direction.empty() && Direction != "input")
                        AddDiagnostic(Result.Diagnostics, "graph.invalid_pin_direction",
                            "Pin direction must be 'input' or 'output'.", Node.Id, Pin.Id);
                }
            ...
            if (const Json* Properties = MaterialJsonDetail::Find(NodeJson, "properties"))
            {
                if (!Properties->is_object())
                    AddDiagnostic(Result.Diagnostics, "graph.invalid_field_type",
                        "Field 'properties' must be an object.", Node.Id);
                else for (const auto& [Name, ValueJson] : Properties->items())
                {
...
xr_string SerializeMaterialGraphJson(const FMaterialGraph& Graph)
{
    ...
    for (const FMaterialGraphNode& Node : Graph.Nodes)
    {
        ...
        for (const FMaterialGraphPin& Pin : Node.Pins)
        {
            NodeJson["pins"].push_back({{"guid", Pin.Id.Value}, {"name", Pin.Name},
                {"direction", Pin.Direction == EMaterialPinDirection::Output ? "output" : "input"},
