...
u32 ParameterSize(const EMaterialParameterType Type)
{
    ...
    case EMaterialParameterType::SamplerPreset: return 4;
    case EMaterialParameterType::Float2: return 8;
    case EMaterialParameterType::Float3: return 12;
    case EMaterialParameterType::Float4:
    case EMaterialParameterType::Color: return 16;
    case EMaterialParameterType::StaticBool:
    case EMaterialParameterType::StaticEnum: return 0;
    }
    return 0;
}
...
xr_string_view HlslType(const EMaterialParameterType Type)
{
    switch (Type)
    {
    case EMaterialParameterType::Scalar: return "float";
    case EMaterialParameterType::Float2: return "float2";
    case EMaterialParameterType::Float3: return "float3";
    case EMaterialParameterType::Float4:
    case EMaterialParameterType::Color: return "float4";
    case EMaterialParameterType::Texture2D:
    case EMaterialParameterType::TextureCube:
    case EMaterialParameterType::SamplerPreset: return "uint";
    case EMaterialParameterType::StaticBool:
    case EMaterialParameterType::StaticEnum: return {};
    }
    return {};
}
...
bool WriteParameterValue(FMaterialPackedParameterBlock& Block, const FMaterialParameterLayoutField& Field,
    const FMaterialValue& Value, xr_vector<FMaterialDiagnostic>& Diagnostics)
{
    if (!ValueMatchesParameterType(Value, Field.Type))
    {
        AddError(Diagnostics, "parameter_pack.type_mismatch",
            "Parameter '" + Field.Id.Value + "' does not match its material layout type.");
        return false;
    }

    switch (Field.Type)
    {
    case EMaterialParameterType::Scalar: WriteValue(Block.Data, Field.Offset, std::get<float>(Value)); break;
    case EMaterialParameterType::Float2: WriteValue(Block.Data, Field.Offset, std::get<FFloat2>(Value)); break;
    case EMaterialParameterType::Float3: WriteValue(Block.Data, Field.Offset, std::get<FFloat3>(Value)); break;
    case EMaterialParameterType::Float4:
    case EMaterialParameterType::Color: WriteValue(Block.Data, Field.Offset, std::get<FFloat4>(Value)); break;
    case EMaterialParameterType::Texture2D:
    case EMaterialParameterType::TextureCube:
    case EMaterialParameterType::SamplerPreset:
    {
        ...
        if (AssetPath.empty())
        {
            AddError(Diagnostics, "parameter_pack.empty_resource",
                "Resource parameter '" + Field.Id.Value + "' has an empty asset path.");
            return false;
        }
    ...
    const xr_span<const FMaterialParameterDefinition> Definitions)
{
    ...
    for (const FMaterialParameterDefinition& Definition : Definitions)
    {
        ...
        if (!UniqueIds.insert(Definition.Id).second)
            AddError(Result.Diagnostics, "parameter_layout.duplicate_id",
                "Material parameter id '" + Definition.Id.Value + "' is duplicated.");
        if (!ValueMatchesParameterType(Definition.DefaultValue, Definition.Type))
            AddError(Result.Diagnostics, "parameter_layout.invalid_default",
                "Default value for parameter '" + Definition.Id.Value + "' has the wrong type.");
        if (!Definition.IsStatic())
        {
            if (!UniqueHlslNames.insert(HlslFieldName(Definition.Id)).second)
                AddError(Result.Diagnostics, "parameter_layout.hlsl_name_collision",
                    "Parameter '" + Definition.Id.Value + "' collides with another generated HLSL field name.");
    ...
    std::ranges::sort(Sorted, {}, [](const FMaterialParameterDefinition* Definition) { return Definition->Id; });
    u32 Offset = 0;
    for (const FMaterialParameterDefinition* Definition : Sorted)
    {
        ...
        if (Size == 0)
        {
            AddError(Result.Diagnostics, "parameter_layout.unsupported_type",
                "Runtime parameter '" + Definition->Id.Value + "' has an unsupported GPU type.");
            continue;
        }
    ...
    Result.Value.ByteSize = std::max(MaterialParameterBlockAlignment,
        AlignUp(Offset, MaterialParameterBlockAlignment));
    Result.Value.StableHash = LayoutHash(Result.Value);
    return Result;
}

FMaterialParameterPackResult PackMaterialParameters(const FMaterialParameterLayout& Layout,
    const xr_span<const FMaterialParameterDefinition> Definitions, const FMaterialParameterMap& Values)
{
    ...
    for (const FMaterialParameterLayoutField& Field : Layout.Fields)
    {
        ...
        {
            AddError(Result.Diagnostics, "parameter_pack.layout_mismatch",
                "Material layout field '" + Field.Id.Value + "' does not match its parameter definition.");
            continue;
        }
    ...
    const FMaterialResourceReferenceIndexResolver& Resolver)
{
    ...
    for (const FMaterialParameterResourceReference& Reference : Source.Resources)
    {
        ...
        {
            AddError(Result.Diagnostics, "parameter_patch.invalid_offset",
                "Resource parameter '" + Reference.Parameter.Value + "' points outside the packed block.");
            continue;
        }
        const xr_optional<FDescriptorHeapIndex> Index = Resolver(Reference);
        if (!Index || !Index->IsValid())
        {
            AddError(Result.Diagnostics, "parameter_patch.unresolved_resource",
                "Resource parameter '" + Reference.Parameter.Value + "' could not be resolved.");
            continue;
        }
