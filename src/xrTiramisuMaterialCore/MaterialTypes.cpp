...
TiramisuMaterialInstanceDynamic::TiramisuMaterialInstanceDynamic(FMaterialAssetId InMaster,
    FMaterialParameterMap InRuntimeParameters, FMaterialStaticParameterSet InStaticParameters)
    : Master(std::move(InMaster)), RuntimeParameters(std::move(InRuntimeParameters)),
      StaticParameters(std::move(InStaticParameters))
{
}
...
static xr_string FormatGuid(
    xr_array<u8, 16> Bytes, const u8 Version)
{
...
xr_string GenerateDeterministicMaterialGuid(
    const xr_string_view Namespace, const xr_string_view Name)
{
    u64 First = FnvOffset;
...
bool ValueMatchesParameterType(const FMaterialValue& Value, const EMaterialParameterType Type) noexcept
{
    switch (Type)
    {
    case EMaterialParameterType::Scalar: return std::holds_alternative<float>(Value);
    case EMaterialParameterType::Float2: return std::holds_alternative<FFloat2>(Value);
    case EMaterialParameterType::Float3: return std::holds_alternative<FFloat3>(Value);
    case EMaterialParameterType::Float4:
    case EMaterialParameterType::Color: return std::holds_alternative<FFloat4>(Value);
    case EMaterialParameterType::Texture2D:
    case EMaterialParameterType::TextureCube:
    case EMaterialParameterType::SamplerPreset: return std::holds_alternative<xr_string>(Value);
    case EMaterialParameterType::StaticBool: return std::holds_alternative<bool>(Value);
    case EMaterialParameterType::StaticEnum: return std::holds_alternative<s32>(Value);
    }
    return false;
}
...
EMaterialValueType ToValueType(const EMaterialParameterType Type) noexcept
{
    switch (Type)
    {
    case EMaterialParameterType::Scalar: return EMaterialValueType::Float1;
    case EMaterialParameterType::Float2: return EMaterialValueType::Float2;
    case EMaterialParameterType::Float3: return EMaterialValueType::Float3;
    case EMaterialParameterType::Float4:
    case EMaterialParameterType::Color: return EMaterialValueType::Float4;
    case EMaterialParameterType::Texture2D: return EMaterialValueType::Texture2D;
    case EMaterialParameterType::TextureCube: return EMaterialValueType::TextureCube;
    case EMaterialParameterType::SamplerPreset: return EMaterialValueType::Sampler;
    case EMaterialParameterType::StaticBool: return EMaterialValueType::Bool;
    case EMaterialParameterType::StaticEnum: return EMaterialValueType::Integer;
    }
    return EMaterialValueType::Invalid;
}

FMaterialPermutationStatistics CalculateMaterialPermutationStatistics(
    const xr_span<const FMaterialParameterDefinition> Parameters) noexcept
{
...
xr_string_view ToString(const EMaterialDomain Value) noexcept
{
    ...
    case EMaterialDomain::PostProcess: return "post_process";
    }
    return "surface";
}
...
xr_string_view ToString(const EMaterialBlendMode Value) noexcept
{
    ...
    case EMaterialBlendMode::Modulate: return "modulate";
    }
    return "opaque";
}
...
xr_string_view ToString(const EMaterialShadingModel Value) noexcept
{
    switch (Value)
    {
    case EMaterialShadingModel::DefaultLit: return "default_lit";
    case EMaterialShadingModel::Unlit: return "unlit";
    case EMaterialShadingModel::Foliage: return "foliage";
    case EMaterialShadingModel::Hair: return "hair";
    }
    return "default_lit";
}
...
xr_string_view ToString(const EMaterialParameterType Value) noexcept
{
    ...
    case EMaterialParameterType::Float2: return "float2";
    case EMaterialParameterType::Float3: return "float3";
    case EMaterialParameterType::Float4: return "float4";
    case EMaterialParameterType::Color: return "color";
    case EMaterialParameterType::Texture2D: return "texture2d";
    case EMaterialParameterType::TextureCube: return "texture_cube";
    case EMaterialParameterType::SamplerPreset: return "sampler_preset";
    case EMaterialParameterType::StaticBool: return "static_bool";
    case EMaterialParameterType::StaticEnum: return "static_enum";
    }
    return "scalar";
}
...
xr_string_view ToString(const EMaterialValueType Value) noexcept
{
    switch (Value)
    {
    case EMaterialValueType::Invalid: return "invalid";
    case EMaterialValueType::Float1: return "float";
    ...
    case EMaterialValueType::Bool: return "bool";
    case EMaterialValueType::Integer: return "int";
    case EMaterialValueType::Texture2D: return "texture2d";
    case EMaterialValueType::TextureCube: return "texture_cube";
    case EMaterialValueType::Sampler: return "sampler";
    }
    return "invalid";
}
...
{
    if (Value == "surface") return EMaterialDomain::Surface;
    if (Value == "decal") return EMaterialDomain::Decal;
    if (Value == "ui") return EMaterialDomain::UI;
    if (Value == "post_process") return EMaterialDomain::PostProcess;
    return std::nullopt;
}

xr_optional<EMaterialBlendMode> ParseMaterialBlendMode(const xr_string_view Value) noexcept
{
    if (Value == "opaque") return EMaterialBlendMode::Opaque;
    if (Value == "masked") return EMaterialBlendMode::Masked;
    if (Value == "translucent") return EMaterialBlendMode::Translucent;
    if (Value == "additive") return EMaterialBlendMode::Additive;
    if (Value == "modulate") return EMaterialBlendMode::Modulate;
    return std::nullopt;
}

xr_optional<EMaterialShadingModel> ParseMaterialShadingModel(const xr_string_view Value) noexcept
{
    if (Value == "default_lit") return EMaterialShadingModel::DefaultLit;
    if (Value == "unlit") return EMaterialShadingModel::Unlit;
    if (Value == "foliage") return EMaterialShadingModel::Foliage;
    if (Value == "hair") return EMaterialShadingModel::Hair;
    return std::nullopt;
}

xr_optional<EMaterialParameterType> ParseMaterialParameterType(const xr_string_view Value) noexcept
{
    if (Value == "scalar") return EMaterialParameterType::Scalar;
    if (Value == "float2") return EMaterialParameterType::Float2;
    if (Value == "float3") return EMaterialParameterType::Float3;
    if (Value == "float4") return EMaterialParameterType::Float4;
    if (Value == "color") return EMaterialParameterType::Color;
    if (Value == "texture2d") return EMaterialParameterType::Texture2D;
    if (Value == "texture_cube") return EMaterialParameterType::TextureCube;
    if (Value == "sampler_preset") return EMaterialParameterType::SamplerPreset;
    if (Value == "static_bool") return EMaterialParameterType::StaticBool;
    if (Value == "static_enum") return EMaterialParameterType::StaticEnum;
    return std::nullopt;
}

xr_optional<EMaterialValueType> ParseMaterialValueType(const xr_string_view Value) noexcept
{
    if (Value == "float" || Value == "float1") return EMaterialValueType::Float1;
    if (Value == "float2") return EMaterialValueType::Float2;
    if (Value == "float3") return EMaterialValueType::Float3;
    if (Value == "float4" || Value == "color") return EMaterialValueType::Float4;
    if (Value == "bool") return EMaterialValueType::Bool;
    if (Value == "int") return EMaterialValueType::Integer;
    if (Value == "texture2d") return EMaterialValueType::Texture2D;
    if (Value == "texture_cube") return EMaterialValueType::TextureCube;
    if (Value == "sampler") return EMaterialValueType::Sampler;
    return std::nullopt;
}
