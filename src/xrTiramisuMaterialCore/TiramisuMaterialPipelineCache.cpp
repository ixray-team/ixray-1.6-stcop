...
const FMaterialPassProxy* FMaterialPipelineSet::Find(const EMaterialPass Pass, const xr_string_view VertexFactory) const noexcept
{
...
FMaterialPipelineStageResult TiramisuMaterialPipelineCache::Stage(FMaterialPipelineSet Candidate, const xr_span<const FMaterialPassRequest> RequiredPasses)
{
	...
	for (const FMaterialPassRequest& Required : RequiredPasses)
	{
		...
		{
			AddError(Result.Diagnostics, "pipeline_cache.missing_pass", "Pipeline publication is missing pass '" + xr_string(ToString(Required.Pass)) + "' for vertex factory '" + Required.VertexFactory + "'.");
		}
	}
...
xr_optional<FMaterialPassProxy> TiramisuMaterialPipelineCache::Resolve(const FMaterialAssetId& MaterialId, const EMaterialPass Pass, const xr_string_view VertexFactory) const
{
