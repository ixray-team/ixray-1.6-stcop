...
void AddDiagnostic(xr_vector<FMaterialDiagnostic>& Diagnostics, const xr_string_view Code,
    const xr_string& Message, const EMaterialDiagnosticSeverity Severity)
{
    ...
    xr_vector<FMaterialDiagnostic>& Diagnostics)
{
    ...
    if (!MaterialJsonDetail::TryGetString(*Field, Result))
        AddDiagnostic(Diagnostics, "legacy_map.invalid_field_type",
            "Field '" + xr_string(Name) + "' must be a string.", EMaterialDiagnosticSeverity::Error);
    return Result;
}
} // namespace
...
xr_string NormalizeLegacyShaderName(const xr_string_view ShaderName)
{
    ...
    while (Begin < End && std::isspace(static_cast<unsigned char>(ShaderName[Begin]))) ++Begin;
    while (End > Begin && std::isspace(static_cast<unsigned char>(ShaderName[End - 1]))) --End;
    ...
    for (size_t Index = Begin; Index < End; ++Index)
    {
        ...
        if (Separator)
        {
            if (!PreviousSeparator) Result.push_back('\\');
        }
        else
        {
    ...
    const FResolvedLegacyMaterial& Material)
{
    ...
    {
        Result.emplace(FMaterialParameterId{xr_string(LegacyBaseTextureParameterId)},
            Material.Textures[0]);
    }
    if (Material.Textures.size() > 1)
    {
        Result.emplace(FMaterialParameterId{xr_string(LegacyLightmapTextureParameterId)},
            Material.Textures[1]);
    }
    return Result;
}
...
FLegacyMaterialMapParseResult ParseLegacyMaterialMapJson(const xr_string_view JsonText)
{
    ...
    try
    {
        ...
        {
            AddDiagnostic(Result.Diagnostics, "legacy_map.invalid_json",
                "Legacy material map contains invalid JSON.",
                EMaterialDiagnosticSeverity::Error);
            return Result;
        }
        if (!Root.is_object())
        {
            AddDiagnostic(Result.Diagnostics, "legacy_map.invalid_root",
                "Legacy material map root must be an object.", EMaterialDiagnosticSeverity::Error);
            return Result;
        }
        if (const Json* Version = MaterialJsonDetail::Find(Root, "asset_version"))
        {
            if (!MaterialJsonDetail::TryGetUInt32(*Version, Result.Value.Version))
                AddDiagnostic(Result.Diagnostics, "legacy_map.invalid_field_type",
                    "Field 'asset_version' must be an unsigned integer.", EMaterialDiagnosticSeverity::Error);
        }
        ...
        if (Result.Value.Version != LegacyMaterialMapVersion)
            AddDiagnostic(Result.Diagnostics, "legacy_map.unsupported_version",
                "Unsupported legacy material map version " + std::to_string(Result.Value.Version) + ".",
                EMaterialDiagnosticSeverity::Error);
        if (Result.Value.StandardMaterial.empty())
            AddDiagnostic(Result.Diagnostics, "legacy_map.missing_standard", "legacy-map.json has no standard_material.",
                EMaterialDiagnosticSeverity::Error);
        if (Result.Value.ErrorMaterial.empty())
            AddDiagnostic(Result.Diagnostics, "legacy_map.missing_error", "legacy-map.json has no error_material.",
                EMaterialDiagnosticSeverity::Error);
        if (const Json* Mappings = MaterialJsonDetail::Find(Root, "mappings"))
        {
            if (!Mappings->is_object())
                AddDiagnostic(Result.Diagnostics, "legacy_map.invalid_field_type",
                    "Field 'mappings' must be an object.", EMaterialDiagnosticSeverity::Error);
            else for (const auto& [Shader, Material] : Mappings->items())
            {
                xr_string MaterialReference;
                if (!MaterialJsonDetail::TryGetString(Material, MaterialReference))
                    AddDiagnostic(Result.Diagnostics, "legacy_map.invalid_mapping",
                        "Mapping for legacy shader '" + Shader + "' must be a string.",
                        EMaterialDiagnosticSeverity::Error);
                else
...
{
    Json Root{{"asset_version", Map.Version}, {"standard_material", Map.StandardMaterial},
        {"error_material", Map.ErrorMaterial}, {"mappings", Json::object()}};
...
FResolvedLegacyMaterial ResolveLegacyMaterial(const FLegacyMaterialMap& Map, const FLegacyMaterialRequest& Request)
{
    ...
    if (!Map.StandardMaterial.empty() && !Request.ShaderName.empty() && !Result.BaseTexture.empty())
    {
        ...
        AddDiagnostic(Result.Diagnostics, "legacy.automatic_standard",
            "Legacy shader '" + Result.LegacyShaderName + "' used automatic standard material fallback.",
    ...
    AddDiagnostic(Result.Diagnostics, "legacy.error_material",
        "Legacy material could not be mapped and uses the diagnostic error material.",
        EMaterialDiagnosticSeverity::Error);
    return Result;
}
