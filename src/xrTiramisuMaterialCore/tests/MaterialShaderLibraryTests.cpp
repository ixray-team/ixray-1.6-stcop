...
u64 PipelineKey(const EMaterialShaderBlobFormat Format)
{
	...
	return MakeCookedMaterialPipelineKey(Material, *Pass, Format == EMaterialShaderBlobFormat::Dxil ? "d3d12" : "vulkan").StableHash();
}

FMaterialShaderBlob Blob(const EMaterialShaderBlobFormat Format, const EMaterialShaderStage Stage, const u8 BytecodeTag)
{
...
FMaterialBundle CompleteBundle()
{
	...
	Bundle.Records = {
		{EMaterialBundleRecordType::Master, {"master-a"}, {"master-a"}, "master.json", SerializeMaterialAssetJson(Master), "", {}},
		{EMaterialBundleRecordType::FlattenedInstance, {"instance-a"}, {"master-a"}, "instance.json", SerializeMaterialInstanceJson(Instance), "", {}},
	};
	Bundle.ShaderBlobs = {
...
void TestCompleteBackendSelectionAndInstanceAlias(TiramisuMaterialTestRunner& Runner)
{
	...
	const auto Program = Built.Value->Find(
		{"instance-a"}, PipelineKey(EMaterialShaderBlobFormat::Dxil), EMaterialPass::GBuffer, VertexFactory, RenderPassSignature
	);
...
void TestSerializedLibrary(TiramisuMaterialTestRunner& Runner)
{
	...
	const auto Program = Loaded.Value->Find(
		{"master-a"}, PipelineKey(EMaterialShaderBlobFormat::SpirV), EMaterialPass::GBuffer, VertexFactory, RenderPassSignature
	);
	MATERIAL_CHECK(Runner, Program.has_value());
	if (Program)
	{
...
void TestInvalidProgramsRejected(TiramisuMaterialTestRunner& Runner)
{
	...
	Conflict.Records.push_back({EMaterialBundleRecordType::FlattenedInstance, {"instance-a"}, {"master-b"}, "conflict.json", "instance", "", {}});
