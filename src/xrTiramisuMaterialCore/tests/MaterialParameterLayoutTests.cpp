...
void TestPackingAndDescriptorRelocation(TiramisuMaterialTestRunner& Runner)
{
    ...
    const FMaterialParameterPackResult Patched = PatchMaterialParameterResources(Packed.Value,
        [](const EMaterialParameterType Type, const xr_string_view Path) -> xr_optional<FDescriptorHeapIndex>
        {
            if (Type == EMaterialParameterType::Texture2D && Path == "textures/brick") return FDescriptorHeapIndex{17};
            if (Type == EMaterialParameterType::SamplerPreset && Path == "linear_wrap") return FDescriptorHeapIndex{3};
            return std::nullopt;
        });
    ...
    const FMaterialParameterPackResult PatchedByStableParameter =
    ...
    const FMaterialParameterPackResult Unresolved = PatchMaterialParameterResources(Packed.Value,
        [](EMaterialParameterType, xr_string_view) -> xr_optional<FDescriptorHeapIndex> { return std::nullopt; });
...
void TestGpuAbi(TiramisuMaterialTestRunner& Runner)
{
    ...
    MATERIAL_CHECK(Runner,
        sizeof(FMaterialLightGpuData) == MaterialLightGpuDataSize);
    ...
    const FMaterialGpuMatrix GpuMatrix =
        MakeMaterialDrawBufferMatrix(XRayMatrix);
    ...
    for (size_t Component = 0; Component < 4; ++Component)
    {
        ...
        {
            XRayResult[Component] += Position[Axis] *
                XRayMatrix[Axis * 4 + Component];
            HlslResult[Component] +=
                GpuMatrix[Component * 4 + Axis] * Position[Axis];
        }
    }
