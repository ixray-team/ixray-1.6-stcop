...
void TestDescriptorHeapGraphContract()
{
	...
	Options.Parameters.push_back({{"base-texture"}, "BaseTexture", EMaterialParameterType::Texture2D, xr_string("textures/default.dds")});
...
void TestLegacyResolutionOrder()
{
	...
	CHECK(MakeLegacyMaterialInstanceCacheKey(FirstDynamic) == MakeLegacyMaterialInstanceCacheKey(SecondDynamic));
	SameDynamic.Textures[1] = "other_lmap";
	CHECK(MakeLegacyMaterialInstanceCacheKey(FirstDynamic) != MakeLegacyMaterialInstanceCacheKey(ResolveLegacyMaterial(Parsed.Value, SameDynamic)));
