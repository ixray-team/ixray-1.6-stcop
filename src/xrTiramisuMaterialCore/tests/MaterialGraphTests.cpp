...
namespace
{
FMaterialGraphPin Pin(const xr_string& Id, const xr_string& Name, const EMaterialPinDirection Direction, const EMaterialValueType Type)
{
	return {{Id}, Name, Direction, Type};
}

const FMaterialGraphPin& GraphPin(const FMaterialGraphNode& Node, const xr_string_view Name, const EMaterialPinDirection Direction)
{
	const auto Result = std::ranges::find_if(Node.Pins, [Name, Direction](const FMaterialGraphPin& Candidate)
...
void TestTextureSampleParameterBinding(TiramisuMaterialTestRunner& Runner)
{
	...
	Options.Parameters.push_back({{"bound-texture-guid"}, "Albedo", EMaterialParameterType::Texture2D, xr_string{"textures/default"}});
	const FMaterialGraphCompileResult Result = CompileMaterialGraph(Graph, Options);
	MATERIAL_CHECK(Runner, Result.Succeeded());
	MATERIAL_CHECK(Runner, Result.GeneratedHlsl.find("Parameters.P_bound_texture_guid") != xr_string::npos);
