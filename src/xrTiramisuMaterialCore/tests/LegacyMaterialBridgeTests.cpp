...
int main()
{
	...
	for (const FMaterialAssetId& AssetId : SourceAssets)
	{
		...
		MATERIAL_CHECK(Runner, Library.GetMaster(SourceResolved.Value.MasterHandle) != nullptr);
	}
	...
	for (const FLegacyCase& Case : Cases)
	{
		...
		MATERIAL_CHECK(Runner, std::get<xr_string>(RuntimeOverrides.at(FMaterialParameterId{xr_string(LegacyBaseTextureParameterId)})) == "levels\\test\\brick");
		MATERIAL_CHECK(Runner, std::get<xr_string>(RuntimeOverrides.at(FMaterialParameterId{xr_string(LegacyLightmapTextureParameterId)})) == "levels\\test\\brick_lmap");
		...
		if (xr_string_view(Case.Shader) == "lmap")
		{
			...
			for (const auto& [ParameterId, Value] : RuntimeOverrides)
			{
				...
				MATERIAL_CHECK(Runner, RuntimeInstance.SetParameter(*Definition, Value) == EMaterialUpdateError::None);
			}
			...
			const FMaterialParameterPackResult Packed = PackMaterialParameters(
				Layout.Value, Master->Parameters, RuntimeInstance.GetRuntimeParameters()
			);
			...
			const FMaterialParameterPackResult Patched =
				PatchMaterialParameterResources(Packed.Value, [&BaseTexture, &LightmapTexture](const FMaterialParameterResourceReference& Reference) -> xr_optional<FDescriptorHeapIndex>
												{
			...
			MATERIAL_CHECK(Runner, ReadPackedValue<u32>(Patched.Value.Data, BaseField->Offset) == 101);
			MATERIAL_CHECK(Runner, ReadPackedValue<u32>(Patched.Value.Data, LightmapField->Offset) == 202);
		}
	}
	...
	MATERIAL_CHECK(Runner, MakeLegacyMaterialInstanceCacheKey(Cached) != MakeLegacyMaterialInstanceCacheKey(ResolveLegacyMaterial(Map.Value, CachedRequest)));
