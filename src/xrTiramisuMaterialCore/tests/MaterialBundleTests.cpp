...
FMaterialBundle MakeCompleteBundle()
{
    ...
    Bundle.ShaderBlobs = {
        {{"master-a"}, 20, EMaterialShaderBlobFormat::SpirV, "main", {4, 5, 6},
            EMaterialPass::Shadow, EMaterialShaderStage::Pixel, "level_static", "shadow:d32"},
        {{"master-a"}, 10, EMaterialShaderBlobFormat::Dxil, "main", {1, 2, 3},
            EMaterialPass::GBuffer, EMaterialShaderStage::Pixel, "level_static",
...
void TestDeterministicSerialization(TiramisuMaterialTestRunner& Runner)
{
    ...
    MATERIAL_CHECK(Runner,
        Read.Value.ShaderBlobs[0].RenderPassSignature == "gbuffer:rgba8+rgba16f+rgba16f+rg16f:d24s8");
}
