...
FMaterialShaderCompileResult CompileFile(const TiramisuMaterialShaderCompiler& Compiler,
    const EMaterialShaderBackend Backend, const char* Path, const char* Profile)
{
    ...
    Request.IncludeDirectories = {"gamedata/shaders/r5", "gamedata/shaders/r5/common",
        "gamedata/shaders/r5/deferred", "gamedata/shaders/r5/lighting",
        "gamedata/shaders/r5/postprocess", "gamedata/shaders/r5/global"};
...
int main()
{
    ...
    MATERIAL_CHECK(Runner, MaterialGpuAbiSource.find(
        "TIRAMISU_MATERIAL_GPU_ABI_VERSION 2u") != xr_string::npos);
    MATERIAL_CHECK(Runner, MaterialGpuAbiSource.find(
        "TIRAMISU_MATERIAL_LIGHT_GPU_DATA_SIZE 64u") != xr_string::npos);
    MATERIAL_CHECK(Runner, MaterialGpuAbiSource.find(
        "ResourceDescriptorHeap[LightDataBufferIndex]") != xr_string::npos);
    MATERIAL_CHECK(Runner, MaterialGpuAbiSource.find(
        "LightDataOffset + LightIndex") != xr_string::npos);
    MATERIAL_CHECK(Runner, MaterialLightingSource.find(
        "min(LightCount, 64u)") != xr_string::npos);
    MATERIAL_CHECK(Runner, MaterialLightingSource.find(
        "LoadMaterialLightGpuData(Index)") != xr_string::npos);
    MATERIAL_CHECK(Runner, MaterialForwardSource.find(
        "EvaluateMaterialSceneDirectLighting") != xr_string::npos);
    ...
    for (const char* InstanceReference : {
        "ee5ffbc0-bd24-4aa8-9e16-50651ca1c269", "1f7e0305-f4a2-4447-a268-e67a74d0f3f6",
        "c30e92c2-d38c-4423-9f27-e42d245caf54", "68ae697d-7049-46ab-af7d-a1c4ddf2b32d",
        "0f8efc7b-8438-42c8-ac98-876cd4a5c289", "50400bdb-5aab-4fb9-b15f-dba129a042a4",
        "1b5644fb-ed55-443f-af03-eaee4b7f56cb"})
    {
        ...
        const xr_string Implementation = ReadText(std::filesystem::path("gamedata/shaders/r5") /
            Master->Implementation.Source.c_str());
        const FMaterialSourceAssemblyResult Source = AssembleMaterialShaderSourceForPass(*Master, Template,
            Implementation, Resolved.Value.StaticParameters, PassSource, GBufferPass->ShaderSource);
        ...
        for (const EMaterialShaderBackend Backend : {EMaterialShaderBackend::D3D12, EMaterialShaderBackend::Vulkan})
        {
            ...
            Request.IncludeDirectories = {"gamedata/shaders/r5", "gamedata/shaders/r5/materials",
                "gamedata/shaders/r5/materials/passes", "gamedata/shaders/r5/common"};
