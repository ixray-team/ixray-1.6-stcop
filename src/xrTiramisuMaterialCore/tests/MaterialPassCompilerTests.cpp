...
void TestManifestAndRouting(TiramisuMaterialTestRunner& Runner)
{
    ...
    MATERIAL_CHECK(Runner, GetRequiredMaterialPasses(Asset) ==
        xr_vector<EMaterialPass>({EMaterialPass::Depth, EMaterialPass::Shadow, EMaterialPass::GBuffer}));
...
void TestAllPassesCompile(TiramisuMaterialTestRunner& Runner)
{
    ...
    for (const FMaterialPassDefinition& Pass : GetMaterialPassManifest())
    {
        ...
        for (const EMaterialShaderBackend Backend : {EMaterialShaderBackend::D3D12, EMaterialShaderBackend::Vulkan})
        {
            ...
            Request.IncludeDirectories = {"gamedata/shaders/r5", "gamedata/shaders/r5/materials",
                "gamedata/shaders/r5/materials/passes"};
    ...
    const FMaterialSourceAssemblyResult MaskedSource = AssembleMaterialShaderSourceForPass(Masked, Template,
        Implementation, StaticParameters, ReadText(std::filesystem::path("gamedata/shaders/r5") / Depth->ShaderSource),
        Depth->ShaderSource);
...
void TestVertexFactoriesCompile(TiramisuMaterialTestRunner& Runner)
{
    ...
    for (const FMaterialVertexFactoryDefinition& VertexFactory : GetMaterialVertexFactoryManifest())
    {
        ...
        const FMaterialSourceAssemblyResult Source = AssembleMaterialShaderSourceForPass(
            Parsed.Value, Template, Implementation, StaticParameters, VertexSource,
