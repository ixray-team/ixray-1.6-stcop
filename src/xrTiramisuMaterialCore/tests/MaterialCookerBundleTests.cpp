...
int main(const int ArgumentCount, char** ArgumentValues)
{
    ...
    size_t DxilCount = 0;
    size_t SpirVCount = 0;
    xr_set<std::tuple<xr_string, u64,
        EMaterialShaderBlobFormat, EMaterialShaderStage>> UniqueBlobs;
    ...
    for (const FMaterialShaderBlob& Blob : Read.Value.ShaderBlobs)
    {
        ...
        MATERIAL_CHECK(Runner, UniqueBlobs.emplace(
            Blob.MaterialId.Value, Blob.PipelineKey, Blob.Format, Blob.Stage).second);
    ...
    MATERIAL_CHECK(Runner,
        StageCounts[EMaterialShaderStage::Vertex] ==
        StageCounts[EMaterialShaderStage::Pixel]);
