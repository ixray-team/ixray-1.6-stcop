...
void TestInheritanceAndPrecedence(TiramisuMaterialTestRunner& Runner)
{
	...
	MATERIAL_CHECK(Runner, HasDiagnostic(DuplicateRegistration.Diagnostics, "library.duplicate_reference"));
...
void TestPermutationStatistics(TiramisuMaterialTestRunner& Runner)
{
	...
	FMaterialParameterDefinition Quality{{"quality"}, "Quality", EMaterialParameterType::StaticEnum, s32{1}};
	...
	{
		Parameters.push_back({{"overflow-" + std::to_string(Index)}, "Switch", EMaterialParameterType::StaticBool, false});
	}
	const FMaterialPermutationStatistics Overflow =
		CalculateMaterialPermutationStatistics(Parameters);
	MATERIAL_CHECK(Runner, Overflow.Overflow);
	MATERIAL_CHECK(Runner, !Overflow.Exact);
	MATERIAL_CHECK(Runner, Overflow.PermutationCount == std::numeric_limits<u64>::max());
}
} // namespace
