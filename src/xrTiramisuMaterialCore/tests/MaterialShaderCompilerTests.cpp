...
{
    return AssembleMaterialShaderSource(Asset,
        ReadText(std::filesystem::path("gamedata/shaders/r5") / Asset.HlslTemplate.c_str()),
        Implementation, DefaultStaticParameters(Asset));
}

FMaterialShaderCompileResult Compile(const TiramisuMaterialShaderCompiler& Compiler,
    const EMaterialShaderBackend Backend, const xr_string& Source, const xr_string& Name = "compiler-test.hlsl")
{
...
FMaterialShaderCompileResult CompileShaderFile(const TiramisuMaterialShaderCompiler& Compiler,
    const EMaterialShaderBackend Backend, const std::filesystem::path& Path,
    const xr_string_view TargetProfile)
{
    FMaterialShaderCompileRequest Request;
    Request.Backend = Backend;
...
void TestSourceAssembly(TiramisuMaterialTestRunner& Runner)
{
    ...
    const FMaterialSourceAssemblyResult Assembled = AssembleAsset(Parsed.Value,
        ReadText("gamedata/shaders/r5/materials/StandardSurface.hlsl"));
    ...
    MATERIAL_CHECK(Runner, Assembled.Source.find(
        "ResourceDescriptorHeap[NonUniformResourceIndex(ResourceIndex)]") != xr_string::npos);
    MATERIAL_CHECK(Runner, Assembled.Source.find(
        "SamplerDescriptorHeap[NonUniformResourceIndex(SamplerIndex)]") != xr_string::npos);
    ...
    const FMaterialSourceAssemblyResult Rejected = AssembleMaterialShaderSource(Parsed.Value,
        ReadText("gamedata/shaders/r5/materials/MaterialTemplate.hlsl"),
        ReadText("gamedata/shaders/r5/materials/StandardSurface.hlsl"), Unknown);
...
void TestBackendCompilation(TiramisuMaterialTestRunner& Runner, const TiramisuMaterialShaderCompiler& Compiler)
{
    ...
    MATERIAL_CHECK(Runner, SpirV.Bytecode.size() >= SpirVMagic.size() &&
        std::equal(SpirVMagic.begin(), SpirVMagic.end(), SpirV.Bytecode.begin()));
    ...
    // physical input file.
    const FMaterialShaderCompileResult CookerStyleName = Compile(Compiler,
        EMaterialShaderBackend::D3D12, Assembled.Source,
        "standard_surface.material.json:gbuffer:pixel");
...
{
    const FMaterialShaderCompileResult Invalid = Compile(Compiler, EMaterialShaderBackend::D3D12,
        "float4 Main() : SV_Target0 { this_is_not_hlsl; }");
