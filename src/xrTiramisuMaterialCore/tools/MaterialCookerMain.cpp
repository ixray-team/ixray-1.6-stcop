...
void PrintDiagnostics(const xr_vector<FMaterialDiagnostic>& Diagnostics, const xr_string_view Asset)
{
	...
	{
		const char* Severity = Diagnostic.Severity == EMaterialDiagnosticSeverity::Error ? "error" : Diagnostic.Severity == EMaterialDiagnosticSeverity::Warning ? "warning"
																																								 : "info";
...
int main(const int ArgumentCount, char** ArgumentValues)
{
	...
	try
	{
		...
		for (const std::filesystem::directory_entry& Entry : std::filesystem::recursive_directory_iterator(Arguments->Input))
		{
			...
			if (EndsWith(Name, ".material-instance.json"))
			{
				InstancePaths.push_back(Entry.path());
			}
			else if (EndsWith(Name, ".material.json"))
			{
				MasterPaths.push_back(Entry.path());
			}
			else if (Name == "legacy-map.json")
			{
				LegacyMapPath = Entry.path();
			}
		}
		...
		for (const FMaterialAsset& Master : Masters)
		{
			...
			const xr_string HandWrittenImplementation = Master.Implementation.Type == EMaterialImplementationType::Hlsl ? ReadText(Arguments->ShaderRoot / Master.Implementation.Source.c_str()) : xr_string{};
			for (const FMaterialStaticParameterSet& StaticParameters : Permutations[Master.Id.Value])
			{
				...
				for (const EMaterialPass Pass : Passes)
				{
					...
					const FMaterialSourceAssemblyResult VertexSource = AssembleMaterialShaderSourceForPass(
						Master, TemplateSource, Implementation, StaticParameters, VertexFactorySource, VertexFactoryDefinition->ShaderSource
					);
					PrintDiagnostics(VertexSource.Diagnostics, Master.SourcePath + " [" + xr_string(VertexFactoryDefinition->Name) + "/vertex]");
					if (!VertexSource.Succeeded())
					{
						Failed = true;
						continue;
					}
						 ...
						 {EMaterialShaderBackend::D3D12, EMaterialShaderBackend::Vulkan})
					{
						...
						const EMaterialShaderBlobFormat Format =
							Backend == EMaterialShaderBackend::D3D12 ? EMaterialShaderBlobFormat::Dxil : EMaterialShaderBlobFormat::SpirV;

						const auto CompileStage = [&](const FMaterialSourceAssemblyResult& Source,
													  const xr_string_view EntryPoint,
													  const xr_string_view TargetProfile,
													  const EMaterialShaderStage Stage,
													  const xr_string_view StageName)
						{
							...
							Request.IncludeDirectories = {Arguments->ShaderRoot, Arguments->ShaderRoot / "common", Arguments->ShaderRoot / "materials", Arguments->ShaderRoot / "materials" / "passes", Arguments->ShaderRoot / "materials" / "vertex"};
						...
						CompileStage(VertexSource, VertexFactoryDefinition->EntryPoint, VertexFactoryDefinition->TargetProfile, EMaterialShaderStage::Vertex, "vertex");
						CompileStage(PixelSource, PassDefinition->EntryPoint, PassDefinition->TargetProfile, EMaterialShaderStage::Pixel, "pixel");
					}
				}
			}
		}
