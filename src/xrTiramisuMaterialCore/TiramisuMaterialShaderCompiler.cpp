...
void AddDiagnostic(xr_vector<FMaterialDiagnostic>& Diagnostics, const EMaterialDiagnosticSeverity Severity, xr_string Code, xr_string Message)
{
...
xr_string SanitizeDxcSourceName(xr_string Value)
{
	...
	const xr_string_view FileName = LastSlash == xr_string::npos
										? xr_string_view(Value)
										: xr_string_view(Value).substr(LastSlash + 1);
...
FMaterialSourceAssemblyResult AssembleMaterialShaderSource(const FMaterialAsset& Asset, const xr_string_view TemplateSource, const xr_string_view ImplementationSource, const FMaterialStaticParameterSet& StaticParameters, const bool AppendValidationEntryPoint)
{
	...
	{
		AddDiagnostic(Result.Diagnostics, EMaterialDiagnosticSeverity::Error, "shader.template_parameters_include", "Material template must contain exactly one MaterialParameters.generated.hlsl include.");
	}
	if (CountOccurrences(TemplateSource, ImplementationInclude) != 1)
	{
		AddDiagnostic(Result.Diagnostics, EMaterialDiagnosticSeverity::Error, "shader.template_implementation_include", "Material template must contain exactly one MaterialImplementation.generated.hlsl include.");
	}
	if (ImplementationSource.empty())
	{
		AddDiagnostic(Result.Diagnostics, EMaterialDiagnosticSeverity::Error, "shader.empty_implementation", "Material implementation source is empty.");
	}
	...
	std::ranges::sort(SortedParameters, {}, [](const FMaterialParameterDefinition* Parameter)
	...
	for (const FMaterialParameterDefinition* Parameter : SortedParameters)
	{
		...
		{
			AddDiagnostic(Result.Diagnostics, EMaterialDiagnosticSeverity::Error, "shader.static_parameter_type", "Static parameter '" + Parameter->Id.Value + "' has an invalid permutation value.");
			continue;
		}
	...
	{
		const auto Existing = std::ranges::find_if(Asset.Parameters, [&Id](const FMaterialParameterDefinition& Definition)
												   { return Definition.Id == Id && Definition.IsStatic(); });
		if (Existing == Asset.Parameters.end())
		{
			AddDiagnostic(Result.Diagnostics, EMaterialDiagnosticSeverity::Error, "shader.unknown_static_parameter", "Permutation contains unknown static parameter '" + Id.Value + "'.");
		}
	}
...
FMaterialSourceAssemblyResult AssembleMaterialShaderSourceForPass(const FMaterialAsset& Asset, const xr_string_view TemplateSource, const xr_string_view ImplementationSource, const FMaterialStaticParameterSet& StaticParameters, const xr_string_view PassSource, const xr_string_view PassSourceName)
{
	...
	if (PassSource.empty())
	{
		AddDiagnostic(Result.Diagnostics, EMaterialDiagnosticSeverity::Error, "shader.empty_pass", "Material pass source is empty.");
	}
	if (HasErrors(Result.Diagnostics))
	{
		return Result;
...
TiramisuMaterialShaderCompiler::TiramisuMaterialShaderCompiler()
...
FMaterialShaderCompileResult TiramisuMaterialShaderCompiler::Compile(const FMaterialShaderCompileRequest& Request) const
{
	...
	if (!IsAvailable())
	{
		AddDiagnostic(Result.Diagnostics, EMaterialDiagnosticSeverity::Error, "shader.dxc_unavailable", Impl ? Impl->InitializationError : "DXC compiler instance is not initialized.");
		return Result;
	}
	if (Request.Source.empty() || Request.EntryPoint.empty() || Request.TargetProfile.empty())
	{
		AddDiagnostic(Result.Diagnostics, EMaterialDiagnosticSeverity::Error, "shader.invalid_request", "Shader source, entry point and target profile must be provided.");
		return Result;
	}
	...
	if (Request.Backend == EMaterialShaderBackend::Vulkan)
	{
		...
		Arguments.emplace_back(L"-fvk-s-shift");
		Arguments.emplace_back(L"0");
		Arguments.emplace_back(L"0");
		Arguments.emplace_back(L"-fvk-t-shift");
		Arguments.emplace_back(L"128");
		Arguments.emplace_back(L"0");
		Arguments.emplace_back(L"-fvk-b-shift");
		Arguments.emplace_back(L"32");
		Arguments.emplace_back(L"2");
		Arguments.emplace_back(L"-fvk-u-shift");
		Arguments.emplace_back(L"64");
		Arguments.emplace_back(L"0");
		Arguments.emplace_back(L"-fvk-bind-resource-heap");
		Arguments.emplace_back(L"0");
		Arguments.emplace_back(L"0");
		Arguments.emplace_back(L"-fvk-bind-sampler-heap");
		Arguments.emplace_back(L"1");
		Arguments.emplace_back(L"1");
	}
	Arguments.emplace_back(L"-T");
	Arguments.push_back(Utf8ToWide(Request.TargetProfile));
	Arguments.emplace_back(L"-E");
	Arguments.push_back(Utf8ToWide(Request.EntryPoint));
	...
	const HRESULT CompileStatus = Impl->Compiler->Compile(&Buffer, ArgumentPointers.data(), static_cast<u32>(ArgumentPointers.size()), Impl->IncludeHandler.Get(), IID_PPV_ARGS(Compilation.ReleaseAndGetAddressOf()));
	if (FAILED(CompileStatus) || !Compilation)
	{
		AddDiagnostic(Result.Diagnostics, EMaterialDiagnosticSeverity::Error, "shader.dxc_call_failed", "DXC invocation failed with HRESULT " + std::to_string(static_cast<unsigned long>(CompileStatus)) + ".");
		return Result;
	}
	...
	{
		AddDiagnostic(Result.Diagnostics, EMaterialDiagnosticSeverity::Error, "shader.dxc_compile_failed", Messages.empty() ? "DXC rejected the material shader." : Messages);
		return Result;
	}
	if (!Messages.empty())
	{
	...
	{
		AddDiagnostic(Result.Diagnostics, EMaterialDiagnosticSeverity::Error, "shader.dxc_missing_object", "DXC succeeded but returned no shader object.");
		return Result;
	}
