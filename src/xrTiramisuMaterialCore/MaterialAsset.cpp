...
void AddDiagnostic(xr_vector<FMaterialDiagnostic>& Diagnostics, const xr_string_view Code,
    const xr_string& Message, const EMaterialDiagnosticSeverity Severity = EMaterialDiagnosticSeverity::Error)
{
...
xr_optional<FMaterialValue> ParseTypedValue(const Json& Value, const EMaterialParameterType Type)
{
    ...
    case EMaterialParameterType::Scalar:
        if (MaterialJsonDetail::TryGetFloat(Value, Components[0])) return Components[0];
        break;
    case EMaterialParameterType::Float2:
        if (ReadVector(2)) return FFloat2{Components[0], Components[1]};
        break;
    case EMaterialParameterType::Float3:
        if (ReadVector(3)) return FFloat3{Components[0], Components[1], Components[2]};
        break;
    case EMaterialParameterType::Float4:
    case EMaterialParameterType::Color:
        if (ReadVector(4)) return FFloat4{Components[0], Components[1], Components[2], Components[3]};
        break;
    ...
    {
        xr_string Text;
        if (MaterialJsonDetail::TryGetString(Value, Text)) return Text;
        break;
    }
    case EMaterialParameterType::StaticBool:
    {
        bool Boolean = false;
        if (MaterialJsonDetail::TryGetBoolean(Value, Boolean)) return Boolean;
        break;
    }
    case EMaterialParameterType::StaticEnum:
    {
        s32 Integer = 0;
        if (MaterialJsonDetail::TryGetInt32(Value, Integer)) return Integer;
        break;
    }
    }
    return {};
...
xr_optional<FMaterialValue> ParseOverrideValue(const Json& Value, const bool Static)
{
    ...
    float Scalar = 0.0f;
    if (MaterialJsonDetail::TryGetFloat(Value, Scalar)) return Scalar;
    ...
    if (Value.size() == 2) return FFloat2{Components[0], Components[1]};
    if (Value.size() == 3) return FFloat3{Components[0], Components[1], Components[2]};
    return FFloat4{Components[0], Components[1], Components[2], Components[3]};
}

xr_string ReadStringField(const Json& Object, const xr_string_view Name,
...
bool ReadBooleanField(const Json& Object, const xr_string_view Name, const bool Default,
    xr_vector<FMaterialDiagnostic>& Diagnostics)
{
    const Json* Field = MaterialJsonDetail::Find(Object, Name);
    if (!Field)
...
const Json& ReadCollectionField(const Json& Object, const xr_string_view Name, const Json& Empty,
    const bool Array, xr_vector<FMaterialDiagnostic>& Diagnostics)
{
    const Json* Field = MaterialJsonDetail::Find(Object, Name);
    if (!Field)
        return Empty;
    ...
    {
        AddDiagnostic(Diagnostics, "asset.invalid_field_type", "Field '" + xr_string(Name) + "' must be " +
            (Array ? "an array." : "an object."));
        return Empty;
    }
    return *Field;
}

u32 ReadVersionValue(const Json& Value, const xr_string_view Name,
    xr_vector<FMaterialDiagnostic>& Diagnostics)
{
    u32 Version = 0;
...
xr_optional<float> ReadOptionalFloat(const Json& Object, const xr_string_view Name,
    xr_vector<FMaterialDiagnostic>& Diagnostics)
{
    const Json* Field = MaterialJsonDetail::Find(Object, Name);
    if (!Field)
...
u32 ReadVersion(const Json& Root, xr_vector<FMaterialDiagnostic>& Diagnostics)
{
    ...
    {
        AddDiagnostic(Diagnostics, "asset.migrated_version_field",
            "Migrated legacy 'version' field to 'asset_version'.", EMaterialDiagnosticSeverity::Warning);
...
xr_string ReadGuid(const Json& Root, xr_vector<FMaterialDiagnostic>& Diagnostics)
{
    ...
    {
        AddDiagnostic(Diagnostics, "asset.migrated_id_field", "Migrated legacy 'id' field to 'guid'.",
            EMaterialDiagnosticSeverity::Warning);
...
bool ParseParameterArray(const Json& Values, const bool Static, xr_vector<FMaterialParameterDefinition>& Parameters,
    xr_vector<FMaterialDiagnostic>& Diagnostics)
{
    if (!Values.is_array())
    {
...
{
    Json Result{{"guid", Parameter.Id.Value}, {"name", Parameter.Name}, {"type", ToString(Parameter.Type)},
        {"default", SerializeValue(Parameter.DefaultValue)}};
    Json Ui = Json::object();
    if (!Parameter.DisplayName.empty()) Ui["display_name"] = Parameter.DisplayName;
    if (!Parameter.Category.empty()) Ui["category"] = Parameter.Category;
    if (!Parameter.Description.empty()) Ui["description"] = Parameter.Description;
    if (Parameter.Minimum) Ui["min"] = *Parameter.Minimum;
    if (Parameter.Maximum) Ui["max"] = *Parameter.Maximum;
    if (!Ui.empty()) Result["ui"] = std::move(Ui);
    return Result;
}
} // namespace
...
{
    const auto Iterator = std::ranges::find_if(Parameters,
        [&Parameter](const FMaterialParameterDefinition& Definition) { return Definition.Id == Parameter; });
...
FMaterialAssetParseResult ParseMaterialAssetJson(const xr_string_view JsonText, const xr_string_view SourcePath)
{
    ...
    try
    {
        ...
        {
            AddDiagnostic(Result.Diagnostics, "asset.invalid_json",
                "Material asset contains invalid JSON.");
            return Result;
        }
        if (!Root.is_object())
        {
        ...
        if (!Domain) AddDiagnostic(Result.Diagnostics, "asset.invalid_domain", "Unknown material domain.");
        else Result.Value.Domain = *Domain;
        if (!Blend) AddDiagnostic(Result.Diagnostics, "asset.invalid_blend", "Unknown material blend mode.");
        else Result.Value.BlendMode = *Blend;
        if (!Shading) AddDiagnostic(Result.Diagnostics, "asset.invalid_shading_model", "Unknown material shading model.");
        else Result.Value.ShadingModel = *Shading;
        ...
        ParseParameterArray(ReadCollectionField(Root, "parameters", EmptyArray, true, Result.Diagnostics), false,
            Result.Value.Parameters, Result.Diagnostics);
        ParseParameterArray(ReadCollectionField(Root, "static_parameters", EmptyArray, true, Result.Diagnostics), true,
            Result.Value.Parameters, Result.Diagnostics);
...
FMaterialInstanceParseResult ParseMaterialInstanceJson(const xr_string_view JsonText, const xr_string_view SourcePath)
{
    ...
    try
    {
        ...
        {
            AddDiagnostic(Result.Diagnostics, "asset.invalid_json",
                "Material instance contains invalid JSON.");
            return Result;
        }
        if (!Root.is_object())
        {
            ...
            ReadCollectionField(Root, "overrides", EmptyObject, false, Result.Diagnostics).items())
        {
            ...
            if (!Value) AddDiagnostic(Result.Diagnostics, "instance.invalid_override", "Invalid runtime override '" + Id + "'.");
            else Result.Value.Overrides.emplace(FMaterialParameterId{Id}, *Value);
        }
        for (const auto& [Id, ValueJson] :
            ReadCollectionField(Root, "static_overrides", EmptyObject, false, Result.Diagnostics).items())
        {
            ...
            if (!Value) AddDiagnostic(Result.Diagnostics, "instance.invalid_static_override", "Invalid static override '" + Id + "'.");
            else Result.Value.StaticOverrides.emplace(FMaterialParameterId{Id}, *Value);
        }
    }
    catch (const std::exception& Error)
    {
...
{
    Json Root{{"asset_version", Asset.Version}, {"guid", Asset.Id.Value}, {"name", Asset.Name},
        {"domain", ToString(Asset.Domain)}, {"blend_mode", ToString(Asset.BlendMode)},
        {"shading_model", ToString(Asset.ShadingModel)}, {"two_sided", Asset.TwoSided},
        {"template", Asset.HlslTemplate}};
    if (Asset.Implementation.Type == EMaterialImplementationType::Hlsl)
        ...
        Root["implementation"] = {{"type", "graph"},
            {"graph", Json::parse(SerializeMaterialGraphJson(
                Asset.Implementation.Graph), nullptr, false)}};
...
{
    Json Root{{"asset_version", Instance.Version}, {"guid", Instance.Id.Value}, {"name", Instance.Name},
...
bool TiramisuMaterialLibrary::ResolveRecursive(const xr_string_view Reference, xr_vector<xr_string>& ActiveReferences,
    FResolvedMaterialInstance& Resolved, xr_vector<FMaterialDiagnostic>& Diagnostics) const
{
    ...
    if (const auto MasterHandle = FindMaster(Reference))
    {
        ...
        {
            if (Parameter.IsStatic()) Resolved.StaticParameters.emplace(Parameter.Id, Parameter.DefaultValue);
            else Resolved.Parameters.emplace(Parameter.Id, Parameter.DefaultValue);
        }
        return true;
    }
