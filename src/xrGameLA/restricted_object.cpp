...
#ifdef _DEBUG
	Msg("Next line will veryfy position, positionx = %f positiony = %f positionz = %f", position.x, position.y, position.z);
#endif
	if (!accessible(position)) {
	#ifdef DEBUG
		if (psAI_Flags.test(aiBrain)) {
			Msg("Accessible nearest: object name: %s not accessible. Bad nodes or restrictions Pos: %f %f %f", object().Name(), position.x, position.y, position.z);
		}
	#endif
		//Level().space_restriction_manager().remove_restrictions(m_object->ID(), Level().space_restriction_manager().out_restrictions(object().ID()), Level().space_restriction_manager().in_restrictions(object().ID()));
		//result = position;
		//return u32(0);
	}
	if (result.x == 0.000000000 && result.y == 0.000000000 && result.z == 0.000000000) {
	#ifdef DEBUG
		if (psAI_Flags.test(aiBrain)) {
			Msg("Result position for object: %s is zero. Bad nodes or restrictions. Pos: %f %f %f", object().Name(), position.x, position.y, position.z);
		}
	#endif
			//result = position;
			//return u32(0);
