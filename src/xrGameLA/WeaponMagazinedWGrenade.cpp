...
{
	BOOL bResult = CHudItemObject::net_Spawn(server_entity); // обходим нет спавн базового оружи€(чтобы не перетолковать одни и те же переменные) отрабатываем нетспавн худ итема и нетспавним этот класс по собственному алгоритму

	CSE_ALifeItemWeaponMagazinedWGL* const weapon_gl = smart_cast<CSE_ALifeItemWeaponMagazinedWGL*>(server_entity);

	R_ASSERT(weapon_gl);

	// «агрузить с сервера то, что мы экспортировали через ::net_export
	...
	// “ут немного сложно, но пока по другому никак. ¬ зависимости от того, включен ли режим гранатомета, дл€ магазина патронов и магазина подствола выбераетс€ активный или неактивный магазины дл€ хранени€ зар€дов
	xr_vector<CCartridge>& grenade_mag = grenadeMode_ ? m_magazine : inactiveMagazine_; // указатель на переменную хрон€щую зар€ды дл€ подствольника
	xr_vector<CCartridge>& regular_ammo_mag = grenadeMode_ ? inactiveMagazine_ : m_magazine; // указатель на переменную хрон€щую зар€ды дл€ обычного магазина

	u8& grenade_ammo_index = grenadeMode_ ? m_ammoType : inactiveAmmoIndex_; // указатель на переменную хрон€щую тип гранат
	u8& regular_ammo_index = grenadeMode_ ? inactiveAmmoIndex_ : m_ammoType; // указатель на переменную хрон€щую тип патронов

	if (grenadeMode_)
		maxMagazineSize_ = 1;

	// «агрузить подствольник
