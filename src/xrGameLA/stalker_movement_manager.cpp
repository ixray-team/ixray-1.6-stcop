...
void CStalkerMovementManager::set_desired_position(const Fvector *desired_position)
{
	if (desired_position) {
		...
		Msg("Object %s not accessible to desired position. Bad nodes or restrictions. Desired pos: %f %f %f", *object().cName(), desired_position->x, desired_position->y, desired_position->z);
		}
#endif
