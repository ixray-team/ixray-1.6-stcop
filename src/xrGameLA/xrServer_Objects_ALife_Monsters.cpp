...
{
	m_owner_id					= u32(-1);
	m_maxPower					= pSettings->r_float(caSection, "hit_power");
...
void CSE_ALifeCreatureAbstract::on_death		(CSE_Abstract *killer)
{
	...
	//Msg("!!on_death: Next line will set health to -1.0 for npc %s", name_replace());
	fHealth						= -1.f;
}
...
void CSE_ALifeCreatureAbstract::kill_entity			(ALife::_OBJECT_ID id)
{
	if (fHealth>0.f)
	{
		...
		//Msg("!!kill_entity(skyloader function): Next line will set health to 0.0 for npc %s", name_replace());
