	...
	if (!xr_strcmp(base1, "_0"))
		...
		//для безоружного гг отдельные анимации
			if (xr_strcmp(base0, "norm"))
				...
				moving[eIdle]	= K->ID_Cycle_Safe(xr_strconcat(buf,base0,"_torso",base1,"_idle_0"));
...
void SVehicleAnimCollection::Create(IKinematicsAnimated* V,u16 num)
{
	...
	steer_left =	V->ID_Cycle_Safe(xr_strconcat(buf,"steering_idle_",buff1,"ls"));
	if (!steer_left) steer_left = V->ID_Cycle("steering_idle_0_ls");

	steer_right =	V->ID_Cycle_Safe(xr_strconcat(buf,"steering_idle_",buff1,"rs"));
