...
public:
	float						buy_item_condition_factor {0.f};
...
public:
	...
	IC	bool							enabled			(action_show type, const shared_str &section) const;
	...
	IC	void							process			(_action_type type, Container);
	
	template <typename Container>
	IC	void							process			(action_show, Container);
	...
	template <typename _action_type>
	IC	void							default_disabled(_action_type type, bool);
};
