...
LPCSTR CScriptGameObject::installed_scope_name				() const
{
	CWeapon				*wpn			= smart_cast<CWeapon*>(&object());
	if (!wpn) {
		ai().script_engine().script_log			(ScriptStorage::eLuaMessageTypeError,"CScriptEntity : cannot access class member installed_scope_name!");
		return nullptr;
	}
	return wpn->IsScopeAttached()
		? wpn->GetScopeName().c_str()
		: nullptr;
}
...
bool CScriptGameObject::default_invulnerable() const
{
	auto invOwner = smart_cast<CInventoryOwner*>(&object());
	if (!invOwner) {
		ai().script_engine().script_log			(ScriptStorage::eLuaMessageTypeError,"CInventoryOwner : cannot access class member invulnerable!");
		return		(false);
	}

	return			(invOwner->SpecificCharacter().Invulnerable());
}
...
float CScriptGameObject::GetActorWalkWeight ()
{
	...
	return actor->conditions().MaxWalkWeight();
}
