...
void CAttachmentOwner::LOG_AttachedItemsList()
{
	for (u32 i = 0; i < m_attached_objects.size(); ++i)
		if (m_attached_objects[i]){
			Msg("Attached item # %u = %s", i, m_attached_objects[i]->item().object().cNameSect().c_str());
		}
