...
#pragma warning(pop)
		...
		if (specific_character_supply)
		{
			...
			selected_char.LoadSCharacter(m_SpecificCharacter);

			// New XML-based supplies
			SSpecificCharacterSupplies sup = selected_char.Supplies();
			spawn_supplies(dynamic_object, sup);

			// Old LTX-based supplies
			dynamic_object->spawn_supplies(selected_char.SupplySpawn());
		}
	}
}

void CSE_ALifeTraderAbstract::SpawnTemplate(CSE_ALifeDynamicObject *dynamic_object, const SSpecificCharacterSupplies::TemplateRef& defn)
{
	...
	for (u32 i = 0; i < defn.count; ++i)
	{
		...
		{
			spawn_supplies(dynamic_object, tmpl.Supplies());
		}
	}
	--nestLevel;
}

static void SpawnAmmoForWeapon(CSE_ALifeDynamicObject *dynamic_object, const SSpecificCharacterSupplies::Item::AmmoDef& ammoDefn)
{
	if (!ammoDefn.section.empty())
	{
		for (int j = 0; j < ammoDefn.spawnCount; j++)
		{
			dynamic_object->spawn_item(ammoDefn.section.c_str());
		}
	}
}

void CSE_ALifeTraderAbstract::SpawnItem(CSE_ALifeDynamicObject *dynamic_object, const SSpecificCharacterSupplies::Item& defn)
{
	for (u32 i = 0; i < defn.count; ++i)
	{
		if (::Random.randF(1.f) < defn.probability)
		{
			...
			auto W = smart_cast<CSE_ALifeItemWeapon*>(item);
			if (W)
			{
				if (defn.ammo.idx > 0)
				{
					W->set_ammo_type(defn.ammo.idx);
				}
				if (defn.ammo.rounds >= 0)
				{
					W->set_ammo_elapsed(_min(W->get_ammo_magsize(), defn.ammo.rounds));
				}
				SpawnAmmoForWeapon(dynamic_object, defn.ammo);
				
				auto WGL = smart_cast<CSE_ALifeItemWeaponMagazinedWGL*>(item);
				if (WGL)
				{
					if (defn.grenade.idx > 0)
					{
						WGL->grndID_ = defn.grenade.idx;
					}
					if (defn.grenade.rounds >= 0)
					{
						WGL->grndIsLoaded_ = 1;
					}
					SpawnAmmoForWeapon(dynamic_object, defn.grenade);
				}
			}
		}
	}
...
void CSE_ALifeTraderAbstract::spawn_supplies(CSE_ALifeDynamicObject *dynamic_object, const SSpecificCharacterSupplies::ITEMS& items, const SSpecificCharacterSupplies::TEMPLATES& templates)
{
	if (!templates.empty())
	{
		for (auto& tmplRef : templates)
		{
			SpawnTemplate(dynamic_object, tmplRef);
		}
	}
	if (!items.empty())
	{
...
void CSE_ALifeTraderAbstract::spawn_supplies(CSE_ALifeDynamicObject *dynamic_object, const SSpecificCharacterSupplies& supplies)
{
	spawn_supplies(dynamic_object, supplies.m_items, supplies.m_templates);
	if (!supplies.m_selects.empty())
	{
		for (auto& select : supplies.m_selects)
		{
			for (int i = 0; i < select.count; ++i)
			{
				if (!fsimilar(select.probability, 1.f) && ::Random.randF(1.f) > select.probability) continue;
				float rnd = ::Random.randF(select.totalWeight);
				float current = 0.f;
				for (auto& opt : select.options)
				{
					current += opt.weight;
					if (current > rnd)
					{
						spawn_supplies(dynamic_object, opt.items, opt.templates);
						break;
					}
				}
			}
