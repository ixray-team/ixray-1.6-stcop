...
{
	R_ASSERT2(pChild, make_string<const char*>("[%s]: attaching null child, are you kiddin' me?! :D", WindowName()));
	R_ASSERT2(!IsChild(pChild), make_string<const char*>("[%s]: attaching already attached child, are you foolin' me?! :@", WindowName()));
	u32 ret = 0;
...
void CUIWindow::DetachChild(CUIWindow* pChild)
{
	...
	Fvector2 p;
	GetAbsolutePos		(p);
	R_ASSERT2				(it!=m_ChildWndList.end(), make_string<const char*>("Can't detach child with position [%3.1f][%3.1f] because it not found", p.x, p.y));
#else
...
{	
	...
	for(u32 i = 0; it!=first; ++it, i++)
	{
		CUIWindow* w	= (*it);
#ifdef DEBUG
		R_ASSERT2(w, make_string<const char*>("Incorrect window in childlist(%d) - [%s]",i,*m_windowName));
#else
		if (w==nullptr)
		{
