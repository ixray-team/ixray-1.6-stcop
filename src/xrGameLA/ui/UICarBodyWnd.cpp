...
void CUICarBodyWnd::Init()
{
	...
	m_pUIStaticDesc->TextItemControl()->SetText		(nullptr);
...
void CUICarBodyWnd::InitInventoryBox(CInventoryOwner* pOur, CInventoryBox* pInvBox)
{
    m_pOurObject									= pOur;
	m_pOthersObject = m_pCar = nullptr;
...
void CUICarBodyWnd::InitCustomInventory(CInventoryOwner* pOur, CInventoryOwner* pOthers)
{
...
void CUICarBodyWnd::UpdateLists()
{
	...
	if (m_pOthersObject && m_pCar)
	{
		InventoryUtilities::UpdateWeightContainer(*m_pUIOthersBagWnd, &m_pCar->inventory(), m_othersBagWndPrefix);
	}
	else
	{
		m_pUIOthersBagWnd->SetText(m_othersBagWndPrefix);
	}
	m_b_need_update									= false;
}
...
bool CUICarBodyWnd::TransferItem(PIItem itm, CInventoryOwner* owner_from, CInventoryOwner* owner_to, bool b_check)
{
	...
	if(b_check)
	{
		...
		if (invWeight + itmWeight > maxWeight + EPS_S)	return false;
	}
