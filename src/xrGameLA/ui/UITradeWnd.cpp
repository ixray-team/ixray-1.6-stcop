...
void CUITradeWnd::ShowCantDealMsg(LPCSTR text, float time)
{
	...
	msg->SetText(text);	
	msg->m_endTime	= Device.fTimeGlobal + time;// sec
...
void CUITradeWnd::MarkItems(CUICellItem* itm)
{
	...
	MarkItems(weapon, addons, m_uidata->UIOthersBagList, color);
	MarkItems(weapon, addons, m_uidata->UIOurTradeList, color, true);
	MarkItems(weapon, addons, m_uidata->UIOthersTradeList, color);
}
...
void CUITradeWnd::MarkItems(CWeaponMagazined* weapon, const xr_vector<shared_str>& addons, CUIDragDropListEx& list, u32 ammoColor, bool colorize)
{
	...
	for (u32 i = 0; i<item_count; ++i) {
		...
		if (weapon && invitem && invitem->Useful()) {
			auto type = GetWeaponAccessoryType(invitem->object().cNameSect(), weapon->m_ammoTypes, addons);
			switch (type) {
...
CUITradeWnd::EAccossoryType CUITradeWnd::GetWeaponAccessoryType(const shared_str& itemSect, const xr_vector<shared_str>& ammoTypes, const xr_vector<shared_str>& addons)
{
...
void CUITradeWnd::SetCurrentItem(CUICellItem* itm)
{
	...
	if (m_uidata->UIDealMsg && m_uidata->UIDealMsg->IsActual())
	{
		...
		m_uidata->UIDealMsg->m_endTime = Device.fTimeGlobal;
	}
