...
//.	NameComplex			();
	...
	m_SectNightVision		= READ_IF_EXISTS(pSettings, r_string, section, "nightvision_sect", nullptr);
...
{
	P.r_float_q8(condition_, 0.0f, 1.0f);
};
...
void CInventoryItem::save(NET_Packet &packet)
{
	...
	//if (m_eItemPlace == eItemPlaceSlot) Msg("Saving item %s, slot: %d", object().cNameSect_str(), m_currentSlot);
...
void CInventoryItem::load(IReader &packet)
{
	...
	{
		//Msg("Loading item %s with slot: %d", object().cNameSect_str(), place.slot_id);
...
{
	LPCSTR return_string = READ_IF_EXISTS(pSettings, r_string, m_object->cNameSect(), "upgr_icon", "");
