...
{
	return (target.position.distance_to(this->object->Position()) < 5.f);
}
...
{
	return ((this->object->ai_location().level_vertex_id() == target.node) && !this->object->control().path_builder().is_moving_on_path() && (target.position.distance_to(this->object->Position()) > 5.f));
}
