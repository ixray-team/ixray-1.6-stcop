...
CControllerAura::~CControllerAura(){
	/*if (b_do_double_vision_effect){
		Console->Execute("r2_aa off");
		if (hack_mblur_controll){
			...
			else
			{
				if (m_effector) {
					...
					if (m_time_fake_aura < time()){
						...
						/*hack_mblur_controll = Console->GetBool("r2_mblur_enable");
						if (b_do_double_vision_effect){
...
float xr_stdcall CControllerAura::get_effector_strength()
{
	...
	{
		return time() < m_time_fake_aura
			? current_effector_strength * (m_time_fake_aura - time()) / effectorFadeTime
			: 0.f;
	}
