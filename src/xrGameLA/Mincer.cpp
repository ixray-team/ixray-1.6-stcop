...
void CMincer::OnStateSwitch(EZoneState new_state)
{
	if(m_eZoneState!=eZoneStateBlowout && new_state==eZoneStateBlowout)
	{
		...
		for(it = m_ObjectInfoMap.begin(); m_ObjectInfoMap.end() != it; ++it) 
		{
			...
			bool isCar = smart_cast<CCar*>(it->object) != nullptr;
			if (GO)					Telekinesis().activate(GO, isCar ? m_fThrowInImpulseCar : m_fThrowInImpulse, m_fTeleHeight, 100000);
		}
	}
...
void CMincer::feel_touch_new				(CObject* O)
{
	...
	if( m_eZoneState==eZoneStateBlowout && (m_dwBlowoutExplosionTime>(u32)m_iStateTime))
	{
		...
		bool isCar = smart_cast<CCar*>(O) != nullptr;
		Telekinesis().activate(GO, isCar ? m_fThrowInImpulseCar : m_fThrowInImpulse, m_fTeleHeight, 100000);
	}
}
...
{
	float power = Power(dist) * GetPullDeltaPower() * m_fThrowInHitKoefAlive;
	//Fvector dir;
	//dir.random_dir(throw_in_dir,2.f*M_PI);
	if (EA->CLS_ID!=CLSID_OBJECT_ACTOR && power > EPS)
	{
		// Msg("Hit %s by pull: %.2f, dist: %.2f, delta: %.2f", EA->cName().c_str(), power, dist, GetPullDeltaPower());
...
{
	float power = Power(dist) * GetPullDeltaPower() * m_fThrowInHitKoefCar;
	if (smart_cast<CCar*>(GO) && power > EPS)
	{
		CreateHit(GO->ID(), ID(), throw_in_dir, power, 0, Fvector().set(0,0,0), 0.0f, m_eHitTypeBlowout);
	}
}
