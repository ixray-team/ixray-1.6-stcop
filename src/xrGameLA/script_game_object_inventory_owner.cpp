...
luabind::object CScriptGameObject::InstalledUpgrades		()
{
	CInventoryItem*	inventory_item = smart_cast<CInventoryItem*>(&object());
	if (!inventory_item)
	{
	...
	auto& upgs = inventory_item->upgrades();
...
CScriptGameObject *CScriptGameObject::item_in_slot	(u32 slot) const
{
	CInventoryOwner	*inventory_owner = smart_cast<CInventoryOwner*>(&object());
	if (!inventory_owner) {
	...
	TSlotId slot_id = (TSlotId)(slot + 1);
...
u32	CScriptGameObject::active_slot()
{
	...
	u32 activeSlot = inventory_owner->inventory().GetActiveSlot();
	return activeSlot - 1;
}

void CScriptGameObject::PlayAnim(LPCSTR anim, BOOL cycle, u32 slot)
{
	R_ASSERT2(slot<=LAST_SLOT,"invalid slot in function PlayAnim");
	CActor *actor = smart_cast<CActor*>(&object());
...
bool CScriptGameObject::MoveToSlot(CScriptGameObject *obj, bool not_activate)
{
	...
	if (!(item = actor->inventory().get_object_by_id(obj->ID())))
		return false;	
	return actor->inventory().Slot(item->BaseSlot(), item, not_activate);
}
...
void CScriptGameObject::activate_slot	(u32 slot_id)
{
	CInventoryOwner	*inventory_owner = smart_cast<CInventoryOwner*>(&object());
	if (!inventory_owner) {
		ai().script_engine().script_log			(ScriptStorage::eLuaMessageTypeError,"CInventoryOwner : cannot access class member activate_slot! See also 'item_in_slot' and 'active_slot' functions.");
		return						;
	}
#pragma todo("Add 1 to slot number for backward compatiblity with old ShoC-based scripts. Need to update all scripts to use CoP slot numbers!")
	inventory_owner->inventory().Activate((TSlotId)(slot_id + 1));
}
...
void CScriptGameObject::OpenSafe()
{
	CUIGameSP* pGameSP = smart_cast<CUIGameSP*>(CurrentGameUI());
	if (!pGameSP) return;
