...
CSE_Abstract *CALifeSimulatorBase::spawn_item	(LPCSTR section, const Fvector &position, u32 level_vertex_id, GameGraph::_GRAPH_ID game_vertex_id, u16 parent_id, bool registration)
{
	...
	if (&ai && ai().get_level_graph() && (!ai().level_graph().valid_vertex_id(level_vertex_id) || !ai().level_graph().inside(level_vertex_id, position))) {
		dynamic_object->m_flags.set(CSE_ALifeDynamicObject::flUsedAI_Locations, FALSE);
	}
