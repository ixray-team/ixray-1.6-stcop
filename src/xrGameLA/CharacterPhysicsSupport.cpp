...
void CCharacterPhysicsSupport::in_UpdateCL( )
{
	...
	if (!fis_zero(m_death_impulse_vec.magnitude())) {
		float reducedMagnitude = m_death_impulse_vec.magnitude() - m_death_impulse_cooldown_speed * m_time_delta;
		m_death_impulse_vec.set_length(fmax(reducedMagnitude, 0.f));
	}
