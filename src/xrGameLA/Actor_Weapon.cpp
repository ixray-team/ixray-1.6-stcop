...
void CActor::g_fireParams	(const CHudItem* pHudItem, Fvector &fire_pos, Fvector &fire_dir)
{
	...
	CWeaponKnife* knife = smart_cast<CWeaponKnife*>(weapon); // don't correct knife fire pos

	if ((weapon && eacLookAt == cam_active) || (weapon && !knife && eacFirstEye == cam_active && psActorFlags.test(AF_CORRECT_FIREPOS)))
