...
void CSpecificCharacter::LoadSCharacter(shared_str id)
{
	R_ASSERT(id.size());
	m_OwnId = id;
...
#pragma todo("this line disables the infoportion dependancy that is already coded")
	...
	data()->s_map_spot		= pXML->Read("map_spot_pointer", 0, "null");
	...
	shared_str invulnerableFlag = pXML->Read("invulnerable", 0, "");
	data()->m_invulnerable = (invulnerableFlag == "true");
...
bool CSpecificCharacter::Invulnerable() const
{
	return data()->m_invulnerable;
}
