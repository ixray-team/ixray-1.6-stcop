...
void CScriptGameObject::SetStashAsSafe(bool is_safe)
{
	CInventoryBox* InventoryBox		= smart_cast<CInventoryBox*>(&object());
	if (!InventoryBox)
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError, "Cant set stash as safe: Object is not an inventory box");
	else InventoryBox->IsSafe = is_safe;
}

bool CScriptGameObject::IsStashASafe()
{
	CInventoryBox*obj = smart_cast<CInventoryBox*>(&object());
	if (obj && obj->IsSafe) {
		return true;
	}
	return false;
...
void CScriptGameObject::SetSafeCode(LPCSTR code)
{
	CInventoryBox* InventoryBox = smart_cast<CInventoryBox*>(&object());
	if (!InventoryBox)
		ai().script_engine().script_log(ScriptStorage::eLuaMessageTypeError, "Cant set code for safe: Object is not an inventory box");
	else { 
		InventoryBox->SafeCode = code;
	}
}

LPCSTR CScriptGameObject::GetSafeCode()
{
	CInventoryBox*obj = smart_cast<CInventoryBox*>(&object());
	if (obj && obj->IsSafe) {
		return obj->SafeCode;
	}
	Msg("GetSafeCode():The object is not a CInventoryBox->IsSafe");
	return "0";
}

void CScriptGameObject::SetUnlockInfo(LPCSTR infoportion)
{
	CInventoryBox*obj = smart_cast<CInventoryBox*>(&object());
	if (obj) {
		obj->UnlockInfo = infoportion;
	}
}

LPCSTR CScriptGameObject::GetUnlockInfo()
{
	CInventoryBox*obj = smart_cast<CInventoryBox*>(&object());
	if (obj) {
