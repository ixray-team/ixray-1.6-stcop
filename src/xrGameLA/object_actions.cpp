...
void CObjectActionShow::initialize		()
{
	...
	VERIFY							(m_item);
//	if (object().inventory().m_slots[m_item->BaseSlot()].m_pIItem)
//		object().inventory().Ruck	(object().inventory().m_slots[m_item->BaseSlot()].m_pIItem);
//	Loxs fix!
...
/*
	bool							result = object().inventory().Activate	(m_item->BaseSlot());
	VERIFY							(result);
*/
	if (!m_weapon)
		return;
