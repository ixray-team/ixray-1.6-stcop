...
void CGameTask::Load(const TASK_ID& id)
{
	...
	if(!g_gameTaskXml){
		...
		g_gameTaskXml->Load			(CONFIG_PATH, "gameplay\\tasks", "game_tasks.xml");
	}
	...
	for (int i = 0; i < tag_num; i++)
	{
		...
		if (i == 0)
		{
			objective.icon_texture_node_name = g_gameTaskXml->Read(g_gameTaskXml->GetLocalRoot(), "icon", 0, nullptr);

			if (objective.icon_texture_node_name.size() &&
				0 != stricmp(*objective.icon_texture_node_name, "ui\\ui_icons_task"))
			{
		...
		LPCSTR showhide = g_gameTaskXml->Read(l_root, "default_show", 0, nullptr);

		if (showhide && !xr_strcmp(showhide, "0"))
		{
			objective.visible_subtask = false;
		}
...
//------infoportion_skiped
		...
		for(j=0; j<info_num; ++j)
			objective.m_skipedInfos[j]	= g_gameTaskXml->Read(l_root, "infoportion_skipped", j, nullptr);
...
//------infoportion_set_skiped
		...
		for (j = 0; j<info_num; ++j)
			objective.m_infos_on_skiped[j] = g_gameTaskXml->Read(l_root, "infoportion_set_skipped", j, nullptr);
...
//------function_skiped
		...
		for(j=0; j<info_num; ++j){
			str							= g_gameTaskXml->Read(l_root, "function_skipped", j, nullptr);
...
//------function_on_fail
		...
		for(j=0; j<info_num; ++j){
			str							= g_gameTaskXml->Read(l_root, "function_call_skipped", j, nullptr);
...
void SGameTaskObjective::save(IWriter &stream)
{
		...
		save_data(icon_texture_node_name, stream);
...
void SGameTaskObjective::load(IReader &stream)
{
		...
		load_data(icon_texture_node_name, stream);
