...
#pragma todo("Make these params to look for values, not only for existnce of any value; ex: param = '0' still makes it true with current code")
	s = g_uiSpotXml->ReadAttrib(path_base, 0, "store", nullptr);
	if(s)
		m_flags.set( eSerailizable, TRUE);

	s = g_uiSpotXml->ReadAttrib(path_base, 0, "dont_show_if_dead", nullptr);
	if (s)
		m_flags.set(eDontShowIfDead, TRUE);
...
void CMapLocation::UpdateSpot(CUICustomMap* map, CMapSpot* sp )
{
	...
	{
		is_shown = true;
#pragma todo("Cant immagine situation when get_alife will return nullptr in single player")
		CSE_ALifeDynamicObject* obj = nullptr;
		obj = ai().alife().objects().object(m_objectID, true);

		if (!IsUserDefined() && ai().get_alife() && !obj)
		{
		...
		if (m_flags.test(eHideInOffline) && !IsUserDefined() && ai().get_alife() && !obj->m_bOnline)
		{
			is_shown = false;
			return;
		}

		CSE_ALifeCreatureAbstract* alive_obj = obj ? smart_cast<CSE_ALifeCreatureAbstract*>(obj) : nullptr;

		if (m_flags.test(eDontShowIfDead) && !IsUserDefined() && alive_obj && !alive_obj->g_Alive())
		{
			is_shown = false;
			return;
		}

		if (!IsUserDefined() && ai().get_alife() && obj->m_flags.test(CSE_ALifeObject::flVisibleForMap == FALSE))
		{
			is_shown = false;
			return;
		}
...
void CRelationMapLocation::PrintMapLocationInfo()
{
	inherited::PrintMapLocationInfo();
...
void CUserDefinedMapLocation::PrintMapLocationInfo()
{
	inherited::PrintMapLocationInfo();
