...
//		if ( !item.has_upgrade( (*ib)->id() ) )
		...
		if ( !item.has_upgrade_group( U->parent_group_id() ) )
		{
			if ( loading )
			{
				Msg("Loading item: Upgrade <%s> of inventory item [%s] (id = %d) can`t be installed! Error = result_e_parents",
	...
	for ( ; ib != ie ; ++ib )
	{
		...
		if ( item.has_upgrade( (*ib)->id() ) )
		{
			if ( loading )
			{
				Msg( "Loading item: Upgrade <%s> of inventory item [%s] (id = %d) can`t be installed! Error = result_e_group",
					test_upgrade.id_str(), item.m_section_id.c_str(), item.object_id() );
			}
