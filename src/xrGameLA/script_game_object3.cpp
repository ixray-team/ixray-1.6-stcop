...
class TradeParamsFromLuaIterator {
public:
	TradeParamsFromLuaIterator(luabind::object::iterator src) : it_(src) {}
	TradeParamsFromLuaIterator& operator++() {
		++it_;
		pairValid_ = false;
		return *this;
	}
	std::pair<shared_str, shared_str>& operator*() {
		if (!pairValid_) {
			pair_.first = luabind::object_cast<LPCSTR>(it_.key());
			pair_.second = luabind::object_cast<LPCSTR>(*it_);
			pairValid_ = true;
		}
		return pair_;
	}
	bool operator==(const TradeParamsFromLuaIterator& o) const {
		return o.it_ == it_;
	}
	bool operator!=(const TradeParamsFromLuaIterator& o) const { return !(*this == o); }

private:
	luabind::object::iterator it_;
	bool pairValid_ {false};
	std::pair<shared_str, shared_str> pair_;
};

class TradeParamsTableWrapper {
public:
	TradeParamsTableWrapper(luabind::object src) : obj_(src) {}
	TradeParamsFromLuaIterator begin() { return TradeParamsFromLuaIterator(obj_.begin()); }
	TradeParamsFromLuaIterator end() { return TradeParamsFromLuaIterator(obj_.end()); }
...
CInventoryOwner* GetCheckInvOwner				(const CScriptGameObject& sgo, LPCSTR memberName) {
	auto io = smart_cast<CInventoryOwner*>(&sgo.object());
	if (!io) {
		ai().script_engine().script_log			(ScriptStorage::eLuaMessageTypeError,"CInventoryOwner : cannot access class member %s!", memberName);
	}
	return io;
}

void CScriptGameObject::sell_condition			(luabind::object info)
{
	auto inventory_owner = GetCheckInvOwner(*this, "sell_condition");
	if (!inventory_owner) return;

	inventory_owner->trade_parameters().process(CTradeParameters::action_sell(0), TradeParamsTableWrapper(info));
}
...
void CScriptGameObject::sell_condition			(bool enabledByDefault)
{
	auto inventory_owner = GetCheckInvOwner(*this, "sell_condition");
	if (!inventory_owner) return;

	inventory_owner->trade_parameters().default_disabled(CTradeParameters::action_sell(0), !enabledByDefault);
}

void CScriptGameObject::buy_condition			(luabind::object info)
{
	auto inventory_owner = GetCheckInvOwner(*this, "buy_condition");
	if (!inventory_owner) return;

	inventory_owner->trade_parameters().process(CTradeParameters::action_buy(0), TradeParamsTableWrapper(info));
}
...
void CScriptGameObject::buy_condition			(bool enabledByDefault)
{
	auto inventory_owner = GetCheckInvOwner(*this, "buy_condition");
	if (!inventory_owner) return;

	inventory_owner->trade_parameters().default_disabled(CTradeParameters::action_buy(0), !enabledByDefault);
}

void CScriptGameObject::show_condition			(luabind::object tbl)
{
	auto inventory_owner = GetCheckInvOwner(*this, "show_condition");
	if (!inventory_owner) return;

	inventory_owner->trade_parameters().process	(
		CTradeParameters::action_show(0),
		TradeParamsTableWrapper(tbl)
	);
}
...
void CScriptGameObject::buy_supplies			(CScriptIniFile *ini_file, LPCSTR section)
{
	...
	inventory_owner->get_create_purchase_list()->process(
		*ini_file,
		section,
		*inventory_owner
	);
...
{
	default_trade_parameters().process	(CTradeParameters::action_sell(0), TradeParamsTableWrapper(tbl));
}
...
{
	default_trade_parameters().process	(CTradeParameters::action_buy(0), TradeParamsTableWrapper(tbl));
}
...
{
	default_trade_parameters().process	(CTradeParameters::action_show(0), TradeParamsTableWrapper(tbl));
}
