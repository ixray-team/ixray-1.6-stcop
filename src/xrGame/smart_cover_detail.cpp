...
int smart_cover::detail::parse_int		(luabind::object const &table, LPCSTR identifier)
{
	VERIFY2			(luabind::type(table) == LUA_TTABLE, "invalid loophole description passed");
	luabind::object	result = table[identifier];
	VERIFY2			(luabind::type(result) != LUA_TNIL, make_string<const char*>("cannot read number value %s", identifier));
	VERIFY2			(luabind::type(result) == LUA_TNUMBER, make_string<const char*>("cannot read number value %s", identifier));
