...
{
	EngineLog("! Cannot build GAME path! (object {})",*object().cName());
	EngineLog("! CURRENT LEVEL : {}",*Level().name());
	Fvector						temp = ai().game_graph().vertex(object().ai_location().game_vertex_id())->level_point();
	EngineLog("! CURRENT game point position : {}", temp);
	const GameGraph::CVertex	*vertex = ai().game_graph().vertex(game_dest_vertex_id());
	EngineLog("! TARGET LEVEL : {}",*ai().game_graph().header().level(vertex->level_id()).name());
	temp						= vertex->level_point();
	EngineLog("! TARGET  game point position : {}", temp);
	...
	EngineLog("! Object masks ({}) :",m_location_manager->vertex_types().size());
