...
void CHudItem::Load(const char* section)
{
	...
	BaseYPRParams.m_fHudYawInertiaK = pSettings->read_if_exists<float>(hud_sect, "hud_yaw_inertia_k", 0.0f);
	BaseYPRParams.m_fHudPitchInertiaK = pSettings->read_if_exists<float>(hud_sect, "hud_pitch_inertia_k", 0.0f);
	BaseYPRParams.m_fHudRollInertiaK = pSettings->read_if_exists<float>(hud_sect, "hud_roll_inertia_k", 0.0f);
	BaseYPRParams.m_fHudInertiaSpeed = pSettings->read_if_exists<float>(hud_sect, "hud_inertia_speed", 10.0f);

	ZoomYPRParams.m_fHudYawInertiaK = pSettings->read_if_exists<float>(hud_sect, "hud_zoom_yaw_inertia_k", 0.0f);
	ZoomYPRParams.m_fHudPitchInertiaK = pSettings->read_if_exists<float>(hud_sect, "hud_zoom_pitch_inertia_k", 0.0f);
	ZoomYPRParams.m_fHudRollInertiaK = pSettings->read_if_exists<float>(hud_sect, "hud_zoom_roll_inertia_k", 0.0f);
	ZoomYPRParams.m_fHudInertiaSpeed = pSettings->read_if_exists<float>(hud_sect, "hud_zoom_inertia_speed", 10.0f);
	...
	ControllerTime = pSettings->read_if_exists<float>(hud_sect, "controller_time", ControllerTime);
	ProhibitSuicide = pSettings->read_if_exists<bool>(hud_sect, "prohibit_suicide", ProhibitSuicide);
