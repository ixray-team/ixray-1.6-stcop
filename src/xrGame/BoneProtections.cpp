...
void SBoneProtections::add(const shared_str& bone_sect, IKinematics* kinematics)
{
	...
	for (const auto& data : protections.Data)
	{
		...
		if (!xr_strcmp(data.first.c_str(), "default"))
		{
			...
			BP.koeff += (float)atof(_GetItem(data.second.c_str(), 0, buffer));
			BP.armor += (float)atof(_GetItem(data.second.c_str(), 1, buffer));
		}
		else
		{
			s16	bone_id = kinematics->LL_BoneID(data.first);
			R_ASSERT2(BI_NONE != bone_id, data.first.c_str());
