...
void CSE_ALifeDynamicObject::add_offline(const xr_vector<ALife::_OBJECT_ID>& saved_children, const bool& update_registries)
{
	for (u32 i = 0, n = saved_children.size(); i < n; ++i)
	{
		...
		if (!child)
		{
			Msg("~Warning %s CSE_ALifeDynamicObject::add_offline children[%d] not found", name(), i);
			continue;
		}
		...
		// #pragma todo ("hozar to ???: Make a miscalculation of the synchronization of children")
