		...
		for (SZoneObjectInfo& info : m_ObjectInfoMap)
		{
			if (info.object && !info.object->getDestroy())
			{
				if (CPhysicsShellHolder* physics_object = info.object->cast_physics_shell_holder())
				{
					...
					telekinesis()->append_tobject(new CTeleWhirlwindObject(tele_object_params));
				}
			}
		}
	}
...
void CMincer::Load (const char* section)
{
	...
	telekinesis()->set_destroing_particles(shared_str(pSettings->r_string(section,"tearing_particles")));
	telekinesis()->set_throw_power(pSettings->r_float(section,"throw_out_impulse"));
...
bool CMincer::net_Spawn(CSE_Abstract* DC)
{
	...
	Center(C);
	C.y+=m_fTeleHeight;
	telekinesis()->SetCenter(C);
	telekinesis()->SetOwnerObject(cast_game_object());
	return result;
}
...
void CMincer::NotificateDestroy(CPHDestroyableNotificate* dn)
{
	...
	telekinesis()->draw_out_impact(dir,impulse);
	...
	m_tearing_sound.play_at_pos(nullptr,telekinesis()->Center());
