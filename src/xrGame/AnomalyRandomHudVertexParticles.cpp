...
{
	xr_string options_section = READ_IF_EXISTS(pSettings, r_string, section, "procedural_vertex_hud_particles_options_section", "");
	if (!options_section.empty())
	{
		const char* sect = options_section.c_str();
		m_use_procedural_vertex_hud_particles = READ_IF_EXISTS(pSettings, r_bool, sect, "use_procedural_vertex_hud_particles", false);
