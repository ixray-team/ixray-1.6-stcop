...
void transform_to_hud(Fvector& pos, Fvector& dir) {
	dir.add(pos);
	transform_to_hud_temp(dir);
	transform_to_hud_temp(pos);
...
void HudLightTorch::NewTorchlight(const char* section) {
	...
	render_light = ::Render->light_create();
	render_light->set_type((IRender_Light::LT)READ_IF_EXISTS(pSettings, r_u32, section, "torch_render_type", IRender_Light::SPOT));
	render_light->set_range(READ_IF_EXISTS(pSettings, r_float, section, "torch_r2_range", 15.0f));
	render_light->set_shadow(!!READ_IF_EXISTS(pSettings, r_bool, section, "torch_render_shadow", TRUE));

	omni_light = ::Render->light_create();
	omni_light->set_type(IRender_Light::POINT); // (IRender_Light::LT)READ_IF_EXISTS(pSettings, r_u32, section, "torch_omni_type", IRender_Light::POINT));
	omni_light->set_range(READ_IF_EXISTS(pSettings, r_float, section, "torch_r2_omni_range", 0.75f));
	omni_light->set_shadow(!!READ_IF_EXISTS(pSettings, r_bool, section, "torch_omni_shadow", FALSE));

	light_bone = pSettings->r_string(section, "torch_light_bone");

	light_offset.x = READ_IF_EXISTS(pSettings, r_float, section, "torch_attach_offset_x", 0.0f);
	light_offset.y = READ_IF_EXISTS(pSettings, r_float, section, "torch_attach_offset_y", 0.0f);
	light_offset.z = READ_IF_EXISTS(pSettings, r_float, section, "torch_attach_offset_z", 0.0f);

	light_world_offset.x = READ_IF_EXISTS(pSettings, r_float, section, "torch_world_attach_offset_x", 0.0f);
	light_world_offset.y = READ_IF_EXISTS(pSettings, r_float, section, "torch_world_attach_offset_y", 0.0f);
	light_world_offset.z = READ_IF_EXISTS(pSettings, r_float, section, "torch_world_attach_offset_z", 0.0f);

	omni_offset.x = READ_IF_EXISTS(pSettings, r_float, section, "torch_omni_attach_offset_x", light_offset.x);
	omni_offset.y = READ_IF_EXISTS(pSettings, r_float, section, "torch_omni_attach_offset_y", light_offset.y);
	omni_offset.z = READ_IF_EXISTS(pSettings, r_float, section, "torch_omni_attach_offset_z", light_offset.z);

	omni_world_offset.x = READ_IF_EXISTS(pSettings, r_float, section, "torch_omni_world_attach_offset_x", light_world_offset.x);
	omni_world_offset.y = READ_IF_EXISTS(pSettings, r_float, section, "torch_omni_world_attach_offset_y", light_world_offset.y);
	omni_world_offset.z = READ_IF_EXISTS(pSettings, r_float, section, "torch_omni_world_attach_offset_z", light_world_offset.z);

	light_color.r = READ_IF_EXISTS(pSettings, r_float, section, "torch_r2_color_r", 0.60f);
	light_color.g = READ_IF_EXISTS(pSettings, r_float, section, "torch_r2_color_g", 0.55f);
	light_color.b = READ_IF_EXISTS(pSettings, r_float, section, "torch_r2_color_b", 0.55f);
	light_color.a = READ_IF_EXISTS(pSettings, r_float, section, "torch_r2_color_a", 0.80f);
	render_light->set_color(light_color);

	omni_color.r = READ_IF_EXISTS(pSettings, r_float, section, "torch_r2_omni_color_r", 0.60f);
	omni_color.g = READ_IF_EXISTS(pSettings, r_float, section, "torch_r2_omni_color_g", 0.55f);
	omni_color.b = READ_IF_EXISTS(pSettings, r_float, section, "torch_r2_omni_color_b", 0.55f);
	omni_color.a = READ_IF_EXISTS(pSettings, r_float, section, "torch_r2_omni_color_a", 0.80f);
	omni_light->set_color(omni_color);

	render_light->set_cone(deg2rad(READ_IF_EXISTS(pSettings, r_float, section, "torch_spot_angle", 75.0f)));
	render_light->set_texture(pSettings->r_string(section, "torch_spot_texture"));

	is_lightdir_by_bone = !!READ_IF_EXISTS(pSettings, r_bool, section, "light_directions_by_bones", FALSE);
	if (is_lightdir_by_bone)
		lightdir_bone_name = pSettings->r_string(section, "light_dir_bone");

	SwitchTorchlight(is_render_light);
}

void HudLightTorch::SwitchTorchlight(bool is_active)
{
	if (torch_installed)
		is_render_light = is_active;
	else
		is_render_light = false;
	//	Msg("Try SwitchTorchlight");
}

void HudLightTorch::UpdateTorchFromObject(CHudItem* item)
{
	if (!render_light || !omni_light || !item || !item->object().Visual())
		return;

	if (item->GetState() == item->eHidden && item->object().H_Parent()) {
		render_light->set_active(false);
		omni_light->set_active(false);
		return;
	}
	...
	if (is_render_light)
	{
		...
		Fvector light_pos = { 0, 0, 0 };
		Fvector light_dir = { 0, 0, 1 };
		Fvector omni_pos = { 0, 0, 0 };
		u16 light_bone_id = BI_NONE;
		u16 lightdir_bone_id = BI_NONE;
		Fvector up, right;
		if (is_hud_mode) {
			xform = item->HudItemData()->m_item_transform;
			kin = item->HudItemData()->m_model;
			light_bone_id = kin->LL_BoneID(light_bone.c_str());
			kin->LL_GetTransform(light_bone_id).transform_tiny(light_pos, light_offset);
			kin->LL_GetTransform(light_bone_id).transform_tiny(omni_pos, omni_offset);
			if (is_lightdir_by_bone) {
				lightdir_bone_id = kin->LL_BoneID(lightdir_bone_name.c_str());
				kin->LL_GetTransform(lightdir_bone_id).transform_tiny(light_dir, light_offset);
				light_dir = light_dir.sub(light_pos).normalize();
			}
		}
		else {
			...
			light_bone_id = kin->LL_BoneID(light_bone.c_str());
			kin->LL_GetTransform(light_bone_id).transform_tiny(light_pos, light_world_offset);
			kin->LL_GetTransform(light_bone_id).transform_tiny(omni_pos, omni_world_offset);
			if (is_lightdir_by_bone) {
				lightdir_bone_id = kin->LL_BoneID(lightdir_bone_name.c_str());
				kin->LL_GetTransform(lightdir_bone_id).transform_tiny(light_dir, light_world_offset);
				light_dir = light_dir.sub(light_pos).normalize();
			}
		}

		xform.transform_tiny(light_pos);
		xform.transform_tiny(omni_pos);
		xform.transform_dir(light_dir);

		if (is_hud_mode) {
			transform_to_hud(light_pos, light_dir);
			transform_to_hud_temp(omni_pos);
		}

		Fvector::generate_orthonormal_basis_normalized(light_dir, up, right);

		omni_light->set_hud_mode(false);
		omni_light->set_position(omni_pos);

		render_light->set_hud_mode(false);
		render_light->set_position(light_pos);
		render_light->set_rotation(light_dir, right);

		render_light->set_ignore_object(item->object().H_Root());
	}

	render_light->set_active(is_render_light);
	omni_light->set_active(false);
}
