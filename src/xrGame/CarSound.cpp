...
void CCar::SCarSound::Init()
{
	...
	if (ini->section_exist("car_sound") && ini->line_exist("car_sound","snd_volume"))
	{
		...
		// TODO: 
		engine_start_delay = fengine_start_delay;//iFloor((snd_engine_start._handle() ? iFloor(snd_engine_start.get_length_sec()*1000.0f) : 1.f)*fengine_start_delay);
...
void CCar::SCarSound::Drive()
{
	...
	if(!snd_engine.slot()) snd_engine.play(pcar,sm_Looped);
	SetSoundPosition(snd_engine);
}
