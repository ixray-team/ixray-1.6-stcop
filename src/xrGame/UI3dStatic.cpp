...
//////////////////////////////////////////////////////////////////////
#define DIST (Device.fViewportNear + 0.1f)

CUI3dStatic::CUI3dStatic()
{
    m_x_angle = m_y_angle = m_z_angle = 0;
    dist = DIST;
    viewport_near = Device.fViewportNear;
}
...
void CUI3dStatic::Draw()
{
    if (m_pCurrentItem)
    {
        ...
        Frect rect;
        GetAbsoluteRect(rect);
        m_icon_screen_size = rect;

        m_icon_screen_size.x1 *= Device.Width / UI_BASE_WIDTH;
        m_icon_screen_size.x2 *= Device.Width / UI_BASE_WIDTH;

        m_icon_screen_size.y1 *= Device.Height / UI_BASE_HEIGHT;
        m_icon_screen_size.y2 *= Device.Height / UI_BASE_HEIGHT;

    // build camera matrix
        Fbox bb = m_pCurrentItem->getVisData().box;
        bb.getcenter(Device.vCameraPosition);

        Fmatrix mRotation; mRotation.setXYZ(m_x_angle, m_y_angle, m_z_angle);

        Device.vCameraDirection.set(mRotation.k);

        Device.vCameraTop.set(mRotation.j);
        Device.vCameraRight.set(mRotation.i);
        ...
        SizeX = SizeY = std::max(SizeY, SizeX);

        SizeY *= std::max(1.0f, rect.height() / rect.width());
        SizeX *= std::max(1.0f, rect.width() / rect.height());
