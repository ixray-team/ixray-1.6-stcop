...
void WeaponUsageStatistic::OnBullet_Fire(SBullet* pBullet, const CCartridge& cartridge)
{
	...
	CActor* pActor = object_parent->cast_actor();
	if (!pActor) return;
	//-----------------------------------------------------------------------------------
...
void WeaponUsageStatistic::OnBullet_Hit(SBullet* pBullet, ALife::_OBJECT_ID TargetID, s16 element, Fvector HitLocation)
{
	statistic_sync_quard syncg(m_mutex);
	if (!pBullet || !pBullet->flags.allow_sendhit) return;
//	Msg("! OnBullet Hit ID[%d]", pBullet->m_dwID);
	...
	if (!BulletIt->HitRefCount++) 
	{
		...
		if (!pTarget) return;
		CActor* pActor = pTarget->cast_actor();
		if (!pActor) return;
		//---------------------------
		...
		NewHit.BoneName = PKinematics(pTarget->Visual())->LL_BoneName_dbg(element);
		NewHit.count = 1;
		//---------------------------
...
void WeaponUsageStatistic::On_Check_Respond(NET_Packet* P)
{
	...
	for (i=0; i<NumTrue; i++)
	{
		...
		if (WeaponIt->FindHit(BulletID, HitIt))
		{
			...
			if (pObj)
				HData.BoneName			= PKinematics(pObj->Visual())->LL_BoneName_dbg(BoneID);
		}
		//---------------------------------------------------------------
		RemoveBullet(BulletIt);
	}
