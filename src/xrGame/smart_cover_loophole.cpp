	...
	m_range			(0.f)
{
	...
	if (m_fov_direction.square_magnitude() < EPS_L) {
		EngineLog				("! fov direction for loophole %s is setup incorrectly", m_id.c_str());
	...
	if (m_danger_fov_direction.square_magnitude() < EPS_L) {
		EngineLog				("! danger fov direction for loophole %s is setup incorrectly", m_id.c_str());
	...
	if (m_enter_direction.square_magnitude() < EPS_L) {
		EngineLog				("! enter direction for loophole %s is setup incorrectly", m_id.c_str());
