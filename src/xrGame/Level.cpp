...
void CLevel::cl_Process_Event				(u16 dest, u16 type, NET_Packet& P)
{
	...
	if (type != GE_DESTROY_REJECT)
	{
		if (type == GE_DESTROY)
		{
			//AVO: fix for SPAWN_ANTIFREEZE crashes caused by rapid online-offline switch.
			//In such cases inventory items are queued up for a spawn, however parent is already destroyed which cases game to crash
