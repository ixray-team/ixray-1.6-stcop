...
//	R_ASSERT2( e_entity, make_string( "entity not found. parent_id = [%d], entity_id = [%d], frame = [%d]", id_parent, id_entity, Device.dwFrame ).c_str() );
	...
	if ( !e_entity ) {
		EngineLog( "! ERROR on rejecting: entity not found. parent_id = [{}], entity_id = [{}], frame = [{}].", id_parent, id_entity, Device.dwFrame );
		return false;
	}

//	R_ASSERT2( e_parent, make_string( "parent not found. parent_id = [%d], entity_id = [%d], frame = [%d]", id_parent, id_entity, Device.dwFrame ).c_str() );
	VERIFY2  ( e_parent, make_string( "parent not found. parent_id = [{}], entity_id = [{}], frame = [{}]", id_parent, id_entity, Device.dwFrame ).c_str() );
	if ( !e_parent ) {
		EngineLog( "! ERROR on rejecting: parent not found. parent_id = [{}], entity_id = [{}], frame = [{}].", id_parent, id_entity, Device.dwFrame );
		return false;
	}
...
//.	Msg("---ID_Parent [%d], id_parent [%d]", e_entity->ID_Parent, id_parent);
	...
	{
		EngineLog("! ERROR: e_entity->ID_Parent = [{}]  parent = [{}][{}]  entity_id = [{}]  frame = [{}]",
