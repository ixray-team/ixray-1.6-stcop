...
{
	string256 full_name;
	m_fCameraMoveEpsilon	= pSettings->read_if_exists<float>(Section, xr_strconcat(full_name, Prefix, "camera_move_epsilon"),	pSettings->r_float(EFFECTOR_ZOOM_SECTION, "camera_move_epsilon"));
	m_fDispMin				= pSettings->read_if_exists<float>(Section, xr_strconcat(full_name, Prefix, "disp_min"),				pSettings->r_float(EFFECTOR_ZOOM_SECTION, "disp_min"));
	m_fSpeedMin				= pSettings->read_if_exists<float>(Section, xr_strconcat(full_name, Prefix, "speed_min"),			pSettings->r_float(EFFECTOR_ZOOM_SECTION, "speed_min"));
	m_fZoomAimingDispK		= pSettings->read_if_exists<float>(Section, xr_strconcat(full_name, Prefix, "zoom_aim_disp_k"),		pSettings->r_float(EFFECTOR_ZOOM_SECTION, "zoom_aim_disp_k"));
	m_fZoomAimingSpeedK		= pSettings->read_if_exists<float>(Section, xr_strconcat(full_name, Prefix, "zoom_aim_speed_k"),		pSettings->r_float(EFFECTOR_ZOOM_SECTION, "zoom_aim_speed_k"));
	m_dwDeltaTime			= pSettings->read_if_exists<u32>(Section, xr_strconcat(full_name, Prefix, "delta_time"),			pSettings->r_u32(EFFECTOR_ZOOM_SECTION, "delta_time"));
};
