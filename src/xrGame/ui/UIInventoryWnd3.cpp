...
void CUIInventoryWnd::ActivatePropertiesBox()
{
	...
	if(!pOutfit && CurrentIItem()->BaseSlot()!=NO_ACTIVE_SLOT && !m_pInv->m_slots[CurrentIItem()->BaseSlot()].m_bPersistent && m_pInv->CanPutInSlot(CurrentIItem(), CurrentIItem()->BaseSlot()))
	{
	...
	if(CurrentIItem()->Ruck() && m_pInv->CanPutInRuck(CurrentIItem()) && (CurrentIItem()->BaseSlot()==u32(-1) || !m_pInv->m_slots[CurrentIItem()->BaseSlot()].m_bPersistent) )
	{
		if(!pOutfit)
	...
	if(pScope)
	{
		if(m_pInv->m_slots[INV_SLOT_2].m_pIItem != nullptr &&
		   m_pInv->m_slots[INV_SLOT_2].m_pIItem->CanAttach(pScope))
		 {
			PIItem tgt = m_pInv->m_slots[INV_SLOT_2].m_pIItem;
			UIPropertiesBox.AddItem("st_attach_scope_to_pistol",  (void*)tgt, INVENTORY_ATTACH_ADDON);
			b_show			= true;
		 }
		 if(m_pInv->m_slots[INV_SLOT_3].m_pIItem != nullptr &&
			m_pInv->m_slots[INV_SLOT_3].m_pIItem->CanAttach(pScope))
		 {
			PIItem tgt = m_pInv->m_slots[INV_SLOT_3].m_pIItem;
	...
	{
		 if(m_pInv->m_slots[INV_SLOT_2].m_pIItem != nullptr &&
		   m_pInv->m_slots[INV_SLOT_2].m_pIItem->CanAttach(pSilencer))
		 {
			PIItem tgt = m_pInv->m_slots[INV_SLOT_2].m_pIItem;
		 ...
		 if(m_pInv->m_slots[INV_SLOT_3].m_pIItem != nullptr &&
			m_pInv->m_slots[INV_SLOT_3].m_pIItem->CanAttach(pSilencer))
		 {
			PIItem tgt = m_pInv->m_slots[INV_SLOT_3].m_pIItem;
	...
	{
		 if(m_pInv->m_slots[INV_SLOT_3].m_pIItem != nullptr &&
			m_pInv->m_slots[INV_SLOT_3].m_pIItem->CanAttach(pGrenadeLauncher))
		 {
			PIItem tgt = m_pInv->m_slots[INV_SLOT_3].m_pIItem;
