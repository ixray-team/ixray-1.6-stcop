...
MotionID animation_selector::select_animation		(bool &animation_movement_controller)
{
	...
	if (m_callback_called) {
		if (m_planner->initialized()) {
			...
			m_previous_time		= 0.f;
			if (!m_planner->initialized()) {
//				EngineLog				("%6d no planner update, planner is not initialized, exiting", Device.dwTimeGlobal);
				return			(m_object->animation().assign_global_animation(animation_movement_controller));
			}
		}

//		EngineLog					("%6d updating planner", Device.dwTimeGlobal);
		m_planner->update	();

		if (!m_planner->initialized()) {
//			EngineLog				("%6d planner is not initialized after update, exiting", Device.dwTimeGlobal);
			return			(m_object->animation().assign_global_animation(animation_movement_controller));
		}
...
//	EngineLog					( "%d on_mark", Device.dwTimeGlobal );
