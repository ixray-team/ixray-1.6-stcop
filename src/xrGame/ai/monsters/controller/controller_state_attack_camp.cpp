#include "StdAfx.h"

#include "../control_animation_base.h"
#include "../control_direction_base.h"

#include "ai_object_location.h"

#include "controller.h"

#include "controller_state_attack_camp.h"

#include "controller_animation.h"
#include "controller_direction.h"

CStateControlCamp::CStateControlCamp(CBaseMonster* object) : inherited(object)
{ 
	pControllerBase = smart_cast<CControllerBase*>(object);
}

CStateControlCamp::~CStateControlCamp()
{

}

void CStateControlCamp::initialize()
{
	inherited::initialize();

	float angle = ai().level_graph().vertex_low_cover_angle(object->ai_location().level_vertex_id(), deg(10), std::greater<float>());

	collide::rq_result	l_rq{};

	m_angle_from = angle_normalize(angle - EntityDefinitions::CControllerBase::ANGLE_DISP);
	m_angle_to = angle_normalize(angle + EntityDefinitions::CControllerBase::ANGLE_DISP);

	Fvector				trace_from{};
	object->Center(trace_from);
	Fvector				direction{};

	// trace discretely left
	for (float ang = angle; angle_difference(ang, angle) < 
		EntityDefinitions::CControllerBase::ANGLE_DISP; ang = angle_normalize(ang - EntityDefinitions::CControllerBase::ANGLE_DISP_STEP)) {

		direction.setHP(ang, 0.f);

		if (Level().ObjectSpace.RayPick(trace_from, direction,
			EntityDefinitions::CControllerBase::TRACE_STATIC_DIST, collide::rqtStatic, l_rq, object)) {
			if ((l_rq.range < EntityDefinitions::CControllerBase::TRACE_STATIC_DIST)) {
				m_angle_from = ang;
				break;
			}
		}
	}

	// trace discretely right
	for (float ang = angle; angle_difference(ang, angle) < 
		EntityDefinitions::CControllerBase::ANGLE_DISP; ang = angle_normalize(ang + EntityDefinitions::CControllerBase::ANGLE_DISP_STEP)) {

		direction.setHP(ang, 0.f);

		if (Level().ObjectSpace.RayPick(trace_from, direction, EntityDefinitions::CControllerBase::TRACE_STATIC_DIST, collide::rqtStatic, l_rq, object)) {
			if ((l_rq.range < EntityDefinitions::CControllerBase::TRACE_STATIC_DIST)) {
				m_angle_to = ang;
				break;
			}
		}
	}

	m_time_next_updated = 0;

	m_target_angle = m_angle_from;

	Fvector pos{};
	pos.mad(object->Position(), Fvector().setHP(angle, 0.f), 3.f);
	object->dir().face_target(pos);
}

void CStateControlCamp::execute()
{
	update_target_angle();

	Fvector point{};
	point.mad(object->Position(), Fvector().setHP(m_target_angle, 0.f), 3.f);

	pControllerBase->custom_dir().head_look_point(point);
	pControllerBase->custom_anim().set_body_state(CControllerAnimation::eTorsoIdle, CControllerAnimation::eLegsTypeSteal);
}

bool CStateControlCamp::check_start_conditions()
{
	if (object->EnemyMan.see_enemy_now()) return false;
	return true;
}

bool CStateControlCamp::check_completion()
{
	if (object->EnemyMan.see_enemy_now()) return true;
	if (time_state_started + 2000 < time()) return true;

	return false;
}

void CStateControlCamp::update_target_angle()
{
	if (m_time_next_updated > time()) return;
	m_time_next_updated = time() + Random.randI(
		EntityDefinitions::CControllerBase::TIME_POINT_CHANGE_MIN, EntityDefinitions::CControllerBase::TIME_POINT_CHANGE_MAX);

	if (fsimilar(m_target_angle, m_angle_from))
		m_target_angle = m_angle_to;
	else
		m_target_angle = m_angle_from;
}

