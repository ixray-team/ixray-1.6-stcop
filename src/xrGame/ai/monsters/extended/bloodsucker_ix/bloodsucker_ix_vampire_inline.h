...
#include "bloodsucker_ix_vampire_execute.h"
#include "ai/Monsters/states/state_hide_from_point.h"
#include "bloodsucker_ix_vampire_approach.h"
#include "bloodsucker_ix_vampire_hide.h"

#define TEMPLATE_SPECIALIZATION template <\
	typename _Object\
>

#define CStateBloodsuckerVampireAbstract CStateBloodsuckerIXVampire<_Object>

#define RUN_AWAY_DISTANCE 50.f

TEMPLATE_SPECIALIZATION
CStateBloodsuckerVampireAbstract::CStateBloodsuckerIXVampire(_Object* obj) : inherited(obj)
{
    add_state(eStateVampire_ApproachEnemy, new CStateBloodsuckerIXVampireApproach<_Object>(obj));
    add_state(eStateVampire_Execute, new CStateBloodsuckerIXVampireExecute<_Object>(obj));
    add_state(eStateVampire_RunAway, new CStateMonsterHideFromPoint<_Object>(obj));
    add_state(eStateVampire_Hide, new CStateBloodsuckerIXVampireHide<_Object>(obj));
}
...
void CStateBloodsuckerVampireAbstract::initialize()
{
    ...
    this->object->set_visibility_state(CAI_BloodsuckerIX::partial_visibility);

    enemy = this->object->EnemyMan.get_enemy();

    this->object->sound().play(CAI_BloodsuckerIX::eVampireStartHunt);
}
...
void CStateBloodsuckerVampireAbstract::finalize()
{
    ...
    this->object->set_visibility_state(CAI_BloodsuckerIX::full_visibility);
    CAI_BloodsuckerIX::m_time_last_vampire = Device.dwTimeGlobal;
}
...
bool CStateBloodsuckerVampireAbstract::check_start_conditions()
{
    ...
    if (CAI_BloodsuckerIX::m_time_last_vampire + this->object->m_vampire_min_delay > Device.dwTimeGlobal)
        return false;
