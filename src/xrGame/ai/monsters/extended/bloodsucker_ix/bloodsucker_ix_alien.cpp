...
public:
					CAlienEffectorIXPP	(const SPPInfo &ppi, EEffectorPPType type);
	virtual			~CAlienEffectorIXPP	();
...
CAlienEffectorIXPP::CAlienEffectorIXPP(const SPPInfo &ppi, EEffectorPPType type) :
...
CAlienEffectorIXPP::~CAlienEffectorIXPP()
{
}

#define PERIOD_SPEED	0.3f

BOOL CAlienEffectorIXPP::Process(SPPInfo& pp)
{
...
void CAlienEffectorIXPP::Destroy()
{
	fLifeTime			= 0.f;
	CAlienEffectorIXPP	*self = this;
	xr_delete			(self);
}
...
public:
					CAlienEffectorIX (ECamEffectorType type, CAI_BloodsuckerIX *obj);
...
CAlienEffectorIX::CAlienEffectorIX(ECamEffectorType type, CAI_BloodsuckerIX *obj) :
...
BOOL CAlienEffectorIX::ProcessCam(SCamEffectorInfo& info)
{
	// Инициализация
	Fmatrix	Mdef;
	Mdef.identity		();
...
CBloodsuckerIXAlien::CBloodsuckerIXAlien()
{
	m_object	= 0;
}
...
void CBloodsuckerIXAlien::init_external(CAI_BloodsuckerIX *obj)
{
	m_object	= obj;
}

void CBloodsuckerIXAlien::reinit()
{
	m_active				= false;	
	m_crosshair_show		= false;
}

void CBloodsuckerIXAlien::activate()
{
	if (m_active) return;
	...
	// Start effector
	m_effector_pp				= new CAlienEffectorIXPP (m_object->pp_vampire_effector, EFFECTOR_ID_GEN(EEffectorPPType));
	Actor()->Cameras().AddPPEffector(m_effector_pp);

	m_effector					= new CAlienEffectorIX (EFFECTOR_ID_GEN(ECamEffectorType),m_object);
...
void CBloodsuckerIXAlien::deactivate()
{
	if (!m_active) return;
