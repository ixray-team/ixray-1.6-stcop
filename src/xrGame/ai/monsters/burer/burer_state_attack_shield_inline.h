...
void CStateBurerShield<Object>::execute()
{
	...
	if (m_started &&
		this->object->shield_keep_particle != 0 &&
		current_time() > m_next_particle_allowed)
	{
		...
		m_next_particle_allowed = current_time() + this->object->shield_keep_particle_period;
	}
...
{
	if (current_time() < m_last_shield_started + this->object->shield_time + this->object->shield_cooldown)
	{
		return false;
	}
...
{
	if (current_time() > m_last_shield_started + this->object->shield_time)
	{
		return true;
	}
