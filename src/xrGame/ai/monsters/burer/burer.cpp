...
CBurer::CBurer()
{
	...
	fast_gravi = new CBurerFastGravi();

	control().add(fast_gravi, ControlCom::eComCustom1);
...
void CBurer::reload(const char* section)
{
	...
	// add specific sounds
	sound().add(pSettings->r_string(section, "sound_gravi_attack"), DEFAULT_SAMPLE_COUNT, SOUND_TYPE_MONSTER_ATTACKING, MonsterSound::eHighPriority + 2, u32(MonsterSound::eBaseChannel), eMonsterSoundGraviAttack, get_head_bone_name());

	sound().add(pSettings->r_string(section, "sound_tele_attack"), DEFAULT_SAMPLE_COUNT, SOUND_TYPE_MONSTER_ATTACKING, MonsterSound::eHighPriority + 3, u32(MonsterSound::eBaseChannel), eMonsterSoundTeleAttack, get_head_bone_name());
}

void CBurer::ActivateShield()
{
	m_shield_active = true;
	m_shield_expire_time = Device.dwTimeGlobal + shield_time;
}
...
{
	return tele_max_distance;
}

u32 CBurer::get_tele_keep_time()
{
	return tele_time_to_hold;
}
...
void CBurer::Load(const char* section)
{
	...
	gravi.radius = pSettings->r_float(section, "Gravi_Radius");
	gravi.impulse_to_objects = pSettings->r_float(section, "Gravi_Impulse_To_Objects");
	...
	stalker_difficulty_angular_speed = READ_IF_EXISTS(pSettings, r_float, section, "Stalker_Difficulty_Angular_Speed", 100.f);
	veteran_difficulty_angular_speed = READ_IF_EXISTS(pSettings, r_float, section, "Veteran_Difficulty_Angular_Speed", 100.f);
	master_difficulty_angular_speed = READ_IF_EXISTS(pSettings, r_float, section, "Master_Difficulty_Angular_Speed", 100.f);
...
void CBurer::StaminaHit()
{
	...
	float const stamina_hit = weight * weight_to_stamina_hit;

	bool const do_weapon_drop = pActor->conditions().GetPower() < stamina_hit * weapon_drop_stamina_k && !pActor->inventory().SlotIsPersistent(active_weapon->BaseSlot());
	...
	active_weapon->SetActivationSpeedOverride(normalize(dir) * weapon_drop_velocity);
	...
	{
		pDevice->SetActivationSpeedOverride(normalize(dir) * weapon_drop_velocity);
...
{
	if (!gravi_object.active)
	{
		return;
	}

	if (!gravi_object.enemy || (gravi_object.enemy && gravi_object.enemy->getDestroy()))
	{
		gravi_object.deactivate();
		return;
	}

	if (gravi_object.from_pos.distance_to(gravi_object.cur_pos) >
		gravi_object.from_pos.distance_to(gravi_object.target_pos))
	{
		gravi_object.deactivate();
		return;
	}

	float dt = float(Device.dwTimeGlobal - gravi_object.time_last_update);
	float dist = dt * float(gravi.speed) / 1000.f;
	...
	Fvector new_pos;
	Fvector dir;
	dir.sub(gravi_object.target_pos, gravi_object.cur_pos);
	dir.normalize();

	new_pos.mad(gravi_object.cur_pos, dir, dist);

	// Trace to enemy
	Fvector enemy_center;
	gravi_object.enemy->Center(enemy_center);
	dir.sub(enemy_center, new_pos);
	dir.normalize();

	float trace_dist = float(gravi.step);
	...
	if (Level().ObjectSpace.RayPick(new_pos, dir, trace_dist, collide::rqtBoth, l_rq, nullptr))
	{
		...
		if ((l_rq.O == enemy) && (l_rq.range < trace_dist))
		{
			...
			if (b_enemy_visible)
			{
				...
				HitEntity(gravi_object.enemy, gravi.hit_power, gravi.impulse_to_enemy, impulse_dir, ALife::eHitTypeStrike, false);
				gravi_object.deactivate();
				return;
			}
		}
	...
	gravi_object.cur_pos = new_pos;
	gravi_object.time_last_update = Device.dwTimeGlobal;
	...
	// установить позицию
	pos.translate_over(gravi_object.cur_pos);

	ps->UpdateParent(pos, zero_vel);
	ps->Play(false);

	// hit objects
	g_SpatialSpace->q_sphere(nearest_objects, 0, ESPATIAL_TYPE::COLLIDEABLE, gravi_object.cur_pos, gravi.radius);
	...
	for (ISpatialShared& SS : nearest_objects)
	{
		...
		Fvector dir_;
		dir_.sub(obj->Position(), gravi_object.cur_pos);
		dir_.normalize();
		obj->m_pPhysicsShell->applyImpulse(dir_, gravi.impulse_to_objects * obj->m_pPhysicsShell->getMass());
	}

	// играть звук
	Fvector snd_pos = gravi_object.cur_pos;
	snd_pos.y += 0.5f;
...
void CBurer::StopGraviPrepare()
{
	...
	{
		const CEntityAlive* enemy = gravi_object.enemy;
		if (!enemy)
		{
			return;
		}
...
void CBurer::Hit(SHit* pHDS)
{
	...
	bool IsGaussHit = EngineExternal()[EEngineExternalGame::EnableBurerShieldPenetrationWithGauss] && pHDS->hit_type == ALife::eHitTypeFireWound && pHDS->armor_piercing > shield_penetration_border;
	if (!m_shield_active || IsGaussHit)
	{
		if (IsGaussHit)
		{
			pHDS->power *= shield_penetration_damage_coeff;
		}
		inherited::Hit(pHDS);
	}
...
void CBurer::OnEvent(NET_Packet& P, u16 type)
{
	...
	switch (type)
	{
		...
		case GE_BURER_GRAVI_WAVE:
		{
			...
			target_pos.y += 0.5f;
			gravi_object.activate(entity, from_pos, target_pos);
			break;
		}
		case GE_BURER_SHIELD:
		{
			TParticlesPlayer* PPlayer = GetOrCreateComponent<TParticlesPlayer>();
			PPlayer->StartParticles(shield_keep_particle, Fvector().set(0, 1, 0), ID(), -1, true);
			break;
		}
