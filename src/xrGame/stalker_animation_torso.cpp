...
MotionID CStalkerAnimationManager::unknown_object_animation(u32 slot, const EBodyState& body_state) const
{
	...
	if (CurrentStateName == NAME_Fire1 || CurrentStateName == NAME_Fire2 ||
		CurrentStateName == NAME_Aim1 || CurrentStateName == NAME_Aim2 ||
		CurrentStateName == NAME_AimingReady1 || CurrentStateName == NAME_AimingReady2 ||
		CurrentStateName == NAME_AimQueue1 || CurrentStateName == NAME_AimQueue2)
	{
		if (standing())
		{
			return aim_animation(slot, animation, 0);
...
MotionID CStalkerAnimationManager::missile_animation(u32 slot, const EBodyState& body_state)
{
	...
	switch (m_missile->GetState())
	{
		...
		case CMissile::eBore:		return (animation[1].A[1]);
