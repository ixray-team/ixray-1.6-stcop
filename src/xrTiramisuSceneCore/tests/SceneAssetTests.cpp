...
bool HasDiagnostic(const xr_vector<FSceneDiagnostic>& Diagnostics, const xr_string_view Code)
{
...
int main()
{
	...
	{
		std::fstream Corrupt(BinaryWrite.GeometryPath, std::ios::binary | std::ios::in | std::ios::out);
	...
	if (CorruptBinary.Succeeded() ||
		!HasDiagnostic(CorruptBinary.Diagnostics, "static_mesh.binary_header_mismatch"))
	{
	...
	if (MissingBinary.Succeeded() ||
		!HasDiagnostic(MissingBinary.Diagnostics, "static_mesh.binary_read_failed"))
	{
	...
	const FStaticMeshAssetWriteResult InvalidIndex =
		SaveStaticMeshAsset(BinaryRoot / "invalid.static-mesh.json", InvalidMesh);
	if (InvalidIndex.Succeeded() ||
		!HasDiagnostic(InvalidIndex.Diagnostics, "static_mesh.index_out_of_range"))
	{
	...
	const FStaticMeshAssetWriteResult MissingMaterial =
		SaveStaticMeshAsset(BinaryRoot / "missing.static-mesh.json", InvalidMesh);
	if (MissingMaterial.Succeeded() ||
		!HasDiagnostic(MissingMaterial.Diagnostics, "static_mesh.invalid_material_slot"))
	{
	...
	if (DuplicateLight.Succeeded() ||
		!HasDiagnostic(DuplicateLight.Diagnostics, "scene.invalid_light_guid"))
	{
	...
	if (InvalidLightRange.Succeeded() ||
		!HasDiagnostic(InvalidLightRange.Diagnostics, "scene.invalid_light_range"))
	{
	...
	if (InvalidLightColor.Succeeded() ||
		!HasDiagnostic(InvalidLightColor.Diagnostics, "scene.invalid_light_radiometry"))
	{
	...
	if (InvalidLightCone.Succeeded() ||
		!HasDiagnostic(InvalidLightCone.Diagnostics, "scene.invalid_light_cone"))
	{
	...
	InvalidLightType.replace(
		LightType, xr_string("\"type\": \"spot\"").size(), "\"type\": \"area\""
	);
	...
	if (UnknownLightType.Succeeded() ||
		!HasDiagnostic(UnknownLightType.Diagnostics, "scene.invalid_light_fields"))
	{
	...
	if (DuplicateComponent.Succeeded() ||
		!HasDiagnostic(DuplicateComponent.Diagnostics, "scene.invalid_component_guid"))
	{
	...
	InvalidTransform.replace(ArrayBegin + 1, NumberEnd - ArrayBegin - 1, "\"nan\"");
	const FRenderSceneAssetParseResult NonFinite =
		ParseRenderSceneAssetJson(InvalidTransform);
	if (NonFinite.Succeeded() ||
		!HasDiagnostic(NonFinite.Diagnostics, "scene.invalid_component_fields"))
	{
	...
	{
		std::ofstream Output(TemporaryRoot / "native.render-scene.json", std::ios::binary);
	...
	InvalidOverrideScene.StaticMeshComponents[0]
		...
		.MaterialSlot = 1;
	{
		std::ofstream Output(TemporaryRoot / "invalid-override.render-scene.json", std::ios::binary);
		Output << SerializeRenderSceneAssetJson(InvalidOverrideScene);
	}
	const FResolvedRenderSceneResult InvalidOverride =
		LoadRenderSceneAsset(TemporaryRoot / "invalid-override.render-scene.json");
	if (InvalidOverride.Succeeded() ||
		!HasDiagnostic(InvalidOverride.Diagnostics, "scene.material_override_out_of_range"))
	{
	...
	Dump.AssetMappings.push_back({Dump.SourcePath, Dump.TargetPath, Dump.TargetPath + ".migration.json", Mesh.Id, Dump.TargetPayloadPath});
	Dump.MaterialMappings.push_back(
		{"Surface", "legacy-key", Mesh.MaterialSlots[0].Material, false, true}
	);
