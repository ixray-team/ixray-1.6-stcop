...
### Чем отличается билд для игрока и разработчика?

В сборке для разработчика доступен полный внутриигровой инструментарий, включая отладочные консольные команды и читы для тестирования. В сборке для игроков эти возможности отключены - предоставляется чистый движок без дополнительных инструментов. Различается только движок, игровые ресурсы в обеих версиях одинаковы. Модмейкерам рекомендуется начинать работу со сборки для разработчика, а перед релизом заменять папку bin на версию из сборки для игроков

### Нужен ли для некоторых аддонов (например, оружейных паков) собственный форк движка?

Все наши официальные аддоны — такие как STCoP WP, Gunslinger Mod и другие — используют единую кодовую базу движка. Они не требуют отдельных форков или изменений в движке. Если какая-то модификация всё же нуждается в собственном форке, это должно быть явно указано в описании проекта
...
### Помимо движка CoP увидел у вас форки других частей игры, каков их статус?

После интеграции SoC, CS и CoC в основной репозиторий CoP форки движков этих платформ были отправлены в архив. Разработка поддержки SoC, CS и CoC продолжается в рамках основного проекта

### Вы планируете адаптацию модификации Anomaly?

Нет, таких планов нет. Хотя мы понимаем популярность этой платформы, её адаптация потребовала бы тесного сотрудничества с разработчиками оригинальной модификации и их желанием переезда, а также полной переработки как самого мода, так и всех аддонов под специфику платформы IX-Ray. Ключевая особенность Anomaly - её экосистема модификаций и расширений, которые без полного переписывания несовместимы с IX-Ray

### Чем ваш движок отличается от других?

Мы предлагаем полноценную платформу для разработки: игровой движок со встроенным инструментарием, полный набор редакторов, утилит, плагинов и подробную документацию. У каждого серьёзного проекта есть своя документация, релизы и описание - вы можете изучить их и сравнить, чтобы выбрать то, что подходит именно вам. Важно также самостоятельно проверить работоспособность каждой интересующей вас платформы, а не полагаться только на декларируемые возможности
...
### Планируется ли адаптация Gunslinger/STCoP WP/IWP/моей_любимой_оружейки?
